...
- the comment is fetched through the trusted workflow token from the GitHub issue-comments API (comment bytes are data; head-controlled code is never executed); every page is scanned and the **newest** record wins, and any API failure fails closed;
