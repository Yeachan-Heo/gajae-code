...
`robogjc` is a self-hosted GitHub triage-and-fix bot that drives [`gjc --mode rpc`](https://github.com/Yeachan-Heo/gajae-code) as a subprocess. On every issue opened in an allowlisted repository it classifies the issue, applies labels, then branches into one of: reproduce → fix → PR (`bug` / `documentation`), single-comment answer (`question`), single thoughtful comment (`enhancement` / `proposal`), or brief comment (`invalid` / `duplicate`). Follow-up comments and PR review comments resume the same gjc session so the agent keeps its prior reasoning. If the orchestrator restarts mid-task, the dispatcher resumes the same session via `gjc --continue` from the per-issue `session_dir`, so an interrupted task re-enters its prior reasoning instead of restarting from scratch. The orchestrator runs as a single FastAPI process inside Docker with SQLite-backed durable event state.

## Architecture & Data Flow

Webhook → durable queue → async dispatcher → per-issue git worktree → gjc RPC subprocess + host tools.
...
6. `tasks.*` dispatchers build `TaskInputs` and call `worker.run_task()` which spawns `gjc --mode rpc` with `cwd=worktree`, persistent `session_dir`, and a randomly-picked model from `ROBGJC_MODEL` (CSV pool). When `<session_dir>/*.jsonl` already exists the worker passes `--continue`, so both follow-up events and crash-restarted events resume the same session.
7. Inside the subprocess the agent uses **built-in gjc tools** (read/edit/write/bash/lsp, scoped to the worktree) and **host tools** from `host_tools.py` (the only surface allowed to mutate GitHub or write audit rows).
...
- `/Dockerfile` (pi root) — produces `gajae-code/pi:dev` (pi runtime image: python + bun + rustup + pi-natives + gjc_rpc + `/usr/local/bin/gjc` shim + the full pi source under `/pi`). Stages: `natives-builder` → `wheel-builder` → `pi-base` → `pi-runtime` (default). Built via `bun run pi:image`. Robogjc's image extends `pi-base` via `FROM ${PI_BASE}` in `/Dockerfile.robogjc`.
...
bun run robogjc:test:integration   # ROBGJC_INTEGRATION=1, requires gjc on PATH
...
- `src/worker.py` — synchronous gjc RPC driver, prompt assembly via `persona`.
...
- **Container runtime**: Docker Compose v2. The image embeds Bun 1.3.14 + a rustup launcher and exposes `gjc` via a `/usr/local/bin/gjc` shim; `ROBGJC_GJC_COMMAND=gjc` should not need changing.
...
- **Integration**: `tests/test_worker_smoke.py` is gated by `ROBGJC_INTEGRATION=1` (uses `pytestmark.skipif`) and needs `gjc` on `PATH`. Don't enable it in default `bun run test:py`.
