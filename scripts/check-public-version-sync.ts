...
const GENERATED_DOCS_PAYLOAD = "packages/coding-agent/src/internal-urls/docs-payload.generated.bin";
...
export async function checkPublicVersionSync(repoRoot = path.join(import.meta.dir, "..")): Promise<SyncViolation[]> {
	...
	if (await pathExists(path.join(repoRoot, "docs"))) {
		const expected = await buildDocsIndexOutputs(repoRoot);
		for (const [relativePath, expectedOutput] of [
			[GENERATED_DOCS_INDEX, expected.index],
			[GENERATED_DOCS_PAYLOAD, expected.payload],
		] as const) {
			const generatedPath = path.join(repoRoot, relativePath);
			if (!(await pathExists(generatedPath))) {
				violations.push({ path: relativePath, message: "Generated docs output is missing." });
				continue;
			}
			const actual = new Uint8Array(await Bun.file(generatedPath).arrayBuffer());
			const expectedBytes =
				typeof expectedOutput === "string" ? new TextEncoder().encode(expectedOutput) : expectedOutput;
			if (
				...
				violations.push({
					path: relativePath,
					message: "Generated docs output is stale. Run bun run generate-docs-index.",
				});
			}
		}
	}
