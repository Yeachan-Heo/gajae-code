...
SESSION="$1"
[[ "$SESSION" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]{0,127}$ ]] || { echo "unsafe session name: $SESSION" >&2; exit 2; }
[[ -d "$2" ]] || { echo "directory not found: $2" >&2; exit 1; }
WORKDIR="$(cd "$2" && pwd -P)"
...
if [[ "$STATE_DIR" != /* ]]; then STATE_DIR="$(pwd -P)/$STATE_DIR"; fi
...
PY
  then
    ...
    if [[ ! "$grace_seconds" =~ ^[0-9]+$ || "${#grace_seconds}" -gt 3 ]]; then
      grace_seconds=3
    elif (( 10#$grace_seconds > 300 )); then
      grace_seconds=3
    fi
...
try:
    ...
    print(json.dumps({"state":"current", **record, "root_dev":str(root_identity.st_dev), "root_ino":str(root_identity.st_ino)}, separators=(",", ":")))
finally:
    os.close(root_fd)
PY
