...
describe("dev:link", () => {
	test.skipIf(process.platform !== "win32")(
		"finds a Windows gjc executable suffix on PATH during doctor checks",
		async () => {
			const root = await fs.mkdtemp(path.join(os.tmpdir(), "gjc-dev-link-win-path-"));
			tempRoots.push(root);
			const shadowDir = path.join(root, "shadow-bin");
			await fs.mkdir(shadowDir, { recursive: true });
			await Bun.write(path.join(shadowDir, "gjc.exe"), "");

			const result = Bun.spawnSync([process.execPath, "scripts/dev-link.ts", "--check"], {
				env: {
					...process.env,
					PATH: shadowDir,
					Path: shadowDir,
			...
			expect(result.stdout.toString()).toContain(`gjc resolves to: ${path.join(shadowDir, "gjc.exe")}`);
			expect(result.stderr.toString()).toContain("is NOT this checkout's source");
			expect(result.stderr.toString()).not.toContain("is not on PATH");
		},
	);

	test.skipIf(process.platform === "win32")(
		"fails when a shadow gjc earlier on PATH would make smoke-test validate the wrong command",
		async () => {
