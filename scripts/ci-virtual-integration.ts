...
async function baseIsReachable(baseSha: string): Promise<boolean> {
	const result = await $`git merge-base --is-ancestor ${baseSha} origin/dev`.cwd(repoRoot).quiet().nothrow();
 ...
 */
export async function runCanaries(options: { repoDir?: string; runner?: (testPath: string, cwd: string) => Promise<number> } = {}): Promise<void> {
