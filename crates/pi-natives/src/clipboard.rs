// Vendored from oh-my-pi (MIT) crates/pi-natives/src/clipboard.rs @ a85bd5228d9f0f619deade1db78fa49420a721e1
// Local modifications: use a private JsString helper until crate::js lands, tag synchronous text copy, and read DIB pixels from a golden fixture.
...
fn clipboard_text(text: JsString) -> Result<String> {
	text
		.into_utf8()
...
pub fn copy_to_clipboard(text: JsString) -> Result<()> {
	let _profile = crate::prof::profile_region("clipboard.copy_text");
	set_clipboard_text(&clipboard_text(text)?)
}
...
mod tests {
	...
	fn expected_pixels() -> Vec<[u8; 4]> {
		serde_json::from_str(include_str!(
			"../../../packages/natives/test/fixtures/goldens/clipboard/dib-rgba.json"
		))
		.expect("clipboard DIB golden is valid RGBA JSON")
	}
