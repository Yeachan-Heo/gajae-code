// Source: oh-my-pi `crates/pi-natives/src/workspace.rs`
// Pinned revision: a85bd5228d9f0f619deade1db78fa49420a721e1 (MIT).
// Gajae-Code modifications: use pi-walker scans and pi-vfs metadata for
// workspace results.
...
fn file_type_from_vfs(file_type: pi_vfs::FileType) -> Option<FileType> {
...
fn metadata_mtime(metadata: &pi_vfs::Metadata) -> Option<f64> {
	metadata
		.modified()
		.ok()
...
fn glob_match_from_path(config: &WorkspaceConfig, path: &Path) -> Option<GlobMatch> {
	let relative = pi_walker::normalize_relative_path(&config.root, path);
	if relative.is_empty() {
		return None;
	}
	let metadata = config.fs.symlink_metadata(path).ok()?;
	let file_type = file_type_from_vfs(metadata.file_type())?;
	let size = (file_type == FileType::File).then(|| metadata.len() as f64);
	Some(GlobMatch {
...
fn is_file_or_file_symlink(fs: &BlockingFs, path: &Path, file_type: FileType) -> bool {
	match file_type {
		FileType::File => true,
		FileType::Symlink => fs.metadata(path).is_ok_and(|metadata| metadata.is_file()),
		FileType::Dir => false,
	}
}
...
fn collect_agents_md_in_directory(
	...
	let Some(entry) = glob_match_from_path(config, &candidate) else {
		return;
	};
	if !is_file_or_file_symlink(&config.fs, &candidate, entry.file_type) {
		return;
	}
...
pub fn list_workspace(options: ListWorkspaceOptions<'_>) -> task::Promise<ListWorkspaceResult> {
	...
	task::blocking("listWorkspace", ct, move |ct| {
		...
		let root = iofs::resolve_search_dir(&fs, &path)?;
...
mod tests {
	...
	#[test]
	fn workspace_walks_virtual_files_and_surfaces_file_ignored_agents() {
		let (_mem, fs) = MemFs::with_files(&[
			("mem://workspace/.gitignore", "AGENTS.md\nskip/\n"),
			("mem://workspace/AGENTS.md", "root rules"),
			("mem://workspace/src/AGENTS.md", "src rules"),
			("mem://workspace/src/main.ts", "source"),
			("mem://workspace/skip/AGENTS.md", "ignored rules"),
		]);
		let result = run_list_workspace(
			WorkspaceConfig {
				fs,
				root: PathBuf::from("mem://workspace"),
				max_depth: 2,
				walk_max_depth: AGENTS_MD_MAX_DEPTH,
		...
		.expect("virtual workspace scan succeeds");
		...
		assert_eq!(
			result
				...
				.collect::<Vec<_>>(),
			["AGENTS.md", "src", "src/AGENTS.md", "src/main.ts"]
		);
		assert_eq!(result.agents_md_files, ["src/AGENTS.md"]);
