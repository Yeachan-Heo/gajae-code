// Vendored from oh-my-pi (MIT) crates/pi-natives/src/power.rs @ a85bd5228d9f0f619deade1db78fa49420a721e1
// Local modifications: use GJC identity strings and tag public start/stop operations with profile_region.
...
impl PowerAssertion {
	...
	pub fn start(options: Option<PowerAssertionOptions>) -> napi::Result<Self> {
		let _profile = crate::prof::profile_region("power.start");
		let reason = options
			.as_ref()
	...
	pub fn stop(&mut self) -> napi::Result<()> {
		let _profile = crate::prof::profile_region("power.stop");
		#[cfg(target_os = "macos")]
		{
