...
// Modified for gajae-code: yes — lone-surrogate normalization; private UTF-16 helper until P21 lands.
...
//!   is read as UTF-16 and transcoded with `xutf` straight into the shared back
//!   buffer.
...
use napi::{Error, JsString, JsStringUtf16, Result};
use napi_derive::napi;
use parking_lot::{Condvar, Mutex};

/// Read a JS string as UTF-16 while keeping its N-API backing buffer alive.
/// `napi::JsString::into_utf16` includes a trailing NUL in its view, so retain
/// the reported code-unit length separately and slice it off at the callsite.
/// P21's `crate::js::utf16` replaces this allocation when the shared helper is
/// integrated.
fn utf16(data: JsString<'_>) -> Result<(JsStringUtf16<'_>, usize)> {
	let length = data.utf16_len()?;
	Ok((data.into_utf16()?, length))
}
...
fn append_valid_utf16_segment(units: &[u16], output: &mut Vec<u8>) {
	...
	output.resize(start + xutf::transcoded_len::<xutf::Utf16, xutf::Utf8>(units), 0);
...
fn append_utf16(units: &[u16], output: &mut Vec<u8>) -> usize {
	...
	while index < units.len() {
		...
		let low_surrogate = (0xdc00..=0xdfff).contains(&unit);
		if high_surrogate
			&& units
				.get(index + 1)
		...
		if high_surrogate || low_surrogate {
			append_valid_utf16_segment(&units[segment_start..index], output);
	...
	append_valid_utf16_segment(&units[segment_start..], output);
	output.len() - start
}
struct Inner {
...
fn pump_loop(fd: i32, inner: &Inner) {
	...
	loop {
		...
		if inner.dead.load(Ordering::Acquire) {
			...
			// Track each completed write so `pending()` reflects bytes still
			// waiting on a slow terminal rather than whole-batch completion.
...
impl TtyWriter {
	...
	pub fn write(&self, data: JsString) -> Result<u32> {
		...
		let (units, length) = utf16(data)?;
		let units = &units.as_slice()[..length];
		if units.is_empty() {
			return Ok(self.pending());
		}
...
mod tests {
	...
	#[test]
	fn unpaired_surrogates_match_javascript_utf8_replacement() {
		let units = [b'a' as u16, 0xd800, b'b' as u16, 0xdc00, 0xd840, 0xdc00];
		let expected = "a\u{FFFD}b\u{FFFD}𠀀".as_bytes();
		let mut output = Vec::new();

		assert_eq!(append_utf16(&units, &mut output), expected.len());
	...
	fn pending_tracks_partial_drain_progress() {
		...
		// SAFETY: closing the test-owned fd.
		unsafe { libc::close(read_fd) };
	}
