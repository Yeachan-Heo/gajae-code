// Source: oh-my-pi `crates/pi-natives/src/testing.rs`
// Pinned revision: a85bd5228d9f0f619deade1db78fa49420a721e1 (MIT).
// Gajae-Code modifications: pi-vfs memory test helpers restored during 2.4.
...
mod mem_fs_tests {
	...
	#[test]
	fn blocking_filesystem_reads_memory_files_and_metadata() {
		let (mem, fs) = MemFs::with_files(&[("mem://root/file.txt", "hello")]);

		assert_eq!(fs.read_to_string("mem://root/file.txt").unwrap(), "hello");
		assert_eq!(mem.contents("mem://root/file.txt").as_deref(), Some("hello"));
		assert_eq!(fs.metadata("mem://root/file.txt").unwrap().len(), 5);
		assert!(fs.metadata("mem://root").unwrap().is_dir());
	}
}
