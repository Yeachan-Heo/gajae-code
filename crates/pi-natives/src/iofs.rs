// Vendored from oh-my-pi (MIT) crates/pi-natives/src/iofs.rs @
// a85bd5228d9f0f619deade1db78fa49420a721e1 Modified for gajae-code: yes —
// walker pool diagnostic; uses the shared crate::js UTF-8 conversion
...
/// construction.
#[napi]
pub fn walker_pool_status() -> String {
	pi_walker::walk_pool_status().to_owned()
}
...
pub fn invalidate_fs_scan_cache(path: Option<JsString>) -> Result<()> {
	match path {
		Some(path) => {
			let utf8 = crate::js::utf8(path)?;
			pi_walker::invalidate_path_string(&utf8);
		},
