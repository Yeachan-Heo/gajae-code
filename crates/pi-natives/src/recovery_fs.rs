...
#[cfg(all(test, target_os = "linux"))]
static APPEND_FILE_SYNC_FAULT: OnceLock<std::sync::Mutex<bool>> = OnceLock::new();
...
#[cfg(all(test, target_os = "linux"))]
fn set_append_file_sync_fault(fault: bool) {
	*APPEND_FILE_SYNC_FAULT
		.get_or_init(|| std::sync::Mutex::new(false))
		.lock()
		.unwrap_or_else(|poisoned| poisoned.into_inner()) = fault;
}
...
#[cfg(target_os = "linux")]
fn sync_append_file(file: &File) -> std::io::Result<()> {
	#[cfg(test)]
	if *APPEND_FILE_SYNC_FAULT
		.get_or_init(|| std::sync::Mutex::new(false))
		.lock()
		.unwrap_or_else(|poisoned| poisoned.into_inner())
	{
...
fn restore_exchanged_successor(
	...
	let Ok(named) = statat(parent, destination) else {
		return false;
	};
	if named.st_dev.to_string() != staged_identity.dev
		|| named.st_ino.to_string() != staged_identity.ino
		|| named.st_nlink != 1
	{
		return false;
	}
	// SAFETY: parent owns a live directory descriptor and destination is a
	...
	// SAFETY: destination_fd is the uniquely owned descriptor returned by openat.
	let destination_file = unsafe { File::from_raw_fd(destination_fd) };
	let Ok(destination_identity) = regular_identity(&destination_file) else {
		return false;
	};
	let Ok(destination_digest) = digest_hex(&destination_file) else {
		return false;
	};
	if destination_identity.dev != staged_identity.dev
		|| destination_identity.ino != staged_identity.ino
		|| destination_identity.size != staged_identity.size
		|| destination_identity.mtime_ns != staged_identity.mtime_ns
		|| staged_identity.sha256.as_deref() != Some(destination_digest.as_str())
	{
		return false;
	}
...
impl ManagedReplacementStageGuard<'_> {
	fn retire(&mut self) -> bool {
		...
		let retired =
			rename_file_no_replace(self.parent, &self.name, self.recovery, &self.quarantine, || {})
				.is_ok();
		self.active = false;
		retired
	}
...
#[cfg(target_os = "linux")]
/// Acquire the stable advisory lock shared by managed append, move, and remove
/// operations rooted at a retained recovery directory. This lock coordinates
/// cooperating managed writers; it is not used as proof for an existing-name
...
fn append_managed(
	...
	if sync_append_file(&file).is_err() {
		return RecoveryFsResult::append_failure(
			"fsync_failed",
			"committed",
			"not_provable",
...
mod tests {
	...
	#[test]
	fn managed_append_preserves_receipt_when_file_sync_is_unprovable() {
		let _serial = APPEND_TEST_SERIAL
			.get_or_init(|| std::sync::Mutex::new(()))
			.lock()
			.unwrap_or_else(|poisoned| poisoned.into_inner());
		...
		let appended = b"\nappend";
		let identity = managed_file(&root, "transcript", original);
		set_append_file_sync_fault(true);
		let result = append_managed(
			&root,
			None,
			"transcript",
		...
		set_append_file_sync_fault(false);
	...
	fn managed_replace_post_final_verify_successor_is_not_overwritten() {
		...
		// Model an uncoordinated writer winning immediately after the caller's
		// final identity observation: the authorized inode is retained elsewhere
		// and a successor now owns the canonical name. The replacement API has no
		// safe exchange step to run after this point, so it must fail closed.
		fs::rename(temporary.0.join("transcript"), temporary.0.join("retained-predecessor"))
