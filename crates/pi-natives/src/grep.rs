// Vendored from
// can1357/oh-my-pi@a85bd5228d9f0f619deade1db78fa49420a721e1:crates/pi-natives/
// src/grep.rs — MIT (c) 2025 Mario Zechner, 2025-2026 Can Bölük, 2026 Stencil
// Labs, Inc. Modified for gajae-code: yes — restored shared-scan caching and
// its unavailable-Rayon serial fallback; omitted upstream-only injectable
// shell::vfs filesystem support; renamed the PCRE2 JIT override.
...
use crate::{fs_cache, glob_util, iofs, task};

const MAX_FILE_BYTES: u64 = 4 * 1024 * 1024;
/// PCRE2 JIT toggle: `GJC_PCRE2_JIT=1` forces JIT on, `0`/`false` forces it
...
static PCRE2_JIT_ENABLED: LazyLock<bool> = LazyLock::new(|| match std::env::var("GJC_PCRE2_JIT") {
...
fn collect_cached_grep_candidates(
	root: &Path,
	scanned_entries: &[fs_cache::GlobMatch],
	glob_set: Option<&GlobSet>,
	type_filter: Option<&TypeFilter>,
) -> Vec<pi_walker::FileCandidate> {
	scanned_entries
		.iter()
		.filter_map(|entry| {
			...
			if let Some(filter) = type_filter
				&& !matches_type_filter_str(&entry.path, filter)
			{
				return None;
			}
			Some(pi_walker::FileCandidate {
...
fn grep_sync_with_matcher<M: Matcher + Sync>(
	...
	let glob_set = glob_util::try_compile_glob(glob, true)?;
	...
	let (results, _skipped_oversized, files_searched) = if options.cache {
		...
		let mut candidates = collect_cached_grep_candidates(
			&search_path,
			&scan.entries,
			glob_set.as_ref(),
			type_filter.as_ref(),
		);
		if candidates.is_empty() && scan.cache_age_ms >= fs_cache::empty_recheck_ms() {
			let fresh = fs_cache::force_rescan(&search_path, scan_options, true, &ct)?;
			candidates = collect_cached_grep_candidates(
				&search_path,
				fresh.as_ref(),
				glob_set.as_ref(),
				type_filter.as_ref(),
...
mod tests {
	...
	#[cfg(unix)]
	#[test]
	fn grep_cached_path_uses_serial_search_when_rayon_pool_unavailable() {
		struct RestorePoolAvailability(bool);
		impl Drop for RestorePoolAvailability {
			fn drop(&mut self) {
				crate::RAYON_POOL_UNAVAILABLE.store(self.0, Ordering::SeqCst);
			}
		}

		let root = TempDirGuard::new();
		for index in 0..300 {
			write_file(&root.path().join(format!("file_{index:03}.txt")), "needle\n");
		}
		let mut config = base_grep_config(root.path());
		config.cache = true;
		let previous = crate::RAYON_POOL_UNAVAILABLE.swap(true, Ordering::SeqCst);
		let _restore = RestorePoolAvailability(previous);

		let result = grep_sync(config, None, task::CancelToken::default())
			.expect("cached grep should succeed without the local Rayon pool");
		assert_eq!(result.total_matches, 300);
		assert_eq!(result.files_searched, 300);
	...
	fn sequential_reference_result(root: &Path, params: super::SearchParams) -> super::GrepResult {
		...
		let (results, _skipped_oversized, files_searched) = super::run_sequential_grep(
			&BlockingFs::native(),
			root,
			&matcher,
			None,
