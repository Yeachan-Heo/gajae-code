//! Hashline formatting core, compatible with
//! `packages/coding-agent/src/hashline/hash.ts`.
//!
//! Moved from `pi-natives/src/hashline.rs`. Operates on UTF-16 code units so
//! the NAPI facade can pass JS string data through zero-copy; the facade owns
//! only the `JsString`/`Utf16String` interop.
...
/// Format text (as UTF-16 code units, no trailing NUL) into hashline form:
/// each line prefixed with `<lineNumber><2-char-hash>|`.
pub fn format_hash_lines_utf16(text: &[u16], start_line: u32) -> Vec<u16> {
...
mod tests {
	...
	fn format_str(text: &str, start_line: u32) -> String {
		let units: Vec<u16> = text.encode_utf16().collect();
		String::from_utf16(&format_hash_lines_utf16(&units, start_line)).unwrap()
	}

	#[test]
	fn formats_lines_with_number_hash_prefix() {
		let out = format_str("alpha\nbeta", 1);
		let lines: Vec<&str> = out.split('\n').collect();
		assert_eq!(lines.len(), 2);
		assert!(lines[0].starts_with('1') && lines[0].ends_with("|alpha"));
		assert!(lines[1].starts_with('2') && lines[1].ends_with("|beta"));
		// 2-char bigram hash between number and separator
		assert_eq!(lines[0].len(), "1".len() + 2 + 1 + "alpha".len());
	}

	#[test]
	fn hash_is_stable_and_trailing_ws_insensitive() {
		let a = format_str("code", 5);
		let b = format_str("code   ", 5);
		// same content hash: prefix before '|' identical
		assert_eq!(a.split('|').next(), b.split('|').next());
	}

	#[test]
	fn respects_start_line() {
		assert!(format_str("x", 42).starts_with("42"));
	}
}
...
mod ts_parity {
	...
	fn format_str(text: &str) -> String {
		let units: Vec<u16> = text.encode_utf16().collect();
		String::from_utf16(&format_hash_lines_utf16(&units, 1)).unwrap()
	}

	/// Vectors generated by packages/coding-agent/src/hashline/hash.ts
	/// (`formatHashLines`).
	#[test]
	fn matches_ts_reference_vectors() {
		let cases = [
			...
			("code   ", "1zn|code   "),
			("tab\tend\t", "1ry|tab\tend\t"),
			("unicode ✓ line\nsecond 🎉", "1ra|unicode ✓ line\n2jn|second 🎉"),
			("", "1yy|"),
		];
		for (input, expected) in cases {
			assert_eq!(format_str(input), expected, "input {input:?}");
		}
	}
}
