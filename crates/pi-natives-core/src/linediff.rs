//! Line-level diff: a byte-identical Rust port of jsdiff v9.
//!
//! Ports `Diff.diffLines(oldStr, newStr)` with default options, used by the
//! edit tool's `generateDiffString`. Moved from `pi-natives/src/linediff.rs`;
//! the NAPI facade re-wraps the parts into `#[napi(object)]` structs.
//!
