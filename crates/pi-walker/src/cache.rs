...
/// Process-wide filesystem scan and shared-cache budgets.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ScanPolicy {
	/// Maximum entries admitted by one complete scan.
	pub max_entries:   usize,
	/// Maximum logical and retained-capacity bytes admitted by one scan.
	pub max_bytes:     usize,
	/// Maximum complete snapshots retained in the shared cache.
	pub cache_entries: usize,
	/// Maximum retained snapshot bytes in the shared cache.
...
impl ScanCache {
	...
	fn remove(&mut self, key: &CacheKey) {
		if let Some(entry) = self.entries.remove(key) {
			if let Some(bytes) = self.bytes.checked_sub(entry.bytes) {
				self.bytes = bytes;
			} else {
				self.clear();
				self.publication_disabled = true;
			}
		}
	}
	...
	fn expire(&mut self, now: Instant) {
		let expired: Vec<_> = self
			.entries
			.iter()
			.filter(|(_, entry)| now.saturating_duration_since(entry.created_at) >= self.ttl)
			.map(|(key, _)| key.clone())
			.collect();
		for key in expired {
			self.remove(&key);
	...
	fn insert(&mut self, key: CacheKey, entry: CacheEntry, generation: u64, now: Instant) {
		...
		self.remove(&key);
		let remaining_bytes = self.max_bytes - entry.bytes;
		while self.entries.len() >= self.max_entries || self.bytes > remaining_bytes {
			let oldest = self
				.entries
				.iter()
	...
	fn invalidate(&mut self, target: Option<&Path>) {
		...
		let matching: Vec<_> = self
			.entries
			.keys()
			.filter(|key| target.is_none_or(|target| target.starts_with(&key.root)))
			.cloned()
			.collect();
		for key in matching {
			self.remove(&key);
...
static SCAN_POLICY: LazyLock<Result<ScanPolicy, String>> = LazyLock::new(|| {
	Ok(ScanPolicy {
...
/// Read the validated process-wide scan and cache policy.
pub fn scan_policy() -> Result<ScanPolicy, String> {
	SCAN_POLICY.as_ref().copied().map_err(Clone::clone)
}
...
const WALK_POOL_UNINITIALIZED: u8 = 0;
const WALK_POOL_READY: u8 = 1;
const WALK_POOL_UNAVAILABLE: u8 = 2;
...
static WALK_POOL_STATUS: AtomicU8 = AtomicU8::new(WALK_POOL_UNINITIALIZED);
static WALK_POOL: LazyLock<Option<ThreadPool>> = LazyLock::new(|| {
	let workers = walk_workers();
	if workers <= 1 {
		return None;
	}
	// Test hook: the N-API/Tokio runtime may already have cached RUST_MIN_STACK
	// before a walker-backed export first forces this lazy pool.
	if std::env::var_os("PI_WALKER_TEST_FORCE_POOL_UNAVAILABLE").is_some() {
		WALK_POOL_STATUS.store(WALK_POOL_UNAVAILABLE, AtomicOrdering::Release);
		return None;
	}
	if let Ok(pool) = rayon::ThreadPoolBuilder::new()
		...
		.build()
	{
		WALK_POOL_STATUS.store(WALK_POOL_READY, AtomicOrdering::Release);
		Some(pool)
	} else {
		WALK_POOL_STATUS.store(WALK_POOL_UNAVAILABLE, AtomicOrdering::Release);
		None
...
static SCAN_CACHE: LazyLock<Mutex<ScanCache>> = LazyLock::new(|| {
	let policy = scan_policy().unwrap_or(ScanPolicy {
		max_entries:   SCAN_MAX_ENTRIES_DEFAULT,
		max_bytes:     SCAN_MAX_BYTES_DEFAULT,
		cache_entries: CACHE_MAX_ENTRIES_DEFAULT,
		cache_bytes:   CACHE_MAX_BYTES_DEFAULT,
	});
	Mutex::new(ScanCache::new(
		Duration::from_millis(*CACHE_TTL_MS),
		policy.cache_entries,
		policy.cache_bytes,
	))
});
...
pub fn max_cache_entries() -> usize {
	scan_policy().map_or(CACHE_MAX_ENTRIES_DEFAULT, |policy| policy.cache_entries)
}
...
fn walk_pool() -> Option<&'static ThreadPool> {
	WALK_POOL.as_ref()
}

/// Walker pool initialization status without forcing lazy construction.
pub fn walk_pool_status() -> &'static str {
	match WALK_POOL_STATUS.load(AtomicOrdering::Acquire) {
		WALK_POOL_UNINITIALIZED => "uninitialized",
		WALK_POOL_READY => "ready",
		_ => "unavailable",
	}
}

/// Force walker pool initialization and report whether it was created.
pub fn walk_pool_available() -> bool {
	walk_pool().is_some()
}

/// Run an operation on the walker pool, or leave it unexecuted if unavailable.
pub fn with_walk_pool<R>(operation: impl FnOnce() -> R + Send) -> Option<R>
where
	R: Send,
{
	walk_pool().map(|pool| pool.install(operation))
}
...
pub fn should_parallelize(item_count: usize) -> bool {
	walk_workers() > 1 && item_count >= PARALLEL_MIN_FILES && walk_pool().is_some()
}

fn run_pooled_or_serial<T, S, E, I, F>(
...
where
	...
	E: Send,
	I: Fn() -> S + Send + Sync,
	F: Fn(&mut S, &T) -> std::result::Result<(), E> + Send + Sync,
{
	if let Some(pool) = pool {
		pool.install(|| items.par_iter().try_for_each_init(init, operation))
	} else {
		let mut state = init();
		items
			.iter()
...
where
	...
	E: Send,
{
	let pool = (walk_workers() > 1 && items.len() >= PARALLEL_MIN_FILES)
		.then(walk_pool)
		.flatten();
	run_pooled_or_serial(pool, items, || (), |(), item| operation(item))
}
	...
	E: Send,
{
	let pool = (walk_workers() > 1 && items.len() >= PARALLEL_MIN_FILES)
		.then(walk_pool)
		.flatten();
	run_pooled_or_serial(pool, items, init, operation)
}
...
mod parallel_tests {
	...
	#[test]
	fn parallel_for_each_serial_when_pool_unavailable() {
		for count in [256, 1024] {
			let items: Vec<usize> = (0..count).collect();
			let calling_thread = thread::current().id();
			let seen = Mutex::new(Vec::new());
			let failure_at = count / 3;
			let result = run_pooled_or_serial(
				None,
				&items,
				|| (),
				|_, item| {
			...
			assert_eq!(*seen.lock().unwrap(), (0..=failure_at).collect::<Vec<_>>());
		}
	}

	#[test]
	fn parallel_for_each_init_serial_when_pool_unavailable() {
		for count in [256, 1024] {
			let items: Vec<usize> = (0..count).collect();
			let calling_thread = thread::current().id();
			...
			assert_eq!(result, Err(count / 4));
			assert_eq!(init_calls.load(Ordering::Relaxed), 1);
			assert_eq!(*seen.lock().unwrap(), (0..=count / 4).collect::<Vec<_>>());
		}
	}

	#[test]
	fn parallel_branch_runs_inside_walker_pool() {
		let pool = rayon::ThreadPoolBuilder::new()
			.num_threads(4)
			.thread_name(|index| format!("pi-walker-test-{index}"))
			.build()
			.unwrap();
		...
		let result = run_pooled_or_serial(
			...
			|_, _| {
				...
				assert!(name.starts_with("pi-walker-"), "unexpected worker name {name:?}");
...
{
	options.cache = false;
	let scan = crate::scan_entries(fs, root, options, policy, || {
...
where
	...
	E: fmt::Display,
{
	if *CACHE_TTL_MS == 0 || policy.cache_entries == 0 || policy.cache_bytes == 0 {
		return collect_entries_uncached(fs, root, options, policy, heartbeat);
	}
	...
	let scan = collect_entries_uncached(fs, root, options, policy, heartbeat)?;
	...
	E: fmt::Display,
{
	let policy = scan_policy()
		.map_err(|message| WalkError::InvalidData { path: root.to_path_buf(), message })?;
	if options.cache && shares_scan_cache(fs, root) {
		get_or_scan(fs, root, options, policy, &heartbeat)
	} else {
		collect_entries_uncached(fs, root, options, policy, &heartbeat)
	}
}
...
mod tests {
	...
	#[test]
	fn scan_limit_parser_enforces_bounds_and_zero_policy() {
		assert_eq!(super::parse_limit_value("LIMIT", None, 17, 10, 20, false).unwrap(), 17);
		assert_eq!(super::parse_limit_value("LIMIT", Some("10"), 17, 10, 20, false).unwrap(), 10);
		assert_eq!(super::parse_limit_value("LIMIT", Some("20"), 17, 10, 20, false).unwrap(), 20);
		assert_eq!(super::parse_limit_value("LIMIT", Some("0"), 17, 10, 20, true).unwrap(), 0);
		for (value, reason) in [
			("nope", "malformed"),
			("-1", "signed"),
			("+10", "signed"),
			...
			let error = super::parse_limit_value("LIMIT", Some(value), 17, 10, 20, false)
				.expect_err("invalid explicit limits must fail");
			assert!(error.contains(&format!("reason={reason}")), "{error}");
			assert!(error.len() < 512, "configuration errors must be bounded");
		}
	}

	#[test]
	fn over_budget_collection_returns_no_partial_snapshot() {
		let root = TempDirGuard::new();
		for name in ["a.txt", "b.txt", "c.txt"] {
			fs::write(root.path().join(name), name).unwrap();
		}
		let result = crate::scan_entries(
			&BlockingFs::native(),
			root.path(),
			scan_options(true, false, crate::WalkDetail::Minimal),
			super::ScanPolicy {
		...
		let Err(crate::WalkError::InvalidData { message, .. }) = result else {
			panic!("a scan over the entry budget must fail without returning a partial snapshot");
		};
		assert!(message.contains("FS_SCAN_LIMIT operation=collect dimension=entries"), "{message}");
	}
}
