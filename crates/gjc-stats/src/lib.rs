...
//! - `user-metrics.ts` → [`user_metrics`] (validated against vectors
//!   generated by the TS implementation)
//!
//! Deliberately skipped for now (documented deferrals):
//! - `db.ts`, history storage — `SQLite` access; lands with the storage
//!   phase to avoid a premature rusqlite/sqlx dependency decision
//! - `parser.ts`, `aggregator.ts` — depend on `@gajae-code/ai` message
//!   types; ported with the phase 2 provider layer
