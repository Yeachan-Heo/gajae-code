// Vendored from oh-my-pi (MIT) crates/pi-iso/src/apfs.rs @
// a85bd5228d9f0f619deade1db78fa49420a721e1 Local modifications: retained the
// local APFS backend and added selective tree cloning.
...
mod tests {
	...
	impl Fixture {
		fn new() -> Self {
			let nanos = SystemTime::now()
				...
				.as_nanos();
			let root =
				std::env::temp_dir().join(format!("pi-iso-clone-tree-{}-{nanos}", std::process::id()));
			fs::create_dir_all(&root).expect("create clone-tree fixture");
			Self(root)
		}
	}
	...
	fn clone_tree_skips_top_level_entry_and_preserves_symlink() {
		...
		let lower = fixture.0.join("lower");
		let merged = fixture.0.join("merged");
		...
		assert_eq!(fs::read_link(merged.join("link")).unwrap(), Path::new("file"));
		assert!(
