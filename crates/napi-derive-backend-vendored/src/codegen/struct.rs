...
use crate::{
	BindgenResult, FnKind, NapiArray, NapiClass, NapiImpl, NapiObject, NapiStruct, NapiStructKind,
	NapiStructuredEnum, NapiTransparent, TryToTokens,
...
impl NapiStruct {
	...
	fn gen_napi_value_map_impl(&self) -> TokenStream {
		match &self.kind {
			...
			NapiStructKind::Transparent(transparent) => {
				self.gen_napi_value_transparent_impl(transparent)
			},
	...
	fn gen_to_napi_value_ctor_impl_for_non_default_constructor_struct(
		...
		let (object_finalize_impl, to_napi_value_impl, javascript_class_ext_impl) = if self
			.has_lifetime
		{
	...
	fn gen_async_iterator_property(&self, class: &NapiClass, name: &Ident) -> TokenStream {
		...
		// `create_iterator` is unsafe because `ScopedGenerator<'a>` has a lifetime
		// parameter, requiring the caller to uphold lifetime invariants.
		// `create_async_iterator` uses `AsyncGenerator` whose Future must be `Send +
		// 'static`, so all data is owned and no lifetime invariants need to be upheld
		// by the caller.
		quote! {
	...
	fn gen_to_napi_value_obj_impl(&self, obj: &NapiObject) -> TokenStream {
		...
		// For optimized object creation: separate always-set fields from
				...
				syn::Member::Named(ident) => {
					...
					if is_always_set {
						...
						property_descriptors
							.push(gen_named_property_descriptor(&field_name_c_string, &value_var));
					} else {
		...
		let object_creation = if conditional_setters.is_empty() {
			...
			// Some fields are conditionally set - use batched for always-set, then add
			// conditionals
			quote! {
	...
	fn gen_default_getters_setters(&self, class: &NapiClass) -> Vec<(String, TokenStream)> {
		...
		for field in class.fields.iter() {
			...
			let getter_name =
				Ident::new(&format!("get_{}", rm_raw_prefix(&field_name)), Span::call_site());
			let setter_name =
				Ident::new(&format!("set_{}", rm_raw_prefix(&field_name)), Span::call_site());
			let accessor_descriptor_name = Ident::new(
				&format!("__napi_field_accessor_descriptor_{}", rm_raw_prefix(&field_name)),
				Span::call_site(),
			);
			...
			if field.getter {
				...
				getters_setters.push((field.js_name.clone(), quote! {
	...
	fn gen_to_napi_value_structured_enum_impl(
        ...
        let #discriminant_value_var = napi::bindgen_prelude::ToNapiValue::to_napi_value(env, #variant_name_str)?;
			...
			property_descriptors
				.push(gen_named_property_descriptor(&discriminant_c_string, &discriminant_value_var));
				...
				match &field.name {
					syn::Member::Named(ident) => {
						...
						if is_always_set {
							// This field is always set - use batched approach
							let value_var =
								Ident::new(&format!("__variant_value_{}", idx), Span::call_site());
						...
						let raw_ident =
							Ident::new(&format!("__variant_field_raw_{}", idx), Span::call_site());
						obj_field_getters.push(gen_raw_field_getter(
							&alias_ident,
							&raw_ident,
							&ty,
...
impl NapiImpl {
	fn gen_helper_mod(&self) -> BindgenResult<TokenStream> {
		...
		accessor_descriptors.sort_by_key(|(ident, ..)| ident.to_string());
