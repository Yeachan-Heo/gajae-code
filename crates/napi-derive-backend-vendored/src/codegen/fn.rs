...
use quote::ToTokens;
use syn::{Type, TypePath, TypeReference, spanned::Spanned};

use crate::{
	BindgenResult, CallbackArg, Diagnostic, FnKind, FnSelf, NapiFn, NapiFnArgKind,
	TYPEDARRAY_SLICE_TYPES, TryToTokens,
...
impl TryToTokens for NapiFn {
	fn try_to_tokens(&self, tokens: &mut TokenStream) -> BindgenResult<()> {
		...
		let ArgConversions { arg_conversions, args: arg_names, refs, mut_ref_spans, unsafe_ } =
		...
		// The JS engine can't properly track mutability in an async context, so refuse
		// to compile code that tries to use async and mutability together without
		// `unsafe` mark.
				 ...
				 impl NapiRefContainer {
					fn drop(self, env: napi::sys::napi_env) {
					  for r in self.0.into_iter() {
						 ...
						 let unref_status = unsafe {
							napi::sys::napi_reference_unref(env, r, &mut reference_count)
						 };
						 if unref_status == napi::sys::Status::napi_invalid_arg {
							// Bun 1.4 deletes async argument references when their promise
							// settles. The generated cleanup then observes an already-finalized
							// reference; no further unref/delete operation is valid or needed.
							continue;
						 }
						 assert_eq!(
							unref_status,
						 ...
						 let delete_status = unsafe { napi::sys::napi_delete_reference(env, r) };
						 assert!(
							delete_status == napi::sys::Status::napi_ok
							  || (reference_count == 0
								 && delete_status == napi::sys::Status::napi_invalid_arg),
							"failed to delete napi ref: {delete_status}"
						 );
					  }
					}
				 }
...
impl NapiFn {
	fn gen_arg_conversions(&self) -> BindgenResult<ArgConversions> {
														...
														crate::PRIMITIVE_TYPES.iter().find(|(p, _)| ident == *p)
													{
														bail_span!(
															...
															"This type must not be {} \nthis in JavaScript \
															 function must be `Object` type or `undefined`",
															primitive_type
														);
													}
													args.push(
		...
		Ok(ArgConversions { arg_conversions, args, refs, mut_ref_spans, unsafe_: self.unsafe_ })
	}

	/// Returns a type conversion, and a boolean indicating whether this value
	/// needs to have a reference created to extend the lifetime
	/// for async functions.
	fn gen_ty_arg_conversion(
		...
		match ty {
			syn::Type::Reference(syn::TypeReference { mutability: Some(_), elem, .. }) => {
				let q = quote! {
				  let #arg_name = {
					 #type_check
			...
			_ => {
				...
				if let syn::Type::Path(path) = &ty {
					...
					// For example, in `async fn foo(v: Vec<&S>) {}`, we need to handle `v` as a
					// reference.
				...
				// For the elements of the Array, we need to return rather than throw if they
				// are invalid when `return_if_invalid` is true
	...
	fn gen_fn_register(&self) -> TokenStream {
		if self.parent.is_some() || cfg!(test) {
			...
			let cb_name =
				Ident::new(&format!("_napi_rs_internal_register_{name_str}"), Span::call_site());
...
fn hidden_ty_lifetime(ty: &mut syn::Type) -> BindgenResult<()> {
	match ty {
		...
		Type::Reference(TypeReference { lifetime: Some(lt), .. }) => {
			*lt = syn::Lifetime::new("'_", Span::call_site());
		},
		_ => {},
	}
