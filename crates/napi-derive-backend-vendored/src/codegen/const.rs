...
use crate::{BindgenResult, NapiConst, TryToTokens, codegen::js_mod_to_token_stream};
...
impl NapiConst {
	fn gen_module_register(&self) -> TokenStream {
		...
		let cb_name =
			Ident::new(&format!("__register__const__{register_name}_callback__"), self.name.span());
