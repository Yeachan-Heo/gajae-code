...
#[derive(Debug)]
enum Repr {
	Single { text: String, span: Option<(Span, Span)> },
	SynError(Error),
	Multi { diagnostics: Vec<Diagnostic> },
}

impl Diagnostic {
	/// Generate a `Diagnostic` from an informational message with no Span
	pub fn error<T: Into<String>>(text: T) -> Diagnostic {
		Diagnostic { inner: Repr::Single { text: text.into(), span: None } }
	}
	...
	pub fn span_error<T: Into<String>>(span: Span, text: T) -> Diagnostic {
		Diagnostic { inner: Repr::Single { text: text.into(), span: Some((span, span)) } }
	}

	/// Generate a `Diagnostic` from the span of any tokenizable object and a
	/// message
	pub fn spanned_error<T: Into<String>>(node: &dyn ToTokens, text: T) -> Diagnostic {
		Diagnostic { inner: Repr::Single { text: text.into(), span: extract_spans(node) } }
	}

	/// Attempt to generate a `Diagnostic` from a vector of other `Diagnostic`
	/// instances. If the `Vec` is empty, returns `Ok(())`, otherwise returns
	/// the new `Diagnostic`
...
impl From<Error> for Diagnostic {
	fn from(err: Error) -> Diagnostic {
		Diagnostic { inner: Repr::SynError(err) }
	}
}
