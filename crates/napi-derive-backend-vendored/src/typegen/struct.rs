use std::{cell::RefCell, collections::HashMap, iter, vec::Vec};

use super::{ToTypeDef, TypeDef, add_alias, format_js_property_name, ty_to_ts_type};
use crate::{NapiImpl, NapiStruct, NapiStructField, NapiStructKind, typegen::JSDoc, util::to_case};
	...
	fn to_type_def(&self) -> Option<TypeDef> {
		...
		if let Some(output_type) = &self.iterator_yield_type {
			...
			let next_type = if let Some(ref ty) = self.async_iterator_next_type {
				...
				// Make TNext accept undefined so `for await...of` works (it calls next() with
				// no args)
