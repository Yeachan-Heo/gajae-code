...
use super::{ToTypeDef, TypeDef, r#struct::CLASS_STRUCTS, ty_to_ts_type};
use crate::{CallbackArg, FnKind, NapiFn, typegen::JSDoc, util::to_case};
...
impl Display for FnArgList {
	fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
		...
		for (i, arg) in self.args.iter().enumerate() {
			...
			// For setters, never mark parameter as optional (TS1051: A 'set' accessor
			// cannot have an optional parameter)
...
impl FromIterator<FnArg> for FnArgList {
	fn from_iter<T: IntoIterator<Item = FnArg>>(iter: T) -> Self {
		...
		let last_required = args
			...
			.map(|(i, _)| i);
		FnArgList { this, args, last_required, is_setter: false }
	}
}
	...
	format!(
		...
		args = &callback
			...
			.map(|(i, arg)| {
				...
				FnArg { arg: format!("arg{i}"), ts_type, is_optional }
			})
			.collect::<FnArgList>(),
			...
			self
				...
				.filter_map(|arg| match &arg.kind {
					crate::NapiFnArgKind::PatType(path) => {
											...
											))) = angle_bracketed_args.first()
											{
												if let Some(segment) = path.segments.first() {
													if *parent == segment.ident {
														// If we have a Reference<A> in an impl A block, it
														// shouldn't be an arg
														return None;
													}
												}
						...
						Some(FnArg { arg, ts_type, is_optional })
					},
					crate::NapiFnArgKind::Callback(cb) => {
						...
						Some(FnArg { arg, ts_type, is_optional: false })
					},
				})
				.collect::<FnArgList>()
