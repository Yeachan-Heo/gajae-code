...
/// Formats a JavaScript property name, adding quotes if it contains special
/// characters or starts with a digit that would make it an invalid identifier.
///
/// This function checks for characters that are known to be invalid in
/// JavaScript identifiers, rather than trying to enumerate all valid ones
/// (which would need complex Unicode identifier rules). This approach allows
/// Unicode letters and maintains backward compatibility.
pub fn format_js_property_name(js_name: &str) -> String {
	...
	// We explicitly check for invalid characters rather than trying to enumerate
	// all valid ones, which allows Unicode letters while catching common
	// problematic chars.
...
/// Mapping from `rust_type` to (`ts_type`, `is_ts_function_type_notation`,
/// `is_ts_union_type`)
static KNOWN_TYPES: LazyLock<HashMap<&'static str, (&'static str, bool, bool)>> =
	LazyLock::new(|| {
...
fn handle_threadsafe_function_type(args: &[(String, bool)]) -> Option<(String, bool)> {
	...
	if handled_tsfn {
		Some((format!("((err: Error | null, {fn_args}) => {return_ty})"), false))
	} else {
...
fn handle_generic_type(rust_ty: &str, args: &[(String, bool)]) -> Option<(String, bool)> {
	...
	if !args.is_empty() {
		...
		if let Some((alias, _)) = type_alias {
			// If alias contains '<', take the base type as &str, then convert to String for
			// formatting
		...
		arguments
			...
			.filter_map(|(index, arg)| match arg {
				syn::GenericArgument::Type(generic_ty) => {
					...
					// if Type is Function, first argument is lifetime and second is params, third
					// is return type so we need to judge is_function_with_lifetime and set
					// is_return_type if not and just keep the origin's logic
...
pub fn ty_to_ts_type(
	...
	match ty {
			...
			if tuple.elems.is_empty() {
				...
				let variadic = &tuple
					...
					.map(|(i, arg)| {
						...
						r#fn::FnArg { arg: format!("arg{i}"), ts_type, is_optional }
					})
		...
		Type::Path(syn::TypePath { qself: None, path }) => {
			handle_type_path(path, is_return_ty, is_struct_field, convert_tuple_to_variadic)
		},
...
mod tests {
	...
	fn test_escape_json_multiple_escapes() {
		assert_eq!(escape_json(r#"test\\"multiple\\""#), r#"test\\\\\"multiple\\\\\""#);
