// Vendored from can1357/oh-my-pi@a85bd5228d9f0f619deade1db78fa49420a721e1:crates/pi-builtins/src/jobs.rs — MIT (c) 2025 Mario Zechner, 2025-2026 Can Bölük, 2026 Stencil Labs, Inc. Modified for gajae-code: yes, resolves job selectors with the local JobManager API.
use std::io::Write;
...
fn resolve_job_spec<'a>(job_manager: &'a jobs::JobManager, job_spec: &str) -> Option<&'a jobs::Job> {
	if let Some(remainder) = job_spec.strip_prefix('%') {
		match remainder {
			"%" | "+" => job_manager.jobs.iter().find(|job| job.is_current()),
			"-" => job_manager.jobs.iter().find(|job| job.is_prev()),
			id if id.chars().all(char::is_numeric) => {
				let id = id.parse::<usize>().ok()?;
				job_manager.jobs.iter().find(|job| job.id == id)
			},
			_ => None,
		}
	} else {
		let pid = job_spec.parse::<i32>().ok()?;
		job_manager.jobs.iter().find(|job| job.representative_pid() == Some(pid))
	}
}