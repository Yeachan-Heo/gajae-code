...
- `gate_id`: stable identifier for this gate. It matches the paired interactive UI request `id` and is monotonic because it is generated by the same process-wide `Snowflake` id source used by RPC UI frames.
- `stage`: active workflow skill and phase when available (`<skill>:<phase>`), otherwise the UI method.
- `kind`: one of `"question"`, `"approval"`, or `"execution"`.
- `schema`: JSON Schema subset for the expected answer. Runtime validation currently enforces `type`, `enum`, `const`, array `items`, object `required`/`properties`, and `additionalProperties: false`.
- `options`: optional display choices, emitted for select-style gates.
- `context`: method/title/message/timeout metadata plus active workflow skill and phase when known.

Hosts answer with:

```json
{ "id": "resp_1", "type": "workflow_gate_response", "gate_id": "123", "answer": true }
```
