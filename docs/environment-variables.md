...
| `TYPESAFE_API_KEY`              | Jev System One decision auth                     | Explicitly selecting `task.decision.provider=jev`               | Read by the existing trusted `AuthStorage` identity `typesafe` (not by a new chat-provider resolver). Supply it from the launching shell or user-owned environment; a project `.env` is not credential authority for Jev. Not a chat provider. |
...
| `IONET_API_KEY`                 | IO Intelligence (io.net) auth                    | Using the `ionet` provider preset                              | Create at https://io.net/docs/guides/intelligence/api-keys-and-secrets                                 |
...
| `GJC_TASK_COLLECTION` | Local subagent decision/outcome collection: `metadata` excludes assignment/context text; `content` permits bounded text that may contain secrets. Normally unset is off; enabling `task.decision.enabled` permits metadata only when this variable is unset. Explicit `off` and invalid/unsupported values prevent persistence; `GJC_DISABLE_TELEMETRY` overrides all modes. Trusted sources only, not project `.env`. Collection opt-out does not disable separately authorized Kev/Jev inference. See [local collection](./models.md#local-task-decision-collection) and [Kev/Jev decisions](./models.md#kev-and-jev-subagent-decisions). |
