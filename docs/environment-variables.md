...
| `GJC_TUI_KEYBOARD_PROTOCOL` | Enhanced keyboard input (Kitty keyboard protocol + xterm modifyOtherKeys). Enabled by default; set `0` / `false` to leave the keyboard in its default mode. Use this when a terminal breaks IME/Hangul composition while these enhanced modes are active (e.g. Android Termius, Windows CJK/한글 IME, or Apple Terminal.app Korean IME crash reports — see `docs/issue-4025-terminal-ime-crash.md`). |
| `GJC_TUI_IME_CURSOR` | macOS CJK/한글 IME overlay anchor: when the soft cursor is active, the TUI shows a steady block cursor (`\x1b[2 q`) so the IME composition overlay anchors to the correct cell, and standalone cursor nudges are written outside synchronized-output framing (which flushes macOS IME composition). Enabled by default on macOS; set `0` / `false` to disable the anchor and the extra cursor writes. See `docs/issue-4025-terminal-ime-crash.md`. |
