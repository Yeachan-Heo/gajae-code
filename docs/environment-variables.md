...
| `GJC_NOTIFICATIONS_STREAM` | `1` enables live assistant-output streaming to Telegram for this process; `0` / `off` / `false` disables it even when `notifications.telegram.streaming.enabled` is enabled globally. |
