...
| Codex / Claude Code want to delegate planning/execution to GJC | [Canonical gajae-code plugin](./hermes-mcp-bridge.md) under `plugins/` via `gjc setup claude` / `gjc setup codex` | Claude Code has an installable generated local marketplace. Codex setup is preview-only/fail-closed: it emits the documented personal marketplace shape plus `mcp_servers`, but support is not claimed until a versioned local marketplace smoke proves install and runtime activation. |
