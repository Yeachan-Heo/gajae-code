...
The path must be absolute. GJC reads exactly that file, exposes only its MCP tools, and owns the server processes for that session. It does not load server prompts, resources, instructions, sampling, or other config files. Invalid or unavailable configuration fails softly with sanitized diagnostics.
