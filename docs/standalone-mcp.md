...
Session start never blocks on a slow server: GJC waits a bounded budget for the batch and then continues. That budget elapsing is not a verdict on any server.

The wait is not a fixed number. It is derived from the largest valid `timeout` declared by the servers in the batch plus a small grace, floored at a short base when none declared one and capped by an ordinary-startup ceiling; an ACP lifecycle launch supplies its own larger cap from its readiness deadline. The ceiling is what keeps one server's long declared timeout from becoming everyone's startup latency.

`gjc mcp add <name> --timeout <ms>` declares how long *that server* may take to connect. A server still inside its declared window keeps connecting in the background under its own timeout and is reported as connecting, not failed; when it lands, its tools are registered in the session that started it. A server that declared no window, or has already spent it, is disconnected and reported as a startup timeout. The one exception is a server with a cached catalog *and* no declared window, or one whose declared window is still open: it stays connecting behind its deferred tools, which is the pre-existing behavior of that cache rather than something the declared timeout grants. A cached server whose declared window has elapsed is torn down like any other. Exact-file (`--mcp-config`) startup does not use that background path, because it builds its catalog once: it waits for the batch and then reports a terminal result. That wait is bounded by each server's declared `timeout` plus a grace, defaulting to 30 s for a server that declared none, and a server still unfinished at the end is reported as a startup failure rather than left connecting.
