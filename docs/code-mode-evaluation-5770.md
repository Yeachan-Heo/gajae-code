...
**Read-path correction base:** `dev` at `84315340c24b013a781b55edb1504d83c4c5f343` (merge commit for PR #5868; this G010 amendment is layered on that tree).
...
| S20 | `[ {"id":"s","tool":"search","args":{"pattern":"fixture","paths":["fixture.txt"]}},{"id":"r","tool":"read","args":{"path":{"$ref":"s","pointer":"/polluted"}}} ]` (remove the spaces immediately after `[` and before `]`) | After live-equivalent result normalization, require `polluted: "fixture.txt"` to remain inherited (not own). A pure, test-only prototype-following lookup independent of the safe resolver must resolve it; allow exactly the harmless search, then require the own-property-only resolver to reject `/polluted` before dispatching the dependent read. If normalization removes the inherited value, or the baseline is missing, false, or shares the safe resolver, S20 is unexercised and the encoding fails the safety gate. |
