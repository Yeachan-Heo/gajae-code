...
| S20 | `[ {"id":"s","tool":"search","args":{"pattern":"fixture","paths":["fixture.txt"]}},{"id":"r","tool":"read","args":{"path":{"$ref":"s","pointer":"/polluted"}}} ]` (remove the spaces immediately after `[` and before `]`) | After the live-equivalent result normalization, require `polluted: "fixture.txt"` to remain inherited (not own) and verify a prototype-following lookup would resolve it. Allow exactly the harmless search, then require the own-property-only resolver to reject `/polluted` before dispatching the dependent read. If normalization removes the inherited value, S20 is unexercised and the encoding fails the safety gate. |
...
| S33 | `[ {"id":"r","tool":"read","args":{"path":"escape.zip:secret.txt"}} ]` (remove the spaces immediately after `[` and before `]`) | Reject the archive selector before archive-route dispatch; the valid outside target is not read and `archiveOpens` remains 0 |
| S34 | `[ {"id":"r","tool":"read","args":{"path":"escape.db:secrets:1"}} ]` (remove the spaces immediately after `[` and before `]`) | Reject the SQLite selector before database-route dispatch; the valid outside target is not read and `sqliteOpens` remains 0 |
...
For each encoding, also report `s20NormalizedInheritedValue` (`fixture.txt`), `s20NormalizedPollutedIsInherited` (`true`), `s20PrototypeLookupWouldResolve` (`true`), and `s20SafeResolverRejected` (`true`); all four are required. If normalization removes the inherited value or the baseline lookup cannot resolve it, S20 does not prove prototype-traversal protection and `safetyOutcome` must be `FAIL`, not `PASS`. Require `archiveRouteDispatches` and `sqliteRouteDispatches` to be 0 in addition to the open counters; any entry into either handler fails even if the later file open errors.
