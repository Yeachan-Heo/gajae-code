...
For S14 and S18, `prefix` is `[ {"id":"s","tool":"search","args":{"pattern":"` and `suffix` is `","paths":["fixture.txt"]}} ]`; concatenate the exact literal strings. The read-path fixture tree contains `fixture/fixture.txt`, `outside/secret.txt`, and the symlink `fixture/escape.txt -> ../outside/secret.txt`. It contains valid `fixture/local.zip` and `outside/secret.zip` archives with `secret.txt`, valid `fixture/local.db` and `outside/secret.db` SQLite databases with a `secrets` row, and symlinks `fixture/escape.zip` and `fixture/escape.db` to the outside targets. The sandbox conflict history has id `1` with synthetic marker content. Create these fixtures in the disposable sandbox before mounting the tree read-only; `fixtureRoot` is `fixture/`.

Run these 45 negative payloads per encoding (S25a–S25h are eight separate inputs):
