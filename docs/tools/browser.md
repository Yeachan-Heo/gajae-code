...
| `diagnostics` | `boolean` | No | Opt-in. Captures bounded page runtime diagnostics (page exceptions and `console.error` metadata) for the tab, reported in the next successful `run`/`act` response. Passed to `acquireTab(...)` as `runtimeDiagnostics`. |
