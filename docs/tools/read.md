...
Presentation is **opt-in and off by default**. It runs only when the `cmux.presentation` setting is `true` AND the process is inside a verified cmux session (`CMUX_WORKSPACE_ID` matches `cmux --id-format uuids identify` and `help`/`capabilities` pass). When active, a top-level public HTTP(S) URL is opened with `cmux browser open <redacted-url> --focus false`, where `<redacted-url>` is only the `origin + pathname`: query strings, fragments, and any credentialed (`user:pass@`) URL are never presented, so presigned signatures, tokens, and PII cannot reach cmux history. `:raw` still presents (redacted); raw only skips content transforms and the Insane fallback.

This is presentation only and is bounded by the read's abort signal plus a hard deadline, so a slow or never-settling presenter can never delay, block, or change the read. It has no effect on fetch permission, redirects, rendered output, errors, caching, or the non-cmux path. Absent/invalid cmux context, the setting being off, verification failure, timeout, or a later cmux command error silently leaves the read unchanged and disables presentation for that invocation.
...
- `#handleInternalUrl()` behavior:
  ...
  - passes `immutable` through to `resolveFileDisplayMode()` so anchors are suppressed for immutable resources such as artifacts and agent outputs
