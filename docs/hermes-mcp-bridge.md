...
The generated operator instructions distinguish requested input for current work from a separate queued task. `gjc_coordinator_send_prompt` accepts `steer: true` with the existing active `turn_id`; it uses a correlated SDK `turn.steer` and returns `delivery: "steer"`, `steer_client_ref`, and `consumption_verified: false`. The Coordinator requires the active turn's runtime command and turn IDs, and passes their exact `${runtime_command_id}:${runtime_turn_id}` token to the SDK host. The host fences admission against that runtime identity: a runtime-turn mismatch is rejected rather than queued for a successor. The usual mutation consent and session authority checks apply. Combining steering with queue/force is invalid; stale or non-active turns are refused. Existing `queue: true` behavior is unchanged.
...
The Coordinator checks the selected `turn_id` under its transition queue and passes the selected runtime identity for the SDK host to fence at admission. A `turn_not_active` response requires re-reading authoritative state and using a new idempotency key for a fresh attempt, even if the Coordinator turn ID has not changed; a terminal rejection is replayed under its original key. If the active turn changes, retarget the feedback to the new active turn with a new key rather than risking delivery to a successor. Admission still does not prove the model read or applied the evidence (`consumption_verified: false`); inspect the worker acknowledgement and resulting artifacts before treating evidence as consumed.

SDK steering bypasses earlier ordinary ordered controls so they cannot prevent input from reaching its worker, but it waits behind an already-pending `turn.abort_and_prompt` replacement. The expected runtime identity then refuses stale steering instead of delivering it to a successor. Already-running hosts retain their loaded dispatcher until a normal restart/resume; updating only Coordinator cannot hot-patch that code. Treat unanswered steering as uncertain, retain the same idempotency key for retries to the same turn, and do not restart unrelated work as automatic recovery.

Before replacing a live Coordinator executable, compare its package generation with the running SDK broker's discovery record. `ensureBroker` retires an incompatible broker generation; mixing release generations during an in-flight job can therefore stop existing hosts. Backport to the installed release for a live repair, or perform a coordinated upgrade after work has drained. Preserve transcripts and worktrees; do not treat `register_session` as a generic resume API, since a new endpoint must prove its sidecar authority.
