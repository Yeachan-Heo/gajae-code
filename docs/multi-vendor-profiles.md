...
| **Claude + Codex** | the built-in `opus-codex` (Claude main loop + Codex support roles), or `codex-opencodego`. |
...
- **`opencode-go` and `google-antigravity` resolve models from a live catalog fetched after `/login`.** Their runtime model lists include ids newer than any bundled snapshot — e.g. `opencode-go/glm-5.2` (current open-weight leader by AA Index) and `google-antigravity/gemini-3.5-flash`. Because each profile's `required_providers` gates activation on those credentials, the live catalog (carrying these ids) is always present by the time the profile runs. Both confirmed via `gjc -p`.
