...
- **openai-codex on a ChatGPT account** uses provider-authoritative model availability. GJC does not infer eligibility from ChatGPT account or plan labels and does not pre-reject selected models such as GPT-5.6 Sol; the Codex backend accepts or rejects the request, and its response is surfaced through the ordinary provider error path. Model access can vary by account and backend rollout, so treat live provider responses as authoritative.
