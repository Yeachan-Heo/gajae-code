...
- **openai-codex on a ChatGPT account** has entitlement-dependent model access: base GPT includes `gpt-5.5` and `gpt-5.4`. Standalone `-codex` variants (`gpt-5.3-codex`, `gpt-5.2-codex`, `gpt-5.1-codex-max` / `-mini`) return `not supported when using Codex with a ChatGPT account`. For GPT-5.6 Sol, a freshly confirmed Free candidate is excluded locally, while Plus and unknown-plan candidates are ranked and sent to the provider for the authoritative entitlement decision; confirmed Pro candidates remain preferred. Provider-confirmed entitlement failures surface actionable guidance. Spark retains its confirmed-Pro credential filter when a confirmed Pro candidate is available; this Sol policy does not remove that filter.
