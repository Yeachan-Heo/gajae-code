...
Telemetry uses exact `{ availability, value }` records with the explicit `supported`/`unavailable` taxonomy. This protocol requires supported nominal thermal state, normal memory pressure, bounded one-minute load, and bounded free memory before and after each launch. The ambient one-minute load drift limit compares `telemetryBefore.loadAverage1m` across attempts against the first attempt's pre-launch value only; an intended within-attempt workload rise in `telemetryAfter` is not ambient drift. `telemetryAfter` remains required, subject to the absolute maximum load plus every thermal, memory-pressure, power, and free-memory control, and retained in diagnostics. The free-memory fraction drift limit is unchanged: every before and after free-memory value is compared with the first attempt's pre-launch value. Missing/unavailable controls, an absolute telemetry bound violation, excessive pre-attempt ambient drift, critical state, host or power drift, overlap, cooldown under 60 seconds, documented interruption, or a surviving parent/child process invalidates the sealed input before outcomes are parsed. Invalid report structure may consume a frozen attempt and use only its preallocated replacement; the sealed schedule, caps, and slot row never change.
