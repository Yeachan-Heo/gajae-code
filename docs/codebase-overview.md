...
Gajae-Code (`gjc`) is centered on `packages/coding-agent/`. The public workflow surface is intentionally fixed at five source-bundled skills (`deep-interview`, `ralplan`, `team`, `ultragoal`, and `ultratest`) and four public role subagents. Runtime state, specs, plans, goals, team state, and local overrides live under `.gjc/`.
...
- `packages/coding-agent/src/cli.ts` is the executable bootstrap. It registers top-level CLI commands such as `setup`, `skills`, and the default launch path. Bundled workflows, including `ultratest`, are discoverable through `skills`, installable with `setup defaults`, and invoked as skills rather than standalone CLI commands.
