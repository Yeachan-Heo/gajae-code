...
- `cacheRetention`: `none`, `short`, or `long`; request-time options win over model/modelOverride values, then provider values, then `GJC_CACHE_RETENTION`, then the runtime default. The runtime default is `short` for most providers, but the Anthropic provider defaults to `long` because the ~5m cache is fragile for long-running subagent workflows. Canonical Anthropic models emit `ttl: "1h"` when long retention is supported. Non-canonical Anthropic-compatible endpoints emit no generated cache controls by default, regardless of model id; set `compat.promptCacheMode: automatic` or `explicit` to opt into generated caching, then set `compat.supportsLongCacheRetention: true` to opt into 1-hour retention. For OpenAI Responses, this controls `prompt_cache_retention` only; it does not disable `prompt_cache_key` when a stable session id exists.
...
Without an explicit mode, only canonical Anthropic endpoints default to `automatic`; non-canonical Anthropic-compatible endpoints default to `none`, regardless of model id. Set `compat.promptCacheMode: automatic` to enable top-level caching on a compatible endpoint, use `explicit` for block-level markers, or pass a non-`none` request-time `cacheRetention` as a one-request automatic opt-in. Non-canonical endpoints with generated caching use the default ~5m lifetime unless they opt into `supportsLongCacheRetention: true`.
