...
- `codex-{eco,medium,pro}` — GPT-6 Sol/Astra/Luna role mixes tuned by tier and reasoning effort; `lunamaxxing` — OpenAI Codex Luna-only profile with maximum reasoning on delegated roles
...
- `claude-opus` — Anthropic OAuth preset that prefers `claude-opus-5-5`, then `claude-opus-5`, and deterministically falls back to `claude-opus-4-6` when newer Opus models are absent from the active catalog
...
The `eco`, `medium`, and `pro` Codex profile mappings are current product judgments: Eco assigns Astra low/Luna low/Luna high/Astra xhigh/Astra high to default/executor/planner/critic/architect; Medium assigns GPT-6 Sol low/Luna low/Astra high/GPT-6 Sol xhigh/GPT-6 Sol high; Pro assigns GPT-6 Sol medium/GPT-6 Sol medium/GPT-6 Sol high/GPT-6 Sol max/GPT-6 Sol xhigh; and LunaMaxxing assigns Luna medium/Luna xhigh/Luna max/Luna max/Luna max. `opus-codex` retains the Medium Codex executor, critic, and architect roles but uses `anthropic/claude-sonnet-5` for planner; `codex-opencodego` retains the Medium Codex default and architect roles; and `fable-opus-codex` uses the Pro Codex executor and architect roles with `anthropic/claude-opus-5-5:medium` for planner.

GPT-6 Astra supports `low`, `medium`, `high`, `xhigh`, and `max`; the ASTRA profiles use only those catalog-backed levels. In default/executor/architect/planner/critic order, `astra-lite` assigns Astra medium/Luna xhigh/Astra xhigh/Astra xhigh/Sol high; `astra-default` assigns Astra medium/Luna max/Astra xhigh/Astra xhigh/Sol xhigh; and `astra-heavy` assigns Astra medium/Sol max/Astra max/Sol max/Astra max.
...
| --- | --- | --- | --- |
| Claude | `claude-sonnet-4-6` (default), `claude-sonnet-5`, `claude-opus-4-6`, `claude-opus-4-7`, `claude-opus-4-8`, `claude-opus-5`, `claude-opus-5-5`, `claude-fable-5` | `anthropic-messages` | 1M |
| GPT | `gpt-5-2025-08-07`, `gpt-5.2-2025-12-11`, `gpt-5.4`, `gpt-5.5`, `gpt-6-luna`, `gpt-6-sol`, `gpt-6-astra` | `openai-completions` | 922K |
...
Without an explicit mode, canonical Anthropic endpoints default to `automatic`, Claude-family model ids on non-canonical compatible endpoints default to `explicit`, and unknown non-Claude compatible endpoints default to `none`. Non-canonical endpoints get the default ~5m lifetime unless they opt into `supportsLongCacheRetention: true`. When `long` retention is requested (the default) against a compatible endpoint that leaves `supportsLongCacheRetention` unset, GJC omits `ttl` and logs a warning once per provider session naming the provider and model; set the flag to `true` for gateways that forward `ttl: "1h"` (idle gaps of 5-60 minutes then keep hitting the cache instead of rewriting the whole prefix), or to `false` to acknowledge the ~5m lifetime and silence the warning. Set `promptCacheMode: automatic` only when a gateway is known to pass through Anthropic's top-level cache control without adding conflicting block markers.
