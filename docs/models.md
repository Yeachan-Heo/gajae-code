...
There is deliberately no configurable Kev endpoint. A loopback port number proves nothing about which process is listening on it, so local inference is carried over the owned supervisor's authenticated Unix socket (`<root>/control.sock`) with the same per-start token `stop` uses. The supervisor binds the service port itself, before it starts anything, and passes that already-listening socket down to the service; it keeps its own copy open for as long as it manages the service. Forwarding to `127.0.0.1:<port>` therefore reaches a socket this installation bound rather than whatever won a race for the port. A port already in use makes `start` fail closed without launching a service. If no owned running service can be proven, the decision fails with `unavailable` and **no task text is sent anywhere**.
...
Installation uses an isolated Kev environment and requires the upstream-supported Python/MLX toolchain. Install records the exact upstream checkout revision and downloaded local Hugging Face snapshot used by the server; it is not a floating model alias at run time. To change `--model`, stop the owned server first, then run `gjc setup kev install --model <hub-org>/<hub-model>` explicitly; start refuses a model that was not installed. Installation does not start the server; sessions never automatically start or stop it. `start` may report `starting` until the loopback health check is ready, while `status` reports `running` only after ownership, listening, and `/v1/models` health all pass. Stop explicitly with `gjc setup kev stop`. GJC never signals a process merely occupying the port.

`start` does not launch `kev.serve` directly. It launches a GJC-written supervisor (a 0600 `supervisor.py` in the private root, run by the same isolated Python) that owns `kev.serve` as its own child and answers an authenticated Unix socket at `<root>/control.sock` (0600). A random per-start token is handed to the supervisor on stdin — never through argv or the environment — and recorded in the private `server.json`. `stop` proves ownership of the supervisor, then sends that token over the socket; the supervisor compares it in constant time and, only on a match, signals its own child handle, waits up to 10 seconds, force-kills if needed, removes the socket, and exits. **`stop` never calls `kill` on a recorded pid.** That closes the check-to-signal window a bare pid signal has: a pid can be reused between "this pid is ours" and "signal this pid", but a parent's un-reaped child handle cannot. A supervisor that is gone leaves only a stale record to retire; one that is alive but silent, or that refuses the token, leaves the server running and the ownership record in place — nothing is signaled either way. Because the supervisor does not hold the loopback port itself, `status` asks the same authenticated channel which pid does, so the listening check still proves the listener is the supervised child rather than any process answering on that port. `server.json` records are version 2; a version-1 record predates this control channel, is not ours, and is reported `foreign` rather than stopped.

The default root is `~/.gjc/agent/kev`. Pass `--root /absolute/path` to `install`, `start`, `status`, or `stop` to use another user-owned root; a successful install remembers that root for later commands that omit `--root`. A remembered root is local state, not a shared or remote deployment. The setup-managed Kev server is loopback-only (`127.0.0.1`); do not expose it as a three-Mac service or point it at another Mac. It cannot be pointed at another Mac in any case: decisions reach it only through the owned supervisor's local Unix socket.
