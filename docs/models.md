...
There is deliberately no configurable Kev endpoint. A loopback port number proves nothing about which process is listening on it, so local inference is carried over the owned supervisor's authenticated Unix socket (`<root>/control.sock`) with the same per-start token `stop` uses, and the supervisor forwards it to the one process it started. If no owned running service can be proven, the decision fails with `unavailable` and **no task text is sent anywhere**.
