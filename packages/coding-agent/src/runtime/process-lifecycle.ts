...
export function spawnOwnedProcess(cmd: string[], opts: SpawnOwnedOptions = {}): OwnedProcess {
	...
	const groupIdentityMatches = (): boolean => {
		// Non-Linux POSIX has no /proc, so identity cannot be verified via
		// start-time. Fall back to the original behavior: signal the group
		// and let the kernel handle it. Only Linux gets the hardening gate.
