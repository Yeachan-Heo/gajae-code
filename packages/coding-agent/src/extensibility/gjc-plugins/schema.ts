...
function parseHooks(value: unknown, manifestPath: string): GjcPluginHookManifestEntry[] {
	...
	return raw.map((entry, index) => {
		...
		// v1 contract: command hooks are tool_call gates only. Other events can be
		// opened up later once their (non-blocking) semantics are specified.
		if (hasCommand && event !== "tool_call") {
			throw new GjcPluginLoadError(
				"invalid_manifest",
				`Invalid GJC plugin manifest at ${manifestPath}: hooks[${index}].command is only supported for the "tool_call" event`,
			);
		}
