...
function isUserGlobalAgentsOverrideFile(session: ToolSession, filePath: string): boolean {
	if ((session.settings.getGlobal("context.userGlobalAgentsPrecedence") ?? "fallback") !== "override") return false;

	const resolvedPath = path.resolve(filePath);
	const candidates = [path.join(session.settings.getAgentDir(), "AGENTS.md")];
	if ((session.settings.getGlobal("context.homeRootAgents") ?? false) === true) {
