	...
	renderResult(
		...
		return {
			render(width: number) {
				...
				const key = new Hasher().bool(expanded).bool(resolveRenderCapability(options)()).u32(width).digest();
				if (cached?.key === key) return cached.lines;

				let text = header;
				text += renderContentPreview(fileContent, expanded, lang, uiTheme, resolveRenderCapability(options));
