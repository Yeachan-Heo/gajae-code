...
export async function tryInsaneFallback(args: {
	...
	if (args.signal?.aborted) throw new ToolAbortError();
...
/** Reduce a public URL to a consented, non-sensitive origin+path for cmux presentation.
 * Query strings, fragments, and userinfo can carry presigned signatures, tokens, or PII,
 * so credentialed URLs are never presented and query/hash are always stripped. */
export function redactUrlForPresentation(rawUrl: string): string | null {
	try {
		const url = new URL(rawUrl);
...
/** Best-effort cmux presentation bounded by the tool's signal and a hard deadline.
 * A slow or never-settling presenter can never delay or fail the primary read. */
async function presentResearchBounded(
	presenter: CmuxResearchPresenter,
	rawUrl: string,
	signal: AbortSignal | undefined,
	deadlineMs: number,
): Promise<boolean> {
	const redacted = redactUrlForPresentation(rawUrl);
	if (!redacted) return false;
	const timeout = new Promise<boolean>(resolve => {
		const timer = setTimeout(() => resolve(false), Math.max(250, deadlineMs));
		timer.unref?.();
	});
	try {
		return await Promise.race([
			Promise.resolve(presenter.presentResearch(redacted, signal)).catch(() => false),
			timeout,
		]);
	} catch {
		return false;
...
async function renderUrl(
	...
	if (presenter && settings.get("cmux.presentation") === true) {
		cmuxVerified = await presentResearchBounded(presenter, url, signal, Math.min(timeout * 1000, 2000));
	}
	...
	if (isHtml && !raw) {
		...
		if (!htmlResult.ok) {
			...
			const outcome = getInsaneFallbackOutcome({
				...
				content: rawContent,
				javascriptEmpty: isLowQualityOutput(rawContent),
			});
			const insane = outcome
				? await tryInsaneFallback({
						url,
