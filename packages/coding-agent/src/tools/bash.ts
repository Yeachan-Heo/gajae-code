...
import { expandHintSuffix, formatToolWorkingDirectory, replaceTabs, resolveRenderCapability } from "./render-utils";
						...
						if (hasSixelOutput) {
							...
							if (result.skippedCount > 0) {
								outputLines.push(
									uiTheme.fg(
										"dim",
										`… (${result.skippedCount} earlier lines, showing ${result.visualLines.length} of ${result.skippedCount + result.visualLines.length})${expandHintSuffix(uiTheme, resolveRenderCapability(options))}`,
									),
								);
							}
