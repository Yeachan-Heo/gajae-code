...
export class ModelRegistry {
	...
	async saveCustomModelProfile(name: string, definition: ModelProfileConfig): Promise<ModelProfileDefinition> {
		...
		return this.#withModelsConfigMutation(async () => {
			...
			await this.#writeCheckedModelsConfig(checkedConfig);
	...
	async #withModelsConfigMutation<T>(mutation: () => Promise<T>): Promise<T> {
	...
	async #writeCheckedModelsConfig(config: ModelsConfig): Promise<void> {
		...
		const observedPath = `${configPath}.${crypto.randomUUID()}.observed`;
		const verificationRecoveryPath = `${configPath}.${crypto.randomUUID()}.recovery`;
		...
		let exclusiveCommitInstalled = false;
		try {
			fileHandle = syncFs.openSync(tempPath, "wx", existingStat ? existingStat.mode & 0o7777 : 0o600);
			syncFs.writeFileSync(fileHandle, Bun.YAML.stringify(config, null, 2), "utf8");
			if (existingStat && process.platform !== "win32") {
				syncFs.fchownSync(fileHandle, existingStat.uid, existingStat.gid);
			}
			syncFs.fchmodSync(fileHandle, existingStat ? existingStat.mode & 0o7777 : 0o600);
			...
			if (existingStat) {
				...
				const identityChanged =
					rollbackStat.dev !== existingStat.dev ||
					rollbackStat.ino !== existingStat.ino ||
					liveStat.dev !== existingStat.dev ||
					liveStat.ino !== existingStat.ino;
				if (identityChanged || rollbackStat.nlink !== 2 || liveStat.nlink !== 2) {
					throw new Error(`Refusing to replace models config changed during mutation: ${configPath}`);
				}

				syncFs.renameSync(configPath, observedPath);
				observedCreated = true;
				const observedStat = syncFs.statSync(observedPath);
				const movedExpectedFile =
					observedStat.dev === existingStat.dev &&
					observedStat.ino === existingStat.ino &&
					syncFs.fstatSync(originalHandle).nlink === 2;
				if (!movedExpectedFile) {
					try {
						syncFs.linkSync(observedPath, configPath);
						preserveRecovery = true;
					} catch (recoveryError) {
						preserveRecovery = true;
						throw new Error(`Models config changed during commit and recovery failed: ${String(recoveryError)}`);
					}
					throw new Error(`Refusing to replace models config changed during commit: ${configPath}`);
				}

				syncFs.unlinkSync(observedPath);
				observedCreated = false;
				if (syncFs.fstatSync(originalHandle).nlink !== 1) {
					try {
						syncFs.linkSync(rollbackPath, configPath);
						preserveRecovery = true;
					} catch (recoveryError) {
						preserveRecovery = true;
						throw new Error(`Models config hard-link race recovery failed: ${String(recoveryError)}`);
					}
					throw new Error(`Refusing models config replacement because a hard link appeared during commit`);
				}
			} else {
				try {
					syncFs.lstatSync(configPath);
					throw new Error(`Refusing to replace models config created during mutation: ${configPath}`);
				} catch (error) {
			...
			if (rollbackCreated && originalHandle !== undefined && existingStat) {
				if (syncFs.fstatSync(originalHandle).nlink !== 1) {
					preserveRecovery = true;
					throw new Error(`Models config hard-link state became ambiguous during commit; recovery file preserved`);
				}
				this.#materializeModelsConfigRecovery(originalHandle, verificationRecoveryPath, existingStat);
				verificationRecoveryCreated = true;
				syncFs.unlinkSync(rollbackPath);
				rollbackCreated = false;
				let remainingLinks: number;
				try {
					remainingLinks = syncFs.fstatSync(originalHandle).nlink;
				} catch (error) {
					preserveRecovery = true;
					throw new Error(`Models config cleanup verification failed; recovery file preserved: ${String(error)}`);
				}
				if (remainingLinks !== 0) {
					preserveRecovery = true;
					throw new Error(
						`Models config hard-link state became ambiguous during cleanup; recovery file preserved`,
					);
				}
			}
			...
			if (verificationRecoveryCreated) {
				syncFs.unlinkSync(verificationRecoveryPath);
				verificationRecoveryCreated = false;
			}
			this.#fsyncModelsConfigParent(configPath);
		} catch (error) {
			const reportedError = exclusiveCommitInstalled
				? new Error(
						`Models config was committed, but verification failed; recovery evidence preserved: ${String(error)}`,
					)
				: error;
			if (exclusiveCommitInstalled && (rollbackCreated || observedCreated || verificationRecoveryCreated)) {
				preserveRecovery = true;
			}
			...
			if (!configExists && observedCreated) {
				try {
					syncFs.linkSync(observedPath, configPath);
					preserveRecovery = true;
					configExists = true;
					...
					preserveRecovery = true;
					throw new Error(
						`Models config recovery failed after ${String(reportedError)}: ${String(recoveryError)}`,
					);
				}
			}
			if (!configExists && rollbackCreated) {
				try {
					syncFs.linkSync(rollbackPath, configPath);
					preserveRecovery = true;
				} catch (recoveryError) {
					preserveRecovery = true;
					throw new Error(
						`Models config rollback failed after ${String(reportedError)}: ${String(recoveryError)}`,
					);
				}
			}
			throw reportedError;
			...
			if (!preserveRecovery) {
				for (const recoveryPath of [
					observedCreated ? observedPath : undefined,
					rollbackCreated ? rollbackPath : undefined,
					verificationRecoveryCreated ? verificationRecoveryPath : undefined,
				]) {
	...
	#materializeModelsConfigRecovery(handle: number, recoveryPath: string, existingStat: syncFs.Stats): void {
		const content = syncFs.readFileSync(handle);
