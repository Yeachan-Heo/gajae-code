...
export class ModelRegistry {
	...
	async #discoverProviderModels(
		...
		const mergeInput = await this.#discoveryManager.discover(effectiveProviderConfig, strategy, {
			...
			getEvidenceGeneration: provider => this.authStorage.getProviderEvidenceGeneration(provider.provider),
		});
		const authGeneration =
			mergeInput.authGeneration ?? this.authStorage.getProviderEvidenceGeneration(effectiveProviderConfig.provider);
	...
	async #discoverOpenAIModelsList(
		...
		const apiKey =
			discoveryApiKey ?? (await this.authStorage.getApiKey(providerConfig.provider, undefined, { baseUrl }));
		if (apiKey && apiKey !== DEFAULT_LOCAL_TOKEN && apiKey !== kNoAuth) {
			headers.Authorization = `Bearer ${apiKey}`;
		}
	...
	#hasFreshOrStaticModelEvidence(model: Model<Api>): boolean {
		...
		return (
			...
			configuredEvidence?.authGeneration === this.authStorage.getProviderEvidenceGeneration(model.provider) &&
