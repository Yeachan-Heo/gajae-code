...
import {
	CANONICAL_PROVIDER_ID_MAX_BYTES,
	CANONICAL_PROVIDER_ID_PATTERN,
...
export class ModelRegistry {
	...
	#rebuildProviderActivity(): void {
		...
		for (const overlay of [...this.#customModelOverlays, ...this.#runtimeModelOverlays]) {
			const modelIds = staticModelIds.get(overlay.provider) ?? new Set<string>();
			modelIds.add(overlay.id);
			staticModelIds.set(overlay.provider, modelIds);
		}
	...
	#isCredentiallessProviderModelActive(model: Model<Api>): boolean {
	...
	#activeConnectionKind(model: Model<Api>): ActiveProviderDescriptor["connectionKind"] | undefined {
		if (this.authStorage.hasAuth(model.provider)) return "credential";
		if (this.#keylessProviders.has(model.provider) && this.#isCredentiallessProviderModelActive(model)) {
	...
	getActiveProviders(currentModel?: Model<Api>): ActiveProviderDescriptor[] {
		try {
			...
			for (const model of available) {
				if (!isCanonicalProviderId(model.provider)) throw new ActiveProviderResolutionError();
			...
			if (currentModel) {
				if (!isCanonicalProviderId(currentModel.provider)) throw new ActiveProviderResolutionError();
	...
	registerProvider(providerName: string, config: ProviderConfigInput, sourceId?: string): void {
		if (!isCanonicalProviderId(providerName)) {
			throw new Error(
				`Provider ${providerName}: provider ID must match ${CANONICAL_PROVIDER_ID_PATTERN} and be at most ${CANONICAL_PROVIDER_ID_MAX_BYTES} UTF-8 bytes.`,
			);
		}
