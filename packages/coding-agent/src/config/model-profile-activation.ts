...
export function resolveProjectModelProfileShadow(
	...
	const profile = profiles.get(profileName) ?? getModelProfileForReference(modelRegistry, profileName);
...
export async function prepareModelProfileActivation(
	...
	const profile = profiles.get(profileName) ?? getModelProfileForReference(options.modelRegistry, profileName);
	if (!profile) {
