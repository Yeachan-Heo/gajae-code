...
/**
 * SDK prompt deadline bounds. The schema entries below and the SDK bus/host
 * lease fallbacks read these same constants, so a Settings lookup that misses
 * cannot arm a deadline that disagrees with the declared default (#5583).
 */
export const DEFAULT_SDK_PROMPT_DEADLINE_MS = 3_600_000;
export const DEFAULT_SDK_PROMPT_MAX_RUNTIME_MS = 21_600_000;

/** Resolve a `sdk.promptDeadlineMs` read, falling back to the schema default. */
...
/** Resolve a `sdk.promptMaxRuntimeMs` read, falling back to the schema default. */
