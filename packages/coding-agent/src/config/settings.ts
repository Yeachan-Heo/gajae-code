...
export class Settings implements NotificationSettingsReader {
			...
			await withAtomicYamlConfigTransaction(target, async tx => {
				...
				if (marker?.status === "complete") {
					...
					try {
						...
						try {
							...
							// an edited/rewritten backup is not evidence of what the
							// migration wrote, so refuse to roll back on it.
							...
							deletionBackupDoc = JSON.parse(await Bun.file(backup).text()) as Record<string, unknown>;
						} catch {
							this.#warnLegacyFallbackMigration(
						...
						if (sourceHash === marker.sourceSha256 && backupHash === marker.sourceSha256) {
							...
							if (this.#workflowMigrationTargetSatisfies(tx.root, marker)) {
								await this.#writeWorkflowMigrationMarkerAtomic(markerPath, {
									...
									canonicalTargetDir: await fs.promises
										.realpath(path.dirname(target))
										.catch(() => path.dirname(target)),
									canonicalTargetIdentity: await this.#statIdentity(path.dirname(target)),
							...
							if (backupHash === marker.sourceSha256 && sourceHash !== marker.sourceSha256) {
								...
								try {
									editBackupDoc = JSON.parse(await Bun.file(backup).text()) as Record<string, unknown>;
								} catch {
									this.#warnLegacyFallbackMigration(
				...
				if (
					...
					if (marker?.status === "pending" && backupExists) {
						...
						for (const key of marker.migratedKeys) {
							if (extractWorkflowSetting(tx.current, key, { flat: false }).present) {
				...
				for (const key of CONFIG_ROOT_WORKFLOW_MIGRATION_KEYS) {
					...
					if (extracted.malformedParent) {
						...
						// Clear the marker-owned target patches so the malformed
						// source is visible to strict ralplan (exit 2) instead of
						// being shadowed by the stale agent value: fail closed
						// even without a backup (a user override preserved in this
						// window is the lesser risk than silently hiding the
						// malformed explicit settings).
						if (marker?.status === "pending") {
							const staleUnsets: AtomicYamlPatch[] = [];
							const staleFlatKeys: string[] = [];
							for (const ownedKey of marker.migratedKeys) {
								if (extractWorkflowSetting(tx.current, ownedKey, { flat: false }).present) {
				...
				if ((await this.#sha256File(source)) !== sourceSha256) {
					// Nothing was patched: the pending marker's migratedKeys would
					// falsely claim ownership of these patches on the next
					// changed-pending recovery (staleMarkerKey), letting it
					// overwrite a valid user target override. Remove it so the
					// next load starts fresh. (Narrow compound edge: a hard-crashed
					// run that left stale target patches plus a source re-edit
					// inside this recovery window leaves those patches
					// indistinguishable from a user override under target-wins;
					// the source bytes are preserved in .bak.)
					await fs.promises.rm(markerPath, { force: true }).catch(() => undefined);
					this.#warnLegacyFallbackMigration(
						`Settings: config-root workflow migration aborted: ${source} changed during migration; marker cleared, source left active for the next load`,
					);
					return;
				}
				...
				await this.#writeWorkflowMigrationMarkerAtomic(markerPath, {
					...
					backupPath: backup,
					targetPath: target,
					canonicalTargetDir: await fs.promises.realpath(path.dirname(target)).catch(() => path.dirname(target)),
					canonicalTargetIdentity: await this.#statIdentity(path.dirname(target)),
					sourceSha256,
					migratedKeys,
					startedAt,
	...
	async #reconcileMigratedSource(params: {
		...
		try {
			...
			const reconcileBackupHash = await this.#sha256File(backup);
			...
			backupDoc = JSON.parse(await Bun.file(backup).text()) as Record<string, unknown>;
		} catch {
		...
		if (repairPatches.length > 0 || repairFlatKeys.length > 0) {
			await tx.applyPatchesAndRemoveTopLevelKeys(repairPatches, repairFlatKeys);
			// The repairs are COMMITTED: rewrite the pending marker with
			// repairsApplied so the resume (and the direct resolver) treat the
			// recorded repair values as committed-write evidence - not mere
			// intent, which a user override could coincidentally match.
			await this.#writeWorkflowMigrationMarkerAtomic(markerPath, {
				...marker,
				status: "pending",
				priorSourceSha256: durableBackupHash,
