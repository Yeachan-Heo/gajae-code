...
export async function loadProjectContextFilesResult(
	...
	const includeHomeRootAgents = options.includeHomeRootAgents === true;
	const homeRootAgentsPath = path.resolve(os.homedir(), "AGENTS.md");
	...
	const [result, homeRootAgents] = await Promise.all([
		...
		loadHomeRootAgentsFile(os.homedir(), includeHomeRootAgents),
	]);
	...
	const projectFiles = items
		.filter(
			item => item.level === "project" && (includeHomeRootAgents || path.resolve(item.path) !== homeRootAgentsPath),
		)
		.map(item => ({
			path: item.path,
