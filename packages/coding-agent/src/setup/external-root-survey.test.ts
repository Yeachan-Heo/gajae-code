...
function buildSurveyFixture(): string {
	...
	writeText(path.join(root, ".gemini", "settings.json"), JSON.stringify({ mcpServers: { demo: { command: hookCommand } } }));
	writeText(path.join(root, ".gemini", "GEMINI.md"), "# Gemini\nsecret sk-secret1234567890\n");
	writeText(path.join(root, ".gemini", "projects.json"), JSON.stringify({ projects: { [root]: { token: "Bearer rawtoken" } } }));
	writeText(path.join(root, ".gemini", "extensions", "demo", "gemini-extension.json"), JSON.stringify({ name: "demo" }));
	writeText(path.join(root, ".hermes", "config.yaml"), `mcp_servers:\n  demo:\n    command: ${JSON.stringify(hookCommand)}\n`);
	...
	writeText(path.join(root, ".hermes", "skills", "demo-skill", "SKILL.md"), "---\nname: hermes\ndescription: Hermes skill\n---\n# Hermes\n");
...
function assertNoSensitiveMarkers(value: unknown): void {
	...
	for (const marker of ["sk-secret1234567890", "Bearer rawtoken", "raw-ssh-secret", "rm -rf /tmp/not-real", "frontmatter-secret", "ghp_1234567890abcdefABCDEF"]) {
...
describe("external-root-survey", () => {
	test("inventory is shallow and does not read content", async () => {
		...
		expect(result.observations.some(observation => observation.kind === "home_named_anchor" && observation.path === "~/.codex")).toBe(true);
	...
	test("deep tier rejects symlink escapes before stat/list/read/open", async () => {
		...
		writeText(path.join(outside, "escape.md"), "---\nname: outside-md\ndescription: outside markdown secret\n---\n# Outside\n");
		writeText(path.join(outside, "private-skill", "SKILL.md"), "---\nname: outside-dir\ndescription: outside private description\n---\n# Outside\n");
		...
		expect(result.counters.content_read_attempts ?? 0).toBe(0);
		expect(result.skipped.some(skip => skip.reason === "outside_root_symlink" || skip.reason === "outside_root")).toBe(true);
	...
	test("depth and hard-cap validation reject unsafe survey requests", async () => {
		const home = buildSurveyFixture();
		await expect(runExternalRootSurvey({ home, depth: "deep", anchor: ["home"] })).rejects.toThrow("--depth deep requires");
		await expect(
	...
	test("CLI surface executes against synthetic fixtures", () => {
		...
		expect(result.observations.some((observation: { kind: string }) => observation.kind === "config_json")).toBe(true);
		expect(result.observations.some((observation: { kind: string }) => observation.kind === "config_toml")).toBe(true);
		assertNoSensitiveMarkers(result);
		const reportProc = spawnSync(process.execPath, [
			"run",
			"src/cli.ts",
			"setup",
			"external-root-survey",
