...
const SECRET_KEY_PATTERN =
	/(api[_-]?key|secret|token|password|passwd|credential|private|bearer|authorization|auth|cookie|session|oauth)/i;
const SECRET_KEY_VALUE_PATTERN =
	/\b(api[_-]?key|secret|token|password|passwd|credential|private[_-]?key|bearer|authorization|auth|cookie|session|oauth)\b\s*[:=]\s*['"]?[^'"\s,;}]+/gi;
const SECRET_VALUE_REDACTION_PATTERN =
	/(sk-[A-Za-z0-9_-]{12,}|ya29\.[A-Za-z0-9_-]+|xox[baprs]-[A-Za-z0-9-]+|Bearer\s+\S+|AKIA[0-9A-Z]{16}|gh[pousr]_[A-Za-z0-9_]{16,}|github_pat_[A-Za-z0-9_]{20,}|glpat-[A-Za-z0-9_-]{16,}|hf_[A-Za-z0-9]{16,}|AIza[0-9A-Za-z_-]{20,})/g;
const SECRET_VALUE_TEST_PATTERN =
	/(sk-[A-Za-z0-9_-]{12,}|ya29\.[A-Za-z0-9_-]+|xox[baprs]-[A-Za-z0-9-]+|Bearer\s+\S+|AKIA[0-9A-Z]{16}|gh[pousr]_[A-Za-z0-9_]{16,}|github_pat_[A-Za-z0-9_]{20,}|glpat-[A-Za-z0-9_-]{16,}|hf_[A-Za-z0-9]{16,}|AIza[0-9A-Za-z_-]{20,})/;
...
export function formatExternalRootSurveyCatalogReport(result: ExternalRootSurveyResult): string {
	...
	if (result.catalog.exclusions.length === 0) {
		...
		for (const exclusion of result.catalog.exclusions.slice(0, 40)) {
			lines.push(
				`- [${exclusion.source_family}] ${exclusion.path}: ${exclusion.excluded_kind} (${exclusion.reason})`,
			);
		}
		if (result.catalog.exclusions.length > 40)
			lines.push(`- ... ${result.catalog.exclusions.length - 40} more exclusions`);
	}
...
async function observeProviderInventory(
	...
	for (const relativePath of definition.inventoryFiles) {
		await observeExactPath(
			home,
			anchor,
			anchorPath(home, anchor, relativePath),
			"allowlisted_file",
			result,
			deadline,
		);
...
async function observeExactPath(
	...
	const bounded = await resolveBoundedExistingPath(home, anchor, fullPath, result);
	if (!bounded) return;
	const stat = bounded.stat;
	const data = { ...statSummary(stat), symlink: bounded.isSymlink };
	if (stat.isDirectory()) {
		try {
			const counts = await shallowDirectoryCounts(home, anchor, fullPath, result.caps.maxEntries, result, deadline);
			Object.assign(data, counts);
		} catch (error) {
			addWarning(result, {
				anchor,
				path: displayPath(home, fullPath),
				reason: `directory_count_failed:${sanitizeError(error)}`,
			});
		}
	}
...
async function observeSqlite(
	...
	try {
		...
		db = new Database(bounded.realPath, { readonly: true, strict: true });
		...
		for (const table of tableRows) {
			...
			if (stat.size <= result.caps.maxBytes) {
				try {
					const row = db
						.prepare<{ count: number }, []>(`SELECT count(*) AS count FROM ${quoteSqliteIdentifier(table.name)}`)
						.get();
					rowCount = row?.count;
				} catch {
		...
		addWarning(result, {
			anchor,
			path: displayPath(home, fullPath),
			reason: `sqlite_read_failed:${sanitizeError(error)}`,
		});
	} finally {
		db?.close();
	}
...
async function observeLogSummaries(
	...
	const boundedDir = await resolveBoundedExistingPath(home, anchor, dir, result);
	if (!boundedDir) return;
	if (!boundedDir.stat.isDirectory()) {
		addSkip(result, { anchor, path: displayPath(home, dir), reason: "not_directory" });
		return;
	}
	const entries = await fs.readdir(boundedDir.realPath, { withFileTypes: true });
	const summaries = [];
	for (const entry of entries.slice(0, result.caps.maxEntries)) {
		ensureWithinDeadline(deadline);
		if (!(entry.isFile() || entry.isSymbolicLink()) || !LOG_FILE_PATTERN.test(entry.name)) continue;
		const fullPath = path.join(dir, entry.name);
		increment(result, "stat_calls");
		const boundedFile = await resolveBoundedExistingPath(home, anchor, fullPath, result);
		if (!boundedFile?.stat.isFile()) continue;
		const stat = boundedFile.stat;
		const content = await readCappedText(
			home,
			anchor,
			fullPath,
			result,
			stat.size,
			Math.min(result.caps.maxBytes, 256 * 1024),
		);
		if (!content) continue;
		summaries.push({
	...
	addObservation(result, {
		...
		data: {
			files: summaries,
			truncated: entries.length > result.caps.maxEntries || summaries.length >= summaryLimit(result.caps),
		},
	});
}
...
async function observeSkillInventory(
	...
	for (const entry of entries.slice(0, result.caps.maxEntries)) {
		...
		increment(result, "stat_calls");
		const boundedFile = await resolveBoundedExistingPath(home, anchor, candidate, result);
		if (!boundedFile?.stat.isFile()) continue;
		const stat = boundedFile.stat;
...
function classifyExcludedMaterial(pathValue: string, kind: string): string | undefined {
	...
	if (lowerPath.includes("history") || lowerPath.endsWith(".jsonl") || lowerPath.endsWith(".ndjson"))
		return "raw_session_history";
	if (lowerKind.includes("sqlite") || /\.(sqlite|sqlite3|db|db3)(?:$|-wal$|-shm$)/i.test(lowerPath))
		return "sqlite_rows";
	if (lowerKind.includes("log") || lowerPath.includes("/log") || /\.(log|out|err)$/i.test(lowerPath))
		return "raw_logs";
	if (
		lowerPath.includes("claude.md") ||
		lowerPath.includes("agents.md") ||
		lowerPath.includes("gemini.md") ||
		lowerPath.includes("soul.md")
	) {
...
function summarizeAbsorptionCatalog(
	...
	return {
		...
		assets_absorbed: candidates.filter(
			candidate => candidate.treatment === "absorb" || candidate.treatment === "role_guidance",
		).length,
		assets_wrapped: candidates.filter(
			candidate => candidate.treatment === "wrap_catalog" || candidate.treatment === "optional_template",
		).length,
...
function resolveAnchors(
	depth: ExternalRootSurveyDepth,
	anchors: readonly ExternalRootSurveyAnchor[],
): ExternalRootSurveyAnchor[] {
	if (depth === "inventory") return [...anchors];
	const providers = anchors.filter((anchor): anchor is ProviderAnchor => anchor !== "home");
	if (depth === "config")
		return anchors.includes("home") ? dedupeAnchors([...providers, ...PROVIDER_ANCHORS]) : providers;
	if (providers.length === 0)
		throw new Error("--depth deep requires an explicit provider anchor; --anchor home is not allowed by itself.");
	return providers;
}
...
function parseCaps(flags: ExternalRootSurveyFlags, depth: ExternalRootSurveyDepth): ExternalRootSurveyCaps {
	...
	if (caps.sampleLimit > HARD_CAPS.sampleLimit)
		throw new Error(`--sample-limit must be between 0 and ${HARD_CAPS.sampleLimit}.`);
	return caps;
}
...
async function shallowDirectoryCounts(
	...
	increment(result, "list_calls");
	const boundedDir = await resolveBoundedExistingPath(home, anchor, dir, result);
	if (!boundedDir?.stat.isDirectory()) return {};
	const entries = await fs.readdir(boundedDir.realPath, { withFileTypes: true });
	let directories = 0;
	let files = 0;
	let other = 0;
...
function summarizeConfig(content: string, kind: ConfigKind, limit: number): Record<string, unknown> {
	...
	try {
		...
		return {
			parse_ok: false,
			error: `parse_failed:${error instanceof Error ? error.constructor.name : "UnknownError"}`,
		};
	}
}
...
function summarizeValue(value: unknown, limit: number): unknown {
	if (Array.isArray(value)) {
		return {
			type: "array",
			length: value.length,
			items: value.slice(0, limit).map(item => summarizeValue(item, limit)),
...
function summarizeField(key: string, value: unknown, limit: number): unknown {
	...
	if (/command|cmd|script|hook|args?/i.test(key) && typeof value === "string") {
		return {
			type: "string",
			command_like: true,
			sha256_16: hashText(value).slice(0, 16),
			category: categorizeCommand(value),
			length: value.length,
		};
	}
...
function extractKnownConfigSummary(value: unknown, limit: number): Record<string, unknown> {
	...
	for (const key of ["mcpServers", "mcp_servers", "plugins", "hooks", "projects", "profiles"]) {
		...
		if (nested && typeof nested === "object" && !Array.isArray(nested)) {
			...
			summary[safeIdentifier(key)] = {
				count: names.length,
				names: names.slice(0, limit).map(safeIdentifier),
...
function skillCandidatePath(dir: string, entry: Dirent): string | undefined {
	if (entry.isDirectory() || entry.isSymbolicLink()) {
		if (MARKDOWN_FILE_PATTERN.test(entry.name)) return path.join(dir, entry.name);
		return path.join(dir, entry.name, "SKILL.md");
	}
...
async function readCappedText(
	...
	const bounded = await resolveBoundedExistingPath(home, anchor, filePath, result);
	if (!bounded) return undefined;
	increment(result, "content_read_attempts");
	if (size > maxBytes) increment(result, "over_cap_reads");
	const bytes = await Bun.file(bounded.realPath).slice(0, maxBytes).arrayBuffer();
	const text = new TextDecoder("utf-8", { fatal: false }).decode(bytes);
	return { text, bytesRead: bytes.byteLength, truncated: size > maxBytes };
}

async function resolveBoundedExistingPath(
	home: string,
	anchor: ExternalRootSurveyAnchor,
	filePath: string,
	result: ExternalRootSurveyResult,
): Promise<BoundedPath | undefined> {
	const display = displayPath(home, filePath);
	let linkStat: Stats;
	try {
		linkStat = await fs.lstat(filePath);
	} catch (error) {
		if (isUnavailable(error)) {
			addSkip(result, { anchor, path: display, reason: "missing" });
			return undefined;
		}
		throw error;
	}
	...
	if (isSymlink) increment(result, "symlink_paths");

	let realPath: string;
	try {
		realPath = await fs.realpath(filePath);
	} catch (error) {
		addSkip(result, { anchor, path: display, reason: `realpath_failed:${sanitizeError(error)}` });
		return undefined;
	}

	const allowedRoot = await resolvedAllowedRoot(home, anchor, filePath);
	if (!allowedRoot || !isPathInsideOrEqual(realPath, allowedRoot)) {
		addSkip(result, {
			anchor,
			path: display,
			reason: isSymlink ? "outside_root_symlink" : "outside_root",
		});
		return undefined;
	}
	...
	return { realPath, stat, isSymlink };
}

async function resolvedAllowedRoot(
	home: string,
	anchor: ExternalRootSurveyAnchor,
	filePath: string,
): Promise<string | undefined> {
	const homeReal = await fs.realpath(home);
	if (anchor === "home") return homeReal;
	const providerRoot = path.resolve(rootFor(home, anchor));
	const providerRootReal = await safeRealpath(providerRoot);
	if (!providerRootReal || !isPathInsideOrEqual(providerRootReal, homeReal)) return undefined;
	const lexicalPath = path.resolve(filePath);
	if (isPathInsideOrEqual(lexicalPath, providerRoot)) return providerRootReal;
	return homeReal;
}
...
function displayPath(home: string, filePath: string): string {
	...
	if (relative && !relative.startsWith("..") && !path.isAbsolute(relative))
		return path.posix.join("~", relative.split(path.sep).join("/"));
	if (!relative) return "~";
	return filePath;
}
...
function safeIdentifier(value: string): string {
	...
	if (SECRET_KEY_PATTERN.test(trimmed) || SUSPICIOUS_IDENTIFIER_PATTERN.test(trimmed))
		return `redacted:${hashText(trimmed).slice(0, 12)}`;
