...
<instruction>
- `name` is the skill name as it appears in `/skill:<name>` (e.g. `ralplan`, `ultragoal`, `ultratest`, `team`, `deep-interview`)
...
- The chain is refused while a native workflow caller is still active. If `deep-interview`, `ralplan`, `ultragoal`, or `team` has not reached a terminal phase, prepare it with `gjc state <skill> write --input '{"current_phase":"handoff"}' --json`. `ultratest` is terminal-only: complete it with `gjc state ultratest write --input '{"current_phase":"complete"}' --json` before chaining. Runtime project/user skills do not use `gjc state <skill>`.
