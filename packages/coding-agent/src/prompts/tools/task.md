...
{{#if contextEnabled}}
- `.inheritContext` (optional): `true` requests a sanitized, bounded forked snapshot of the parent conversation for this task. Works only when the global `task.forkContext.enabled` setting is true and the target agent declares `forkContext: allowed`; otherwise the call is rejected. Bundled agents that support it: `executor`, `architect`. Use it when the subagent's value depends on what the parent has already established (architect reviewing code the parent has been discussing; executor continuing a mid-investigation handoff). Skip it for independent work — passing context the child will not use wastes tokens. Cost: when parent and child run on the same provider+model, the seed reuses the parent's prefix-cache and the marginal cost is near-zero; different provider or model means no cache hit, treat the seed as full re-billing.
{{/if}}
