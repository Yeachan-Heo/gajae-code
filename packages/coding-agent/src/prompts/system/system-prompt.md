...
- An explicit user request to use a worktree (for example, "use worktree") overrides direct editing: delegate implementation through `task` with `worktree: true`, or `worktree: "<branch-name>"` when the user named a branch. That is the in-session counterpart of launching `gjc --worktree` and produces the same persistent git worktree; it is a single-task call, is mutually exclusive with `isolated`, and works regardless of `task.isolation.mode`. Plain `isolated: true` is disposable PAL isolation, not a real worktree, so do not substitute it for a worktree request. If the worktree cannot be provisioned, report the returned conflict instead of editing in the parent session.
