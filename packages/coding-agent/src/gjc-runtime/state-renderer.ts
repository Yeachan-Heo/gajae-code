...
export function renderTeamStatusMarkdown(snapshot: {
	...
	worker_lifecycle_by_id?: Record<string, { lifecycle_state?: string; worker_status_state?: string }>;
	...
	const workers = snapshot.workers
		.map(worker => {
			...
			if (!lifecycle) return `${worker.id}:${worker.status}`;
			return `${worker.id}:runtime=${worker.status},lifecycle=${lifecycle.lifecycle_state ?? "unknown"},reported=${lifecycle.worker_status_state ?? "unknown"}`;
		})
		.join(" ");
	const lines = [
		"# team status",
		...
		`- workers: ${snapshot.workers.length} (${workers})`,
	];
	if (snapshot.notification_summary) {
		const byState = Object.entries(snapshot.notification_summary.by_state)
			.map(([state, count]) => `${state}=${count}`)
			.join(" ");
		lines.push(
			`- notifications: total=${snapshot.notification_summary.total} replay_eligible=${snapshot.notification_summary.replay_eligible}${byState ? ` ${byState}` : ""}`,
		);
	}
		...
		lines.push(
			`- integrations: ${integrations
				.map(([worker, state]) => {
					...
					return `${worker}:${state.status ?? "unknown"}${files}`;
				})
				.join(" ")}`,
		);
	else if (snapshot.phase === "awaiting_integration") lines.push("- integrations: no attempts recorded");
	return `${lines.join("\n")}\n`;
}
