...
function ensureLaunchWorktreeSync(
	...
	if (!isReplaceableWorktreeTarget(plan.worktreePath)) throw launchGuard("worktree_path_conflict", plan.worktreePath);
	...
	try {
		...
		if (!isReplaceableWorktreeTarget(plan.worktreePath)) {
			throw launchGuard("worktree_path_conflict", plan.worktreePath);
		}
