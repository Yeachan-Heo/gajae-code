...
async function classifyTranscript(input: {
	...
	const { transcript, now, settings } = input;
	...
	let status: "within_thresholds" | "over_threshold" | "ambiguous" = "within_thresholds";
...
export const sessionInventoryGcAdapter: GcStoreAdapter = {
	...
	async collect(ctx: GcContext): Promise<GcCollectResult> {
		...
		// Default-OFF must be off at the SCAN boundary, not just at classification.
		// Enumerating scopes and recursively sizing artifact trees is real filesystem
		// work (and a real cost on a large store), so a disabled inventory does none
		// of it and reports nothing.
		if (settingsFailed || !settings.enabled) return { records, errors };
		let scopes: string[];
		try {
