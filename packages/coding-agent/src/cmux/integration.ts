...
export interface CmuxCommandRunner {
	run(command: string, args: readonly string[], timeoutMs: number): Promise<CmuxCommandResult>;
}

export interface CmuxResearchPresenter {
	presentResearch(research: { kind: "url"; url: string } | { kind: "query"; query: string }): Promise<boolean>;
}
...
export class CmuxPresentationAdapter implements CmuxResearchPresenter {
	...
	async presentResearch(research: { kind: "url"; url: string } | { kind: "query"; query: string }): Promise<boolean> {
		if (!(await this.#available())) return false;
		const target =
			research.kind === "url"
				? research.url
				: `https://www.google.com/search?q=${encodeURIComponent(research.query)}`;
		const result = await this.#invoke("browser", ["browser", "open", target, "--focus", "false"]);
		return result !== null;
	}
		...
		for (let attempt = 0; attempt < CAPABILITY_RETRIES; attempt++) {
			try {
				const [help, capabilities, identify] = await Promise.all([
					this.#runner.run(this.#command, ["help"], this.#timeoutMs),
					this.#runner.run(this.#command, ["capabilities"], this.#timeoutMs),
					this.#runner.run(this.#command, ["--id-format", "uuids", "identify"], this.#timeoutMs),
				]);
				if (
					help.exitCode === 0 &&
	...
	async #invoke(operation: string, args: string[]): Promise<CmuxCommandResult | null> {
		if (this.#capability !== "verified" || this.#disposed) return null;
		try {
			const result = await this.#runner.run(this.#command, args, this.#timeoutMs);
...
export class CmuxProjectionSubscription {
	...
	#onLifecycle(payload: unknown): void {
		...
		const text = terminal
			...
			: sanitizeCmuxDisplayText(
					typeof lifecycle.description === "string"
						? lifecycle.description
						: typeof lifecycle.agent === "string"
							? lifecycle.agent
							: "",
					DISPLAY_LIMIT,
				);
	...
	#progressText(progress: ProgressLike): string {
		const output = Array.isArray(progress.recentOutput)
			? progress.recentOutput
					...
					.join("\n")
			: "";
		const currentTool = typeof progress.currentTool === "string" ? `Tool: ${progress.currentTool}` : "";
		const detail =
			output ||
			(typeof progress.description === "string"
				...
				: typeof progress.task === "string"
					...
					: typeof progress.agent === "string"
						? progress.agent
						: "");
		return sanitizeCmuxDisplayText([currentTool, detail].filter(Boolean).join("\n"), DISPLAY_LIMIT);
	}
}
