...
interface CoordinatorServices {
	...
	forceCloseSession?: (sessionName: string, env: NodeJS.ProcessEnv) => Promise<unknown>;
...
export function createCoordinatorMcpServer(options: CoordinatorMcpServerOptions = {}) {
	...
	const forceCloseSession =
		...
		((name: string, targetEnv: NodeJS.ProcessEnv) => forceCloseGjcTmuxSession(name, targetEnv));
	...
	async function reapSession(
		...
		return await withSessionMutation(`${namespaceDir}::${id}`, async () => {
			...
			if (tmuxSession) {
				try {
					await forceCloseSession(tmuxSession, env);
				} catch (err) {
					return {
						ok: false,
			...
			await appendCoordinatorEvent(namespaceDir, {
				...
				metadata: { reason: opts.reason ?? null, force: opts.force === true },
			});
