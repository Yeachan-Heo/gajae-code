...
export class TelegramNotificationDaemon {
			...
			const rec = await this.topics.getOrCreateTopic(
				...
				async topicId => {
					try {
						...
						// A cancelled create is never published locally; remote cleanup is best-effort.
					}
				},
			);
			await this.persistTopics();
	...
	private async deleteTopic(sessionId: string): Promise<void> {
		...
		const record = this.topics.get(sessionId);
		if (record) this.topics.delete(sessionId);
		...
		if (record) {
			try {
				// The captured record is already scoped to this configured chatId, so
				// teardown can attempt deletion without another fallible getChat probe.
				const response = await this.botApi.call("deleteForumTopic", {
					chat_id: this.opts.chatId,
					message_thread_id: Number(record.topicId),
				});
				deleted = topicDeleteApplied(response);
			} catch {
				// Remote deletion is best-effort; retain retry state below.
			}
		}

		if (!record) return;
		if (!deleted) this.topics.restoreIfAbsent(sessionId, record);
		try {
			// Serialize the registry as it exists now. A replacement published while
			// cleanup awaited wins over failed-delete retry state for the stale record.
			await this.persistTopics();
		} catch {
			// Best-effort: in-memory state still retains any required deletion retry.
		}
	}
