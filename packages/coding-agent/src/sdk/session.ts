	...
	try {
		...
		const contextFilesResultPromise = options.contextFiles
			...
			: logger.time("discoverContextFiles", loadContextFilesResultInternal, {
					cwd,
					userGlobalAgentsPrecedence: settings.getGlobal("context.userGlobalAgentsPrecedence") ?? "fallback",
					includeHomeRootAgents: settings.getGlobal("context.homeRootAgents") ?? false,
				});
