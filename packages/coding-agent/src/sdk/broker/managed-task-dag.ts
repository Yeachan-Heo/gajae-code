...
export async function loadManagedEnrollmentRecord(agentDir: string): Promise<ManagedEnrollmentRecord> {
	...
	if (process.platform !== "linux" && (await isAbsentBeneathRealDirectories(agent, target))) return emptyRecord;
	try {
		...
		if (!gjcStat.isDirectory() || gjcStat.isSymbolicLink()) throw new Error("corrupt managed enrollment index");
