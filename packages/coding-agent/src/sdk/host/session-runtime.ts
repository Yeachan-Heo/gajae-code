...
export interface SdkOnlyTerminalAbortSeams {
	...
	abortPromptAndWaitWithTerminal: (
		handle: string,
		options: { graceMs: number; terminal?: { scope: "turn" | "owned"; expectedEpoch?: number; steeringSnapshotToken?: number } },
	...
	const terminalAbort = async (
		...
		try {
			proof = await terminalAbortSeams.abortPromptAndWaitWithTerminal(handle, {
				graceMs: 10_000,
				terminal: { scope, expectedEpoch: epoch, ...(steeringSnapshotToken !== undefined ? { steeringSnapshotToken } : {}) },
			});
		} catch {
