		...
		this.showSelector(done => {
			...
			modelSelector = new ModelSelectorComponent(
				...
				async selection => {
					...
					let previousSessionModel: Model | undefined;
					let previousSessionThinkingLevel: ThinkingLevel | undefined;
					let previousSessionDefaultModelSelector: string | undefined;
					...
					try {
						...
						previousSessionModel = this.ctx.session.model;
						previousSessionThinkingLevel = this.ctx.session.thinkingLevel;
						previousSessionDefaultModelSelector = this.ctx.session.getSessionDefaultModelSelector();
						...
						if (sessionModelChanged) {
							await this.ctx.session.restoreModelRuntime(
								previousSessionModel,
								previousSessionThinkingLevel,
								previousSessionDefaultModelSelector,
							);
							sessionModelChanged = false;
						}
						modelSelector.refreshRoleAssignments({
