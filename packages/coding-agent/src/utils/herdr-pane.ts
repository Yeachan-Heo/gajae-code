...
function watchSocketReplacement(socketPath: string, onReplaced: () => void): () => void {
	/**
	 * The inode number alone is not the file's identity. Linux hands the just
	 * freed inode straight back to the socket that replaces it — measured
	 * identical across unlink/bind on ext4 and overlayfs — so the change time is
	 * what separates the new socket from the old one there. macOS does not reuse
	 * the number, and the pair is correct on both.
	 */
