...
6. The executor QA/red-team lane must prove evidence by the real surface under test:
   ...
   - CLI surfaces require either a safe runtime argv replay (`schemaVersion: 1`, `kind: "cli-replay"`, `replaySafe: true`) or an audited `replayExempt` fallback to a structured `bun-test-report`. Runtime replay is limited to the pinned Bun runtime for `bun --version` or literal `bun -e "console.log(...)"`; the gate never executes model-authored test files. Focused `bun test` evidence is recorded in a JSON report with explicit test argv, zero-failure counts, exit code, and stdout digest. Shells, interpreters with code strings, path-qualified executables, package/git/network mutation commands, and arbitrary argv are rejected. The complete schemas and audit contract are specified below.
...
For safe CLI replay artifacts, the JSON at `path` must be an object like `{"schemaVersion":1,"kind":"cli-replay","replaySafe":true,"command":["bun","-e","console.log(\"ultragoal-cli-ok\")"],"cwd":".","env":{"LC_ALL":"C"},"timeoutMs":30000,"expectedExitCode":0,"recordedStdout":"ultragoal-cli-ok\n","recordedStderr":"","invariants":[{"type":"substring","value":"ultragoal-cli-ok"},{"type":"not_substring","value":"error"}]}`. `replaySafe: true` is required but is never authority by itself: executable replay is limited to the pinned Bun runtime for `bun --version` or deterministic literal `bun -e "console.log(...)"`. Shells, nested interpreters, path-qualified executables, test source, install/publish commands, git mutation, network clients, and every other argv are rejected. Replay cwd and artifact files are realpath-confined beneath the repository, mixed inline/nested/file-backed rows fail closed, POSIX timeout cleanup signals the replay process group, stdout and stderr are validated after normalization, output is capped at 1 MiB, and the child environment is scrubbed to `CI`, `NO_COLOR`, `GJC_ULTRAGOAL_REPLAY`, plus optional `LANG`, `LC_ALL`, `LC_CTYPE`, and `TZ`.

Focused `bun test` results are evidence-only and are never executed by the quality gate. Use audited `replayExempt` metadata with `reasonCode: "unsafe_side_effect"`, a substantive `reason`, `approvedBy`, and `fallbackArtifactRefs` pointing to a repository-contained artifact whose outer and JSON `kind` are `bun-test-report`. The report schema is `{"schemaVersion":1,"kind":"bun-test-report","command":["bun","test","packages/coding-agent/test/example.test.ts"],"total":1,"passed":1,"failed":0,"skipped":0,"exitCode":0,"stdoutPath":"artifacts/bun-test-output.txt","stdoutSha256":"<64 lowercase hex>"}`. The command must describe focused `bun test` argv with explicit existing repository-contained non-symlink `*.test.*`/`*.spec.*` files; counts must be internally consistent, non-empty, and zero-failure; `stdoutSha256` must match the realpath-confined captured output bytes. Other non-replayable commands may use the existing audited exemption contract only with a structurally valid same-surface fallback. Allowed `reasonCode` values remain `unsafe_side_effect`, `requires_credentials`, `requires_network`, `non_deterministic_external`, `destructive`, `interactive_only`, and `platform_unavailable`.
