...
6. The executor QA/red-team lane must prove evidence by the real surface under test:
   ...
   - CLI surfaces require runtime argv replay: `schemaVersion: 1`, `kind: "cli-replay"`, `replaySafe: true`, and a command from the deterministic replay contract. The contract permits `bun --version`, literal `bun -e "console.log(...)"`, and focused `bun test` with explicit repository-contained non-symlink `*.test.*`/`*.spec.*` files; shells, interpreters, path-qualified executables, package publishing/install, network clients, and arbitrary commands are rejected. The complete field-by-field replay schema, safe `env` constraints, and `replayExempt` audit contract are specified once in the "For CLI replay artifacts" paragraph below the quality-gate JSON; follow it exactly.
...
For CLI replay artifacts, the JSON at `path` must be an object like `{"schemaVersion":1,"kind":"cli-replay","replaySafe":true,"command":["bun","test","packages/coding-agent/test/example.test.ts"],"cwd":".","env":{"LC_ALL":"C"},"timeoutMs":30000,"expectedExitCode":0,"recordedStdout":"","recordedStderr":"","invariants":[{"type":"substring","value":"1 pass"},{"type":"not-substring","value":"error"}]}`. Accepted replay fields are `command`, optional `cwd`, safe `env`, `timeoutMs`, `expectedExitCode`, `recordedStdout`, `recordedStderr`, `normalization`, and `invariants`. `replaySafe: true` is required but never self-authorizes an arbitrary executable: commands are limited to `bun --version`, deterministic literal `bun -e "console.log(...)"`, or `bun test` with one or more explicit repository-contained non-symlink `*.test.*`/`*.spec.*` files; focused tests may additionally use `--bail` and `--test-name-pattern`. Shells, nested interpreters, path-qualified executables, globs, parent paths, install/publish commands, network clients, and all other argv are rejected. Replay cwd and artifact files are realpath-confined beneath the repository, the Bun executable is pinned to the current runtime, POSIX timeout cleanup signals the replay process group, output is capped at 1 MiB, and the child environment is scrubbed. `env` may contain only `LANG`, `LC_ALL`, `LC_CTYPE`, and `TZ`, never credentials or machine/user-specific secrets. `normalization` is optional and, when provided, must be exactly `"default"`; invariants may be substring, regex, or not-substring checks. When positive invariants are present they replace exact `recordedStdout` equality; without them normalized stdout must match exactly. Commands outside this deterministic contract must use audited `replayExempt` metadata with exact fields `reasonCode`, `reason`, `approvedBy`, and `fallbackArtifactRefs` plus a structurally valid same-surface fallback artifact. Allowed `reasonCode` values are exactly `unsafe_side_effect`, `requires_credentials`, `requires_network`, `non_deterministic_external`, `destructive`, `interactive_only`, and `platform_unavailable`.
