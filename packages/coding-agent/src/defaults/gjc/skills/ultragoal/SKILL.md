...
- `create-goals` queues an aggregate reconciliation pending request even when an existing active goal is present. The harness may replace mismatched inline goals via the pending request on the next tool boundary; the session-file writer never replaces a live goal directly. Stale completed plans (all goals complete but the active goal lacks matching brief provenance) require a fresh `create-goals` to start a new run.
