...
test("createAppServer: production wiring with built-in handlers registered", () => {
...
test("createAppServer: end-to-end initialize + fs/readFile through production pipeline", () => {
	const path = join(tempDir, "prod-test.txt");
	writeFileSync(path, "production wiring works");
	const server = createAppServer();
	// Initialize handshake
	server.process(enc('{"id":1,"method":"initialize","params":{}}'));
	server.process(enc('{"method":"initialized"}'));
	// fs/readFile through the production pipeline
	const result = server.process(enc(`{"id":2,"method":"fs/readFile","params":{"path":"${path}"}}`));
...
test("createAppServer: config/read returns codexHome populated", () => {
	const server = createAppServer();
	server.process(enc('{"id":1,"method":"initialize","params":{}}'));
	server.process(enc('{"method":"initialized"}'));
	const result = server.process(enc('{"id":2,"method":"config/read","params":{}}'));
	const parsed = dec(result.response)!;
	const res = parsed.result as Record<string, unknown>;
...
test("createAppServer: each connection gets independent state", () => {
	const a = createAppServer();
	const b = createAppServer();
	a.process(enc('{"id":1,"method":"initialize","params":{}}'));
