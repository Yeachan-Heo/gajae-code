...
export default class DeepInterview extends Command {
	...
	static flags = {
		...
		"session-id": Flags.string({
			description:
				"Route state/spec handoff through the resolved session root (fresh: .gjc/sessions/_session-{sessionid}; existing legacy sessions remain at .gjc/_session-{sessionid})",
		}),
		...
		slug: Flags.string({
			description:
				"Safe slug for <resolved-session-root>/specs/deep-interview-<slug>.md (legacy session layout remains immutable)",
		}),
