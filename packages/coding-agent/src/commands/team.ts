		...
		if (action === "api") {
			...
			if (!operation || operation === "--help" || operation === "help") {
				writeText([
					...
					'Typed lane task example: create-task --input \'{"team_name":"demo","subject":"Verify delivery","description":"Run verification","owner":"worker-1","lane":"verification","required_role":"executor","depends_on":["task-1"],"write_paths":["packages/product/test/change.test.ts"]}\' --json',
				]);
				return;
			}
