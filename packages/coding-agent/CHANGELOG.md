...
- Concurrent interactive launches now retain unique core SDK discovery even when notification-daemon ownership is blocked, wait through bounded session-index contention with dead-owner recovery, and recover exact tmux-owner cleanup from competing valid terminal-verdict publishers.
