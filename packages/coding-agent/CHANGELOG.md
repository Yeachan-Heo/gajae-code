...
### Fixed
- MCP discovery now keeps tools loaded from an explicit `--mcp-config` hidden behind `search_tool_bm25` when `--tools` is omitted, while preserving the normal default GJC tool set.
