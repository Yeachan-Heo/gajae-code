...
- Improved `deep-interview` natural-language quality so non-English sessions stop emitting malformed output (misspellings / invented or transliterated non-words). The `language.instruction` carried into state is now built from one general, language-agnostic template that names the detected/requested language and requires natural, grammatically correct native phrasing, forbids invented/calqued/transliterated non-words, and keeps code identifiers, file paths, commands, settings/JSON keys, and library/API/product names in their original form. English remains the default; no language-specific directive is baked into the runtime. The SKILL question/policy steps carry the same general carve-out the spec step already had.
