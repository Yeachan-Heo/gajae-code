...
- Coordinator SDK prompts now bind their runtime turn id to the active prompt or its exact queued follow-up, so each durable coordinator turn writes its own terminal receipt. This prevents a queued follow-up from being attributed to its predecessor and makes `await_turn` complete instead of timing out.
- Advanced Telegram's operational daemon generation to 19 and Discord's to 9 for their native-authority lifecycle contract updates.
