...
- Broker-spawned session hosts are reclaimed once their client is gone. Clients do not send `session/close` on exit, so detached hosts accumulated indefinitely across restarts; a host now shuts itself down after a grace period during which its launching client is gone, no client connection is open, and no turn is in flight. The launcher is identified by pid *and* the process incarnation recorded when the session was requested, so a recycled pid cannot keep an orphaned host alive, and an unidentifiable launcher is left running rather than risking a live session. Expiry routes through the lifecycle owner's shutdown path so the session is still disposed and persisted, a reattached client keeps the host alive from its first frame rather than only once capabilities are negotiated, and a disposal failure now exits non-zero instead of reporting a clean shutdown. `GJC_SDK_HOST_IDLE_REAP_MS` overrides the 30-minute default.
- A prompt aborted before `agent_start` no longer pins a session busy forever. `turn.abort` left its correlation in `pendingPromptCorrelations`, which `isBusy()` reads, so every later prompt was refused; those correlations are now terminalized through the failure path.
