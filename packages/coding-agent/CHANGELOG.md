...
- Round-0 deep-interview asks that incorrectly emit review-only intent metadata now explain that `intent_contract` is required and `intent_review` is post-Round-0 only, preventing repeated retries against an opaque pre-coercion validation error.
