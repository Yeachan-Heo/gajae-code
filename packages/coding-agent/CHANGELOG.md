...
- The Telegram daemon's root scan now reaps orphaned session forum topics: when a local (loopback) endpoint's record is explicitly stale or its owning process is dead, the daemon deletes the session's topic and then unlinks the dead endpoint file once its topic record is cleared (a failed delete keeps the record and is retried on the next scan). Previously a hard-killed session (SIGKILL, crash, terminal pane close) never sent its graceful `session_closed` frame, and `scanRoots()` skipped dead-pid endpoints without cleanup, so the topic was orphaned in the paired chat forever. Remote (non-loopback) endpoints are never reaped, whether stale or dead-pid: a local pid check is meaningless for them and their lifecycle belongs to the remote host.
