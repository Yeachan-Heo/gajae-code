...
- Light-theme status evidence now injects a fixed project path and Git snapshot through the production `StatusLineComponent`, preventing clone paths, branch names, and worktree state from changing exact-head corpus bytes across capture and CI hosts.
