...
- Bash artifacts now carry native callback-loss byte evidence through result metadata and retained-output references, so an overloaded shell callback queue can no longer make a partial artifact claim to contain full output.
