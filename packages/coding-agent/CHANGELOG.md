...
- Light-theme status evidence now injects a fixed project path and Git snapshot through the production `StatusLineComponent`, and the consumer atlas injects a fixed key-display platform into the production `WelcomeComponent`; clone paths, branch names, worktree state, and replay-host platform no longer change exact-head corpus bytes across capture and CI.
