...
- The documented `GJC_*` environment-variable names now actually work. `docs/environment-variables.md` was renamed wholesale in the rebrand but ~25 variables were still only read under their legacy `PI_*` names (`GJC_NO_TITLE`, `GJC_TIMING`, `GJC_EDIT_VARIANT`, `GJC_SMOL_MODEL`/`GJC_SLOW_MODEL`/`GJC_PLAN_MODEL`, `GJC_NO_PTY`, `GJC_PY`/`GJC_JS`, `GJC_PYTHON_SKIP_CHECK`/`GJC_PYTHON_IPC_TRACE`, `GJC_SUBPROCESS_CMD`, `GJC_BLOCKED_AGENT`, `GJC_TASK_MAX_OUTPUT_BYTES`/`_LINES`, `GJC_DISABLE_LSPMUX`, `GJC_AUTH_NO_BORROW`, `GJC_FORCE_IMAGE_PROTOCOL`/`GJC_ALLOW_SIXEL_PASSTHROUGH`, `GJC_COMMIT_*`, `GJC_OPENAI_CODE_*` (legacy `PI_CODEX_*`), `GJC_OPENAI_STREAM_IDLE_TIMEOUT_MS`, `GJC_STREAM_IDLE_TIMEOUT_MS`/`GJC_STREAM_FIRST_EVENT_TIMEOUT_MS`, `GJC_HARDWARE_CURSOR`/`GJC_CLEAR_ON_SHRINK`/`GJC_DEBUG_REDRAW`/`GJC_TUI_DEBUG`/`GJC_TUI_WRITE_LOG`, `GJC_BASH_NO_CI`/`GJC_BASH_NO_LOGIN`/`GJC_SHELL_PREFIX`). Every read site now resolves the `GJC_*` name first with the legacy name as a fallback (a new `$pickflag` helper mirrors `$pickenv` for boolean flags), matching the existing `GJC_RPC_EMIT_TITLE` / `GJC_AI_GEMINI_CLI_VERSION` dual-read precedent. Docs corrections in the same pass: the `ANTHROPIC_MODEL_*` "legacy alias" names were rebrand-sed artifacts of the real `CLAUDE_*`/`CLAUDE_CODE_*` aliases, the stale `GJC_RUNTIME_BINARY` bridge section was removed (those commands are native now), the unread `GJC_OPENAI_CODE_WEBSOCKET_V2` row was dropped, and the `GJC_PY` row now describes the actual `GJC_PY`/`GJC_JS` boolean-flag semantics.
