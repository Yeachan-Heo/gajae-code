...
- Alibaba Token Plan first-event timeouts from provider streams and the exported lazy-stream watchdog now surface without session retry/fallback replay, preserve sticky fallback selection across later turns, and avoid same-candidate auto-compaction replay, preventing repeated provider usage (#3026).
