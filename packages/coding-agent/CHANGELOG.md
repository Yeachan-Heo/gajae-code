...
- Bracketed paste handling now preserves normal text typed before the paste start marker when terminal input splits or prefixes the marker, so pasted content no longer absorbs the preceding composer text.
