...
- The Telegram notification daemon now re-checks its spawning owner process on each ownership watchdog tick and self-exits once that process is gone, so a detached daemon orphaned by a closed session no longer keeps polling the bot token alongside the fresh daemon a later session spawns. Telegram round-robins callback queries between the two daemons in that window, and every inline button tied to the older daemon's in-memory message routes missed, flooding the chat with "This button is stale after notification daemon restart." The owner-id already embeds the launcher PID, so this reuses the existing startup-only liveness probe continuously; Windows source launchers with no embedded PID are left untouched (#3584).
