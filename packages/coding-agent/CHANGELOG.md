...
- Windows remote session lifecycle (`session.create` through the SDK broker, Telegram `/session_create`, coordinator MCP `start_session`) no longer kills freshly spawned healthy sessions and reports `terminal_uncertain`: the broker effect-marker write, lifecycle-ledger compaction, and coordinator MCP durable-state writes now tolerate the unsupported Windows directory-`fsync` `EPERM` through the shared classifier from the workflow-gate store fix (directory-level rename durability is best-effort on Windows, as it is for every process there), while every other directory-sync failure remains fail-closed.
