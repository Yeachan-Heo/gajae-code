...
- Fixed session resume crashing with `TypeError: undefined is not an object (evaluating 'usage.input')` when a persisted transcript contained a parseable-but-malformed assistant entry missing `usage` (or a `usage` without `cost`) — as produced by torn concurrent multi-writer / NFS appends. `parseSessionEntries` already tolerates fully unparseable lines via `parseJsonlLenient`; `#buildIndex` now guards its load-path usage aggregation so a single corrupt assistant entry no longer takes down the entire resume.
