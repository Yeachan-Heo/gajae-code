...
- Persistent `monitor` notifications no longer start one agent turn per stdout line. Default delivery debounces (~2s), coalesces to the latest line, uses `on_change` semantics without intermediate `triggerTurn`, and still wakes promptly on process exit; optional `notify` (`on_change` | `every_line` | `display_only`) preserves legacy per-line turns when requested (#3470).
