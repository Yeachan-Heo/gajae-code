...
- Autonomous RLM no-model startup now preserves the outer owner's finalization path when `suppressProcessExit` is enabled: the root command records failure and returns instead of terminating before kernel cleanup and report writing (#3621).
