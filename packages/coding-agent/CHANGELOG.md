...
- `gjc gc` gains a **report-only** `sessions` store that inventories session transcripts and their sibling artifact trees against age and aggregate-size report thresholds. It is opt-in through the user/global settings layer only (`sessions.inventory.enabled`, default `false`; `sessions.inventory.maxAgeDays`; `sessions.inventory.maxTotalSizeMb`) so a project config cannot enable a machine-wide surface, and default-OFF is enforced at the **scan** boundary: a disabled inventory performs no directory enumeration and no artifact-tree sizing at all. It **never deletes anything** — every record is non-removable and `--prune` / `--force` / `--dry-run` all leave transcripts and artifact trees byte-identical. Discovery is bounded to the immediate managed scope and never recurses into artifact trees; sizing uses `lstat` only; transcript headers are read through a capped 64 KiB prefix so a multi-GB transcript cannot exhaust memory.

- `gjc --tmux` sessions no longer accumulate a permanent coordinator sidecar failure after the first in-pane session-identity change. The runtime-state marker's `session_file` and the postmortem finalizer are now rotated at every committed identity transition (`/new` on both commit paths, `switchSession` after its fallible `session_switch` emit, plus fork, handoff and branch), through a separate authorized entry point that keeps the ordinary event path's fail-closed identity assertion intact. The rotation publishes a **fresh non-terminal** successor marker rather than relabeling the predecessor payload, so a queued predecessor `agent_end` can no longer falsely terminalize the successor transcript, and under managed ownership the prior generation must be present, well-typed and exactly equal. Sidecar failures also carry a discriminant: the twelve branches that previously collapsed into one opaque message are now distinguishable in logs (no marker bytes, no owner env values).

- A synchronous transcript-append failure no longer kills the session process. `InteractiveMode` is now the single owner of goal-session-event rejections, so an `ENOSPC` (or `managed_append_missing`) throw from `SessionManager._persist` can no longer escape the floating `session.subscribe` callback and reach the process-level fatal handler — which, under `gjc --tmux`, exits the pane the agent is running as. The session instead enters a persistent read-only quarantine: a composer banner names the transcript, the sanitized errno, that later entries are **not** saved, and the recovery action; new turns are refused at every entrant (prompt admission, the shared steer/follow-up/custom fence, and scheduled continuations) while inspection, export and exit stay available. Shutdown's awaited teardown steps are contained too, since Ctrl-D and the second Ctrl-C dispatch it fire-and-forget. Durability is unchanged and still fail-closed — `_persist` throws, `#persistError` latches, and goal-exit bookkeeping now writes the resume-critical `mode_change` **before** clearing in-memory goal state so a failed append can never report a completion the transcript does not record. Public surfaces carry only a bounded sanitized failure detail and the transcript basename; full causes stay in protected diagnostics.
