...
- Telegram daemon state and topic-registry publication now complete their Windows replacement with `MOVEFILE_WRITE_THROUGH` instead of a cached rename, so an acknowledged write is on stable storage before publication returns. When the native binding is unavailable, throws, or proves the replacement did not commit, publication falls back to the previous rename so availability is unchanged; it refuses to fall back only when the outcome cannot be proven, because a retry there could resurrect the staged file over a newer successor.
