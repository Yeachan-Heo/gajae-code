...
- Mid-session credential rotation is now observable and treats a content-free 401 like a content-free 429. Accepted same-model rotations emit a structured `credential_switched` session event (`provider`, opaque stored `from`/`to` row ids, `reason`, optional `retryAfterMs`, `timestamp`) so RPC/ACP/subagent consumers can see which credential was left and why, without carrying email/account/key material. The rotate-and-retry gates that previously admitted only `quota`/`rate_limit` now also admit `auth` under the same content-free condition; a plain `forbidden` (HTTP 403 / typed `forbidden` code) remains terminal and still mutates no credential state (#3723, residual of #3724).
