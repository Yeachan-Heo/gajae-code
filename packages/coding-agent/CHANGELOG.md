...
- SDK session lifecycle tests now seed the required cold-memory heartbeat authority and use the supported `gjc sdk session raw global` CLI surface. ACP deletion preserves broker-owned host identity when endpoint transport is unavailable, tombstones deleted sessions so they disappear from discovery, and continues to descriptor-bound cleanup after a stale close refusal; live sessions remain protected by the broker delete fence (#4229).
