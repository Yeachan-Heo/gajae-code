...
- Task subagents that lack the `irc` tool are no longer told to use it. The subagent prompt injected the IRC peer roster and "reach other agents via the `irc` tool" instructions based only on `irc.enabled`, ignoring the agent's tool allowlist — so role agents such as `architect`/`planner`/`critic` (which declare `tools` without `irc`) received coordination instructions for a tool they cannot call. The injection is now gated on the subagent actually having the `irc` tool (`undefined` toolNames means the full default toolset, otherwise the allowlist must include `irc`).
