...
- Model profile switching now removes assignments omitted by the next profile, preserves inherited user/runtime defaults without copying project settings globally, materializes the authoritative profile DEFAULT instead of transient fallback models, and keeps a different session-active profile intact when deleting the persisted default. Profile selection now waits for activation before closing, deletion persistence is failure-reporting and transactional, post-commit notification failures no longer roll back or falsely report successful model changes as failed, and custom snapshots prefer configured/profile defaults over transient live models.
