...
- Provider-agnostic preset ranking now uses AuthStorage's effective selectable credential type. An unusable stored command key no longer misclassifies an OAuth-backed provider as manual-key, and expired or blocked OAuth no longer outranks a usable environment key.
