...
### Added
- Added Telegram `/btw <question>` support through a dedicated SDK ephemeral-turn frame, preserving current-session context without injecting or persisting a user message.
- Telegram `/btw` replies now promote supported Markdown tables and LaTeX delimiters to native rich-message table and mathematical-expression blocks by default, with original-Markdown or HTML fallback for unsupported content, delivery failures, and size/structure limits.
