...
- Interactive Codex sessions now re-import the current Codex CLI OAuth credential once after the previously imported refresh token is definitively rejected. Recovery requires prior acceptance of external credential imports, is not triggered by logout or an ordinary missing key, and does not retry the same rejected external token generation.
