...
- Made workflow management fail fast when launch-only `--mcp-config` is placed before `gjc ralplan` or `gjc state`, while canonical management invocations remain MCP-independent.
- Hardened Ralplan persistence with a read-only preflight, bounded byte-identical retry guidance, and canonical digest-verified receipt recovery that rejects malformed, duplicate, oversized, symlinked, or tampered artifacts.
- Kept rich role-agent review findings in task receipts when a closed completion schema cannot accept a top-level `findings` field.
