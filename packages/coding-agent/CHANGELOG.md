...
- Light-theme status evidence now injects a fixed project path and Git snapshot through the production `StatusLineComponent`, and the consumer atlas injects a fixed key-display platform and terminal color capability into the production `WelcomeComponent`; clone paths, branch names, worktree state, replay-host platform, and detected terminal capability no longer change exact-head corpus bytes across capture and CI.
