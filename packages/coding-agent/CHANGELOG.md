...
- Kiro usage rows continue positional account numbering across limited and unavailable credentials, first-event timeout retries remain bounded when an Ollama/Kiro error message is handled after the active model changes, and a clean auth rejection gets one immediate credential-refresh retry under bare defaults.
- Non-interactive print mode now waits for agent-end retry continuations before rendering or disposing the session, so a credential refresh or managed fallback scheduled after the first failed request completes within the same `gjc -p` invocation.
...
- Added native Kiro provider support: personal login selection (AWS Builder ID, Google, GitHub), account-specific profile/model discovery, streaming inference with tool-call round trips, the Claude-equivalent `kiro-opus` profile, and the all-Kiro `kiro-opus-gpt` profile combining Claude Opus/Sonnet with GPT-5.6.
