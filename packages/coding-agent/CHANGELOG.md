...
- A single bare model selector typo in a custom profile now reports an unknown catalog model configuration error instead of masquerading as a missing provider credential and directing users to `/login`; known aliases with unavailable credentials remain recoverable authentication errors.
- Command-palette cycle availability checks no longer let read-only role-count resolution mutate an active profile's canonical provider affinity; actual user-initiated cycling still uses the live canonical scope. Bare profile aliases now clamp explicit thinking suffixes against the authenticated concrete model while preserving the alias itself.
