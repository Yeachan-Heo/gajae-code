...
- Managed-session `dropSession` no longer fails when native `exactUnlink` retains only an exchange placeholder while the detached artifact quarantine still holds bytes. Direct delete continues recursive removal after a `cleanup_pending` detach, retries placeholder-only quarantine cleanup, retires live transcript paths, and leaves intermediate quarantine GC to `reconcileManagedTombstones` so Dev CI shards that call `dropSession` stay green (#3538).
