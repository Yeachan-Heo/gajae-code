...
### Removed
- Removed the inert `tasks.todoClearDelay` setting from the settings schema, settings UI, and generated `config.schema.json`. The timer-driven auto-clear of completed/abandoned tasks was already deleted from `agent-session.ts` because it mutated canonical `#todoPhases` between tool calls, so the setting had zero runtime effect and misled users who changed it. Branch resynchronization may still intentionally filter completed/abandoned tasks when compaction or session-history transitions rebuild todo state. A leftover `tasks.todoClearDelay` key in an existing `config.yml` is silently ignored (unknown keys are preserved, never read).
