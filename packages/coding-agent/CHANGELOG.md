...
- A rejected resident-cache root now names the condition that rejected it. Every failing shape — a symlinked root, a non-directory, a foreign owner, and a group- or world-accessible mode — collapsed into one `directory_untrusted` message with only the path, so an operator whose profile directory was assembled by an external launcher or restored under a permissive umask could not tell a redirect from a `chmod`. The check now reports `directory_symlink`, `directory_not_a_directory`, `directory_foreign_owner`, or `directory_group_or_world_accessible`, and appends the observed evidence (the offending mode, or the owning uid against the expected one). The trust boundary itself is unchanged: the same paths are rejected, and the memory-fallback and hard-failure policies stay as they were.
