...
- Zero-token empty provider responses are now treated as typed retryable failures instead of successful turns; clean single-model sessions retry within their bounded budget, and managed chains discard the provisional response before falling back.
