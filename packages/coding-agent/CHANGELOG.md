...
- Model profile switching now removes assignments omitted by the next profile, keeps role-only profile effort consistent between the live session and restart, preserves inherited user/runtime defaults without copying project settings globally, materializes the authoritative profile DEFAULT instead of transient fallback models, and keeps a different session-active profile intact when deleting the persisted default. Deletion rollback uses durable conflict detection so it cannot overwrite a later concurrent role update.
