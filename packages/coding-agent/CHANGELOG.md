...
- Ultragoal CLI replay evidence now permits focused `bun test` verification without opening arbitrary host execution. The deterministic contract accepts only the pinned Bun runtime for `--version`, literal `-e "console.log(...)"`, or `test` with explicit repository-contained non-symlink `*.test.*`/`*.spec.*` files; shells, nested interpreters, path-qualified executables, install/publish/network commands, globs, and parent paths remain blocked. Replay cwd and artifact files are realpath-confined, ambiguous inline/file records fail closed, POSIX timeout cleanup signals the replay process group, and existing env/output/exit checks remain enforced (#3533).
- Ultragoal CLI evidence no longer executes model-authored test source or trusts `replaySafe: true` as arbitrary command authority. Runtime replay is limited to the pinned Bun runtime for `--version` and literal `-e "console.log(...)"`; shells, interpreter code strings, path-qualified executables, tests, install/publish/network/git mutation commands, and arbitrary argv are rejected. Focused `bun test` results use an audited, non-executing `bun-test-report` fallback with explicit argv, consistent zero-failure counts, exit code, and stdout digest. Replay cwd/artifact files are realpath-confined, ambiguous rows fail closed, stdout and stderr are checked, and POSIX timeout cleanup signals the process group (#3533).
- Ultragoal CLI replay evidence now reads the replay file referenced by an `executorQa.artifactRefs` entry whose `kind` is `cli-replay`. Such a row previously short-circuited as an inline replay record, so its `path` was never read and validation failed with a confusing `schemaVersion must be 1` diagnostic. Inline, nested, and file-backed replay forms are now disambiguated explicitly; mixed or malformed rows fail closed (#3533).
