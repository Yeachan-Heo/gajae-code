...
- LSP child-process stdin peer closure is now contained at the LSP transport boundary: the dead client is evicted, pending requests fail with a stable transport-closed error, and ignored background notifications can no longer leak `EPIPE` as a process-level unhandled rejection. Non-peer write failures still propagate.
