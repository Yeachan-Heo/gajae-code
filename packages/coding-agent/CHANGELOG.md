...
- Broker-spawned session hosts are reclaimed once their client is gone. Clients do not send `session/close` on exit, so detached hosts accumulated indefinitely across restarts; a host now shuts itself down after a grace period during which the launching client's pid is dead, no client connection is negotiated, and no turn is in flight. Expiry routes through the lifecycle owner's shutdown path so the session is still disposed and persisted, and a reattached client keeps the host alive. `GJC_SDK_HOST_IDLE_REAP_MS` overrides the 30-minute default.
