...
- Telegram tool-activity updates can now be disabled persistently with `/tools off` (and restored with `/tools on`) or from the Notifications preferences UI, without disabling final answers, asks, or other notification delivery.
