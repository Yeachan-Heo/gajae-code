...
### Removed
- Removed the legacy worktree cleanup modules and their package subpath exports (`@gajae-code/coding-agent/cli/worktree-cli`, `@gajae-code/coding-agent/commands/worktree`). The `gjc worktree`/`wt` CLI command has been unregistered since the workflow-surface narrowing; the modules behind it (including the recursive-deletion `clear` path) were only reachable as package imports and are now tombstoned (`null` exports). Leftover managed worktrees under `~/.gjc/wt/` should be inspected and removed manually with `git worktree remove` / `git worktree prune` or plain filesystem tools.
