...
- Telegram topic delete settlement is now fence-epoch bound and route-atomic. `TopicRegistry.settleDelete` rebuilds derived inbound routes after removing a definitely-deleted record, so a surviving colliding record becomes routable and a settled topic id becomes adoptable without waiting for a daemon restart; it also requires the caller's dispatched authority epoch to still equal both the record's own epoch and the session's current epoch, so a held earlier delete can no longer settle a newer scan/close-started fence for the same session and topic and release its quarantine. `restoreDeleteFence` rebuilds routes on successful restoration, so a rolled-back fence after a failed topic-state persist re-quarantines the topic id instead of leaving inbound routing open to the collision partner.
