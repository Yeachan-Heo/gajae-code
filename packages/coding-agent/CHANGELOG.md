...
- Single-model sessions now rotate through every eligible stored credential for the same provider after content-free quota or rate-limit failures, including when provider lifecycle extensions are loaded. Intermediate credential failures stay out of user-visible event streams and the TUI; only the final error is released when the credential pool is exhausted. Explicit retry opt-out, runtime API-key pins, partial model output, and tool-side effects still fail closed without replay.
