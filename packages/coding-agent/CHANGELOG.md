...
- `gjc --worktree` / `gjc -w` launch no longer crashes with a raw uncaught `EEXIST` when the worktree bucket directory (`<repo>.gajae-code-worktrees`) is a broken symbolic link to unmounted or offloaded cold storage. The launch now detects the dangling symlink (or a non-directory bucket entry) before `mkdirSync` and throws an actionable `worktree_bucket_broken_symlink` / `worktree_bucket_not_directory` diagnostic naming the path, the link target, and the `rm` remediation step.
