...
- Deep-interview draft checks now dry-run the same state transition as consume, report the violated invariant with bounded expected/actual evidence, and identify the validated state revision. Failed pre-commit consumes release their attempt for correction, revision conflicts return typed recovery, `rebase --to-current` validates against the latest state, and bounded batch edits remove client-side revision arithmetic without weakening consume CAS.
