...
- Telegram notification daemon ownership hardening (#3048): root registrations now mint per-registration tokens so a stale same-session cleanup can never unregister or roll back a live replacement registration (legacy token-less registrations keep root-match behavior); daemon readiness attaches same-serving-epoch cross-generation owners instead of hard-requiring generation equality; a failed initial heartbeat-sidecar proof after ready publication rolls the state forward to `retired` and exact-unlinks the rebound ownership lock so contenders never attach to a dying owner; bot-wide 429 cooldown flush grants requeue untouched and settle `ambiguous` (retrying after cooldown), and mid-item suppression flips an otherwise-accepted grant to `ambiguous` so a possibly-undelivered frame is never recorded as accepted.
