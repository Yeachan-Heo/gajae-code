...
- Windows directory durability barriers now use the shared `directory-sync` policy across runtime, broker, lifecycle, and configuration writes: unsupported Windows directory-`fsync` (`EPERM`/`EINVAL`/`ENOTSUP`/`EOPNOTSUPP`) is tolerated only at durability-contract barriers, while `EACCES`, `EBADF`, `EIO`, open, and close failures remain fail-closed. Post-replacement paths explicitly use the separate fully best-effort policy.
