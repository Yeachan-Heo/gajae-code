...
- The Telegram daemon's root scan now reaps orphaned session forum topics: when a local (loopback) endpoint's owning process is dead, or the endpoint record is explicitly stale, the daemon deletes the session's topic and then unlinks the dead endpoint file once its topic record is cleared (a failed delete keeps the record and is retried on the next scan). Previously a hard-killed session (SIGKILL, crash, terminal pane close) never sent its graceful `session_closed` frame, and `scanRoots()` skipped dead-pid endpoints without cleanup, so the topic was orphaned in the paired chat forever. Remote (non-loopback) endpoints are left untouched because a local pid check is meaningless for them.
