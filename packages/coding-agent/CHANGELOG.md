...
- Telegram notification daemons treat a bounded streak of sustained `getUpdates` 409 conflicts as self-defense: a process that no longer holds durable ownership exits through normal shutdown (no peer signal/kill), while a still-owning daemon keeps serving so an orphan thrash cannot push the legitimate owner out. Ownership reclaim / PID-reuse provenance is unchanged (bounded secondary fence for #3587; full dual-poller acquisition fence remains open).
