...
- `/new`, `fork()`, handoff, and branch/tree-jump transitions now await verified managed `local://` legacy-root migration for the successor session identity before any synchronous `local://` resolution, matching cold-start `createAgentSession()` (#2797) and `/resume` (#2925). Sending a prompt right after `/new` no longer fails with "local:// legacy migration must complete before path resolution".
