...
- Parallel team workers now use exact canonical `write_paths`, crash-recoverable task and claim authority, and guarded cross-process mutation locks, preventing built-in edit bridges from clobbering another active worker's files without changing standard non-team `write` overwrite behavior.
