...
- Telegram `notify setup` activation now works when `notifications/` is a directory symlink (multi-account shared notification dirs). Exact transition-lock release previously rejected intermediate directory reparse points, left `telegram-daemon.steal` behind, and failed with "provisional ownership could not be retired safely" while durable settings remained armed. Intermediate directory symlinks are resolved before native exact unlink; final-component file symlinks stay rejected (#3761).
