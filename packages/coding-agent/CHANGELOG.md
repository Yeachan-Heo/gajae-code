...
- Reattaching to an ACP session no longer leaves it permanently unpromptable. The turn owned by a lost connection is now abandoned on reattach, and a prompt landing on a host that is still unwinding retries the abort instead of failing; `session/load` and `session/resume` also accept the `mcpServers` field the spec requires, which previously threw `conflict` and made an MCP-configured client unable to reopen a session it had created.
- A headless `ask` is now surfaced to ACP clients instead of leaving an unexplained spinner: the question is presented through `elicitation/create` form mode when the client advertises `elicitation.form`, and otherwise emitted as turn content per the elicitation RFD's text fallback. In the fallback case the ask deliberately stays pending rather than selecting an option on the user's behalf.
