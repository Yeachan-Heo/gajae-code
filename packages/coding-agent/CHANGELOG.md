...
- Converted managed-owner, tmux launch/session, coordinator state, broker lifecycle/ledger, workflow-gate, conversation-store, lifecycle-control, and configuration replacement directory barriers to the shared `directory-sync` policies. Durability-contract barriers tolerate only unsupported Windows directory-`fsync` errors (`EPERM`/`EINVAL`/`ENOTSUP`/`EOPNOTSUPP`); open, close, and unclassified sync failures remain fail-closed, while the three existing post-replacement paths retain fully best-effort semantics.
