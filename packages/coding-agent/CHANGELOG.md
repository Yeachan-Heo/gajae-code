...
- Model selector and Agent Control Center role assignments now update project-shadowed/runtime overrides as well as persisted settings, immediately refreshing role badges and custom-preset snapshots. Per-role persistence also preserves sibling assignments written by concurrent GJC sessions instead of replacing the whole role map.
- Model profile switching now removes assignments omitted by the next profile, keeps role-only profile effort consistent between the live session and restart, preserves inherited user/runtime defaults without copying project settings globally, materializes the authoritative profile DEFAULT instead of transient fallback models, and keeps a different session-active profile intact when deleting the persisted default. Profile deletion and rollback use canonical file locks plus opaque per-mutation ownership tokens, preserving pending same-session choices, ABA updates, symlink-alias writes, concurrent profile edits, and ambiguous durable deletions.
- Profile selection now waits for activation before closing, while post-commit notifications are detached so failed or stalled transports cannot wedge `/model` or selector input. Notification failures no longer roll back or falsely report successful model changes as failed, and custom snapshots prefer configured/profile defaults over transient live models.
- Batch `/model` success output now lists each target when omitted effort preserves different per-role efforts, instead of falsely claiming every target received the DEFAULT selector and effort.
