...
### Added
- Added a deterministic quick-lane task classifier (`classifyQuickLane`) with a read-only `gjc quick-lane classify` surface. It encodes the routing gate that the deep-interview and ralplan skills previously only described in prose: small, clearly scoped, low-risk tasks (explicit file paths, issue/PR numbers, named symbols, explicit tests, numbered steps, acceptance criteria, error references, code blocks, or an explicit `force:`/`!` override) classify to the quick lane, while vague, ambiguous, exploratory, risk-sensitive, or multi-file/cross-contract requests are excluded and keep the existing deep/interview path untouched. The classifier is fail-closed: any exclusion forces the deep lane even when a concrete anchor is present, and it never alters deep-path defaults.
