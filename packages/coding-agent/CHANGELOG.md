...
- Custom `anthropic-messages` providers can now configure `compat.promptCacheMode` (`none`, `explicit`, or `automatic`) and `compat.supportsLongCacheRetention` at provider, model, and model-override levels. Non-canonical Anthropic-compatible endpoints emit no generated cache controls until one of these modes is selected, so gateways that already attach their own block-level markers are not pushed past Anthropic's four-breakpoint limit, and endpoints that need block-level markers or lack 1-hour retention stay configurable.
