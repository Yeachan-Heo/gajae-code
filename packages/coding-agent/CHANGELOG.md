...
- Task result receipts now cap `review.overallCorrectness` at 200 characters, matching the existing finding-summary bound. `buildReview` truncated finding summaries but passed the child-supplied `overall_correctness` verdict through verbatim, so a verbose or misbehaving subagent could pipe an unbounded string into the parent's context through the distilled receipt (the field is also counted as inline tokens during spawn-ROI reconciliation). It is now truncated like the other distilled review fields.
