...
`,
	...
	it("admits only the complete owner-isolation new-session grammar", () => {
		const provider = process.env.GJC_TMUX_COMMAND?.trim() || "tmux";
		const launchCwd = process.platform === "win32" ? "C:\\work" : "/work";
		const validLaunchArgv = [
			provider,
			"-L",
			"socket",
			"new-session",
			...
			"-n",
			"gjc",
			"exec env GJC_SESSION_NAME='owned-session'; exec gjc",
		] as const;
		expect(isTrustedTmuxOwnerIsolationArgv(validLaunchArgv)).toBe(true);
		expect(
			isTrustedOwnerIsolationProtocolRequest({
				...request,
				platform: process.platform,
	...
	it("preserves argv literally and bounds the JSON-line protocol", async () => {
		...
		expect(parseOwnerIsolationRequest(JSON.stringify({ ...request, state_dir: "relative/state" }))).toBeNull();
		expect(parseOwnerIsolationRequest(JSON.stringify({ ...request, session_id: "bad\u0000id" }))).toBeNull();
	...
	it("does not classify a symlinked generation record as absent", () => {
		if (process.platform === "win32") return;
		const state = fsSync.mkdtempSync(path.join(os.tmpdir(), "gjc-owner-generation-symlink-"));
		try {
			const absent = captureOwnerGenerationBaselineSync(state, "session");
			replaceOwnerGenerationSync(state, "session", "generation", absent);
			const generationFile = lifecyclePaths(state, "session", "generation").generationFile;
			const target = path.join(state, "forged-generation.json");
			fsSync.writeFileSync(target, fsSync.readFileSync(generationFile));
			fsSync.unlinkSync(generationFile);
			fsSync.symlinkSync(target, generationFile);
			expect(() => captureOwnerGenerationBaselineSync(state, "session")).toThrow("baseline_generation_corrupt");
		} finally {
			fsSync.rmSync(state, { recursive: true, force: true });
		}
	...
	it("does not publish a successor into a replacement lifecycle root", () => {
		if (process.platform === "win32") return;
		const state = fsSync.mkdtempSync(path.join(os.tmpdir(), "gjc-owner-generation-root-publication-"));
		const sessionId = "session";
		const generation = "generation";
		let displacedRoot = "";
		try {
			const absent = captureOwnerGenerationBaselineSync(state, sessionId);
			replaceOwnerGenerationSync(state, sessionId, generation, absent);
			const baseline = captureOwnerGenerationBaselineSync(state, sessionId);
			const paths = lifecyclePaths(state, sessionId, generation);
			...
			expect(() => replaceOwnerGenerationSync(state, sessionId, "successor", baseline)).toThrow(
				"baseline_generation_changed",
			);
			expect(JSON.parse(fsSync.readFileSync(paths.generationFile, "utf8"))).toMatchObject({ generation });
		} finally {
			__setOwnerGenerationAfterMarkerForTests(undefined);
