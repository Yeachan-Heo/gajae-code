...
describe("session inventory GC", () => {
	it("is default-OFF, including when prune is requested", async () => {
		...
		expect(report.stores.sessions).toHaveLength(2);
		for (const record of report.stores.sessions) {
			expect(record).toMatchObject({ status: "retention_disabled", removable: false, action: "none" });
			expect(await sessionInventoryGcAdapter.prune(record, context(agentDir, true))).toEqual({
				removed: false,
				skipped: "report_only",
			});
		}
	...
	it("loads global inventory settings without initializing the singleton and ignores project settings", async () => {
		...
		expect(report.errors).toEqual([]);
		expect(report.stores.sessions[0]).toMatchObject({ status: "retention_disabled" });
