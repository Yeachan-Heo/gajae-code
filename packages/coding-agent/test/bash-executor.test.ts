...
describe("executeBash", () => {
	...
	it("handles multi-million line output without freeze or OOM", async () => {
		...
		// Output summary should be bounded but non-empty even when the native
		// execution layer caps pathological streams before JavaScript sees every
		// line. The regression contract is no freeze/OOM plus bounded truncation,
		// not exact accounting of all generated bytes across native caps.
		...
		// Native execution may cap pathological streams before JavaScript sees the
		// final generated line, so do not require the visible tail to include the
		// numeric end of the synthetic range. The regression contract is that the
		// command completes and returns a bounded, truncated output preview.
