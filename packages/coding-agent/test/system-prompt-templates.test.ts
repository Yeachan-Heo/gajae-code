...
describe("system Handlebars prompt templates", () => {
	...
	test("system-prompt routes explicit worktree requests through the durable worktree field", async () => {
		const templatePath = path.join(systemPromptsDir, "system-prompt.md");
		const template = await Bun.file(templatePath).text();
		const rendered = prompt.render(template, baseRenderContext);

		expect(rendered).toContain('explicit user request to use a worktree (for example, "use worktree")');
		expect(rendered).toContain("delegate implementation through `task` with `worktree: true`");
		expect(rendered).toContain('`worktree: "<branch-name>"`');
		expect(rendered).toContain("in-session counterpart of launching `gjc --worktree`");
		expect(rendered).toContain("mutually exclusive with `isolated`");
		expect(rendered).toContain("regardless of `task.isolation.mode`");
		expect(rendered).toContain("report the returned conflict instead of editing in the parent session");
		// The old, false claim that plain PAL isolation is worktree-identical must not come back.
		expect(rendered).not.toContain("delegate implementation through `task` with `isolated: true`");
	});
