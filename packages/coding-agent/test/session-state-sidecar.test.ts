...
describe("coordinator runtime state sidecar", () => {
	...
	it("publishes a fresh non-terminal successor marker instead of relabeling predecessor terminal state", async () => {
		const root = await tempRoot();
		const stateFile = path.join(root, "state.json");
		process.env[GJC_COORDINATOR_SESSION_STATE_FILE_ENV] = stateFile;
		process.env[GJC_COORDINATOR_SESSION_ID_ENV] = "terminal-carryover";
		// A queued predecessor `agent_end` can land before the rotation, so the marker
		// on disk may already be terminal when rotation runs.
		const predecessor = {
			schema_version: 1,
			session_id: "terminal-carryover",
			state: "completed",
			ready_for_input: true,
			live: false,
			updated_at: new Date().toISOString(),
			ended_at: new Date().toISOString(),
			final_response: { source: "predecessor-answer" },
			error: { code: "agent_error", message: "predecessor failure", recoverable: true },
			cwd: root,
			workdir: root,
		...
		const rotated = await readPayload(stateFile);
		expect(rotated.session_file).toBe(path.join(root, "second.jsonl"));
		// Rejects a `{...previous, session_file}` spread: the successor must not
		// inherit the predecessor's terminal lifecycle, or a completed predecessor
		// would falsely terminalize it and suppress its real postmortem.
		expect(rotated.state).toBe("running");
		expect(rotated.live).toBe(true);
		expect(rotated.ready_for_input).toBe(false);
		expect(rotated.ended_at).toBeUndefined();
		expect(rotated.final_response).toBeUndefined();
		expect(rotated.error).toBeUndefined();
	});

	it("refuses rotation when managed owner evidence is missing or malformed", async () => {
		const root = await tempRoot();
		...
		try {
			...
			// Under managed ownership a prior marker must carry a well-typed, exactly
			// equal generation. Missing and non-string values are authority downgrades.
			for (const [label, ownerGeneration] of [
				["missing", undefined],
				["non-string", 42],
			] as const) {
				const stateFile = path.join(root, `owner-${label}.json`);
				process.env[GJC_COORDINATOR_SESSION_STATE_FILE_ENV] = stateFile;
				process.env[GJC_COORDINATOR_SESSION_ID_ENV] = "owner-evidence";
				const marker = {
					schema_version: 1,
					session_id: "owner-evidence",
					state: "running",
					cwd: root,
					workdir: root,
					session_file: path.join(root, "first.jsonl"),
