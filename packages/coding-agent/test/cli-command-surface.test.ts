...
describe("GJC public CLI command surface", () => {
	...
	it("rejects an internal managed-owner supervisor invocation without published authority", async () => {
		...
		try {
			const refused = Bun.spawnSync(["bun", cliEntry, "--internal-managed-owner-supervisor"], {
				cwd: repoRoot,
				stdout: "pipe",
				stderr: "pipe",
				env: {
			...
			const refusedOutput = `${refused.stdout.toString()}\n${refused.stderr.toString()}`;
			expect(refused.exitCode, refusedOutput).not.toBe(0);
			expect(refusedOutput).toContain("managed_owner_supervisor_authority_unavailable");
			expect(refusedOutput).not.toContain("gjc/");
			expect(await Bun.file(lifecycle.generationFile).exists()).toBe(false);
		} finally {
