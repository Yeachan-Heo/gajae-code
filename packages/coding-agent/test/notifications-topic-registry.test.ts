...
describe("TopicRegistry", () => {
	...
	test("restores failed-delete state only when no replacement topic exists", async () => {
		const reg = new TopicRegistry();
		const stale = await reg.getOrCreateTopic("s1", async () => "stale");

		expect(reg.delete("s1")).toBe(true);
		expect(reg.restoreIfAbsent("s1", stale)).toBe(true);
		expect(reg.sessionForTopic("stale")).toBe("s1");

		expect(reg.delete("s1")).toBe(true);
		const fresh = await reg.getOrCreateTopic("s1", async () => "fresh");
		expect(reg.restoreIfAbsent("s1", stale)).toBe(false);
