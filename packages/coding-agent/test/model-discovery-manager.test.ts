...
describe("ModelDiscoveryManager", () => {
	...
	test("re-resolves a key after its credential generation changes", async () => {
		const provider = { provider: "test" };
		...
		let authGeneration = "credential-a";
		let peekCount = 0;
		...
		const discovery = manager.discover(provider, "online", {
			...
			fetchModels: async (_provider, key) => {
				expect(key).toBe("credential-b");
				return [model("current")];
			},
			getEvidenceGeneration: () => authGeneration,
		});
		authGeneration = "credential-b";
		resolveApiKey("credential-a");

		expect(await discovery).toMatchObject({ current: true, state: { status: "ok", models: ["current"] } });
