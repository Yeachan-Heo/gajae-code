...
test("runDaemonInternal rejects invalid validation destinations before daemon startup", async () => {
	let settingsInitialized = false;
	const deps = {
		SettingsImpl: {
			init: async () => {
				settingsInitialized = true;
				throw new Error("validation argument parsing must precede settings startup");
			},
		},
	};
	for (const { argv, message } of [
		{
			argv: ["--owner-id", "owner", "--validation-test-supergroup-chat-id"],
			message: "--validation-test-supergroup-chat-id requires a negative -100... chat ID",
		},
		{
			argv: [
				"--owner-id",
				"owner",
				"--validation-test-supergroup-chat-id",
				"-100123",
				"--validation-test-supergroup-chat-id",
				"-100456",
			],
			message: "--validation-test-supergroup-chat-id may be supplied only once",
		},
		{
			argv: ["--owner-id", "owner", "--validation-test-supergroup-chat-id", "--agent-dir"],
			message: "--validation-test-supergroup-chat-id requires a negative -100... chat ID",
		},
		{
			argv: ["--owner-id", "owner", "--validation-test-supergroup-chat-id", "123"],
			message: "--validation-test-supergroup-chat-id requires a negative -100... chat ID",
		},
		{
			argv: ["--owner-id", "owner", "--validation-test-supergroup-chat-id", "-999"],
			message: "--validation-test-supergroup-chat-id requires a negative -100... chat ID",
		},
	]) {
		await expect(runDaemonInternal(argv, deps)).rejects.toThrow(message);
	}
	expect(settingsInitialized).toBe(false);
});
test("runDaemonInternal validation launch acquires, spawns, and confirms a private owner", async () => {
	const agentDir = tempAgentDir();
	const s = setPrivateAgentDir(settings(agentDir), agentDir);
	const childPid = 4243;
	const launcherPid = 4242;
	const validationChatId = "-100123456";
	const spawnedArgv: string[][] = [];
	let published = false;
	const telegramDaemonDeps = {
		pid: launcherPid,
		platform: "linux" as NodeJS.Platform,
		now: () => 1_000,
		randomId: () => "owner",
		pidAlive: (pid: number) => pid === launcherPid || pid === childPid,
		pidIncarnation: (pid: number) => `linux:${pid}`,
		spawn: (_command: string, args: string[]) => {
		...
		sleep: async () => {
			if (published) return;
			published = true;
			const ownerId = spawnedArgv[0]?.[spawnedArgv[0]!.indexOf("--owner-id") + 1];
			if (!ownerId) throw new Error("validation child did not receive an owner ID");
			expect(
				await renewDaemonHeartbeat({
					settings: s,
					ownerId,
	...
	await runDaemonInternal(["--agent-dir", agentDir, "--validation-test-supergroup-chat-id", validationChatId], {
		OwnerSettingsImpl: { init: async () => s },
		telegramDaemonDeps,
		processPid: launcherPid,
	});

	expect(spawnedArgv).toHaveLength(1);
	expect(spawnedArgv[0]!.filter(arg => arg === "--validation-test-supergroup-chat-id")).toHaveLength(1);
	expect(spawnedArgv[0]!).toContain(validationChatId);
	expect(spawnedArgv[0]!.filter(arg => arg === validationChatId)).toHaveLength(1);
	const state = await readDaemonState(s);
	expect(state).toMatchObject({
		pid: childPid,
		incarnation: `linux:${childPid}`,
		ownershipPhase: "ready",
		chatId: "42",
		tokenFingerprint: tokenFingerprint("123456:secret-token"),
	});
	expect(JSON.stringify(state)).not.toContain(validationChatId);

	await expect(
		runDaemonInternal(["--agent-dir", agentDir, "--validation-test-supergroup-chat-id", validationChatId], {
			OwnerSettingsImpl: { init: async () => s },
			telegramDaemonDeps,
			processPid: launcherPid,
		}),
	).rejects.toThrow("Telegram daemon is already owned; refusing validation attach");
	expect(spawnedArgv).toHaveLength(1);
});

test("runDaemonInternal keeps the private owner chat separate from its validation destination", async () => {
	const agentDir = tempAgentDir();
	const s = setPrivateAgentDir(settings(agentDir), agentDir);
	let capturedOptions: TelegramDaemonOptions | undefined;
	...
	class StubDaemon {
		...
		requestStop(_reason?: "reload" | "signal" | "stop"): void {}
		run(): Promise<void> {
			return new Promise<void>(resolve => {
				resolveRun = resolve;
			});
	...
	const run = runDaemonInternal(
		["--agent-dir", agentDir, "--owner-id", "owner", "--validation-test-supergroup-chat-id", "-100123"],
		{
			SettingsImpl: { init: async () => s },
			DaemonImpl: StubDaemon,
			loadInstallationHostId: async () => "validation-host",
			readDaemonState: async () => {
				readStateCount++;
				return { ownerId: "owner", heartbeatAt: 100 } as never;
			},
			setInterval: callback => {
	...
	for (let attempt = 0; attempt < 100 && !watchdogTick; attempt++) await Bun.sleep(1);
	expect(capturedOptions).toMatchObject({
		ownerId: "owner",
		chatId: "42",
		validationTestSupergroupChatId: "-100123",
		installationHostId: "validation-host",
	});
	watchdogTick!();
	await Bun.sleep(0);
	expect(readStateCount).toBe(1);
	resolveRun!();
	await run;
