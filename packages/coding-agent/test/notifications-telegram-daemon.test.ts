...
test("scanRoots reaps stale and dead-PID session topics after the orphan grace window", async () => {
	...
	const s = setPrivateAgentDir(settings(agentDir), agentDir);
	const cwd = path.join(agentDir, "repo");
	await registerNotificationRoot({ settings: s, cwd, sessionId: "stale", randomId: () => "lease-stale", now: () => 1000 });
	await registerNotificationRoot({ settings: s, cwd, sessionId: "dead", randomId: () => "lease-dead", now: () => 1000 });
	const endpointDir = path.join(cwd, ".gjc", "state", "notifications");
	fs.mkdirSync(endpointDir, { recursive: true });
	fs.writeFileSync(
		path.join(endpointDir, "stale.json"),
...
test("scanRoots reaps missing endpoint topics from a readable-empty or definitively-gone (ENOENT) registered root after grace, and compacts the reaped missing root", async () => {
	...
	const s = setPrivateAgentDir(settings(agentDir), agentDir);
	const cwd = path.join(agentDir, "repo");
	await registerNotificationRoot({ settings: s, cwd, sessionId: "missing", randomId: () => "lease-missing", now: () => 1000 });
...
describe("telegram daemon orphan reaping (two-phase candidate-then-reap)", () => {
	...
	test("repeated registration refreshes the lease and resets the orphan candidate grace clock", async () => {
		...
		await registerNotificationRoot({ settings: s, cwd, sessionId: "S", randomId: () => "lease-2", now: () => 130_000 });
	...
	test("only stale and dead-PID endpoints qualify; live, malformed, and active sessions are protected", async () => {
		...
		for (const sid of ["live", "malformed", "stale", "dead"]) {
			await registerNotificationRoot({ settings: s, cwd, sessionId: sid, randomId: () => `lease-${sid}`, now: () => 0 });
		}
		fs.writeFileSync(path.join(endpointDir, "live.json"), JSON.stringify({ url: "ws://live", token: "t", pid: 4242 }));
		fs.writeFileSync(path.join(endpointDir, "malformed.json"), JSON.stringify({ url: 123, token: "t" }));
		fs.writeFileSync(path.join(endpointDir, "stale.json"), JSON.stringify({ url: "ws://stale", token: "t", stale: true }));
		fs.writeFileSync(path.join(endpointDir, "dead.json"), JSON.stringify({ url: "ws://dead", token: "t", pid: 999999 }));
		fs.mkdirSync(daemonPaths(agentDir).dir, { recursive: true });
		fs.writeFileSync(
			path.join(daemonPaths(agentDir).dir, "telegram-topics.json"),
	...
	test("a live duplicate endpoint in another root protects the session over a stale reading", async () => {
		...
		fs.writeFileSync(path.join(rootA, "notifications", "dup.json"), JSON.stringify({ url: "ws://stale", token: "t", stale: true }));
		fs.writeFileSync(path.join(rootB, "notifications", "dup.json"), JSON.stringify({ url: "ws://live", token: "t", pid: 4242 }));
		fs.mkdirSync(daemonPaths(agentDir).dir, { recursive: true });
		fs.writeFileSync(
			path.join(daemonPaths(agentDir).dir, "telegram-topics.json"),
	...
	const malformedScalarRows: MalformedScalarRow[] = [
		...
		["negative candidate observedAt", { seedTopicId: "11", seedLeaseId: "L", patch: r => { r.orphanCandidates!.S!.observedAt = -1; } }],
		["fractional candidate observedAt", { seedTopicId: "11", seedLeaseId: "L", patch: r => { r.orphanCandidates!.S!.observedAt = 1.5; } }],
		["negative session-lease refreshedAt", { seedTopicId: "11", seedLeaseId: "L", patch: r => { r.sessionLeases!.S!.refreshedAt = -5; } }],
		["fractional session-lease refreshedAt", { seedTopicId: "11", seedLeaseId: "L", patch: r => { r.sessionLeases!.S!.refreshedAt = 2.5; } }],
	...
	test.each(malformedScalarRows)(
		"a malformed-scalar deletion candidate that would otherwise be the authority (%s) never matures into a delete",
		async (_label, { seedTopicId, seedLeaseId, patch }) => {
			const agentDir = tempAgentDir();
			const s = setPrivateAgentDir(settings(agentDir), agentDir);
	...
	test("registry compaction is selective: unrelated sessions, leases, and unknown fields survive", async () => {
		...
		fs.writeFileSync(path.join(rootDir, "notifications", "keep.json"), JSON.stringify({ url: "ws://keep", token: "t", pid: 4242 }));
		fs.writeFileSync(
			daemonPaths(agentDir).roots,
			JSON.stringify({
				version: 1,
	...
	test.each(malformedCreatedAtRows)(
		"a malformed persisted topic createdAt (%s) never satisfies orphan deletion grace even with a mature candidate",
