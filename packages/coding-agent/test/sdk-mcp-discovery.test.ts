...
describe("createAgentSession MCP discovery prompt gating", () => {
	...
	it("loads an explicit MCP config into the session catalog without server instructions", async () => {
		...
		try {
			...
			expect(mcpManager).toBeInstanceOf(MCPManager);
			expect(mcpManager?.isToolsOnly()).toBe(true);
			expect(MCPManager.instance()).not.toBe(mcpManager);
	...
	it("does not load an explicit MCP config in canonical sub-session shapes", async () => {
		const discoverAndConnect = vi
			.spyOn(MCPManager.prototype, "discoverAndConnect")
			.mockResolvedValue(createMcpLoadResult([createMcpCustomTool("mcp__exact_lookup", "exact", "lookup")]));
	...
	it("rejects an exact MCP manager in canonical sub-session shapes before startup side effects", async () => {
		const discoverAndConnect = vi
			.spyOn(MCPManager.prototype, "discoverAndConnect")
			.mockResolvedValue(createMcpLoadResult([createMcpCustomTool("mcp__exact_lookup", "exact", "lookup")]));
		const { session: topLevelSession, mcpManager } = await createAgentSession({
			...createIsolatedSessionOptions(),
			mcpConfigPath: path.join(tempDir, "explicit-mcp.json"),
		});

		try {
			if (!(mcpManager instanceof MCPManager)) throw new Error("Expected an exact MCP manager");
			expect(discoverAndConnect).toHaveBeenCalledTimes(1);
			discoverAndConnect.mockClear();

			const credentialSubscription = vi.spyOn(authStorage, "onCredentialDisabled");
			const getTools = vi.spyOn(mcpManager, "getTools");
			const connectServers = vi.spyOn(mcpManager, "connectServers");
	...
	it("fails soft with one generic warning when explicit MCP loading throws", async () => {
		const errorDetail = "sensitive startup detail";
		vi.spyOn(MCPManager.prototype, "discoverAndConnect").mockRejectedValue(new Error(errorDetail));
		const warning = vi.spyOn(logger, "warn").mockImplementation(() => {});
	...
	it("disconnects the manager owned by an explicit MCP config session, including startup errors", async () => {
		const ownedManagers: MCPManager[] = [];
		vi.spyOn(MCPManager.prototype, "setAuthStorage").mockImplementation(function (this: MCPManager) {
			ownedManagers.push(this);
		});
		...
		try {
			if (!(mcpManager instanceof MCPManager)) throw new Error("Expected an exact MCP manager");
			expect(mcpManager).toBe(ownedManagers[0]);
		} finally {
			await session.dispose();
		}
		expect(disconnectAll).toHaveBeenCalledTimes(1);
		expect(disconnectReceivers).toEqual([mcpManager]);
