...
import { parseInThreadConfigCommand, parseRichToggleCommand, parseTelegramControlCommand } from "../src/notifications/config-commands";
...
describe("parseTelegramControlCommand", () => {
	test("parses command roots and bot suffixes", () => {
		expect(parseTelegramControlCommand("/context@GajaeCodeBot", "GajaeCodeBot")).toEqual({ kind: "command", command: { name: "context" } });
		expect(parseTelegramControlCommand("/usage", "GajaeCodeBot")).toEqual({ kind: "command", command: { name: "usage" } });
	...
	test("parses reasoning status, cycle, and levels", () => {
		expect(parseTelegramControlCommand("/reasoning")).toEqual({ kind: "command", command: { name: "reasoning", action: "status" } });
		expect(parseTelegramControlCommand("/reasoning cycle")).toEqual({ kind: "command", command: { name: "reasoning", action: "cycle" } });
		expect(parseTelegramControlCommand("/reasoning HIGH")).toEqual({ kind: "command", command: { name: "reasoning", action: "set", level: "high" } });
	});
	...
	test("recognized invalid forms fail closed", () => {
		...
		expect(parseTelegramControlCommand("/reasoning enormous")).toMatchObject({ kind: "invalid", commandName: "reasoning" });
	});
