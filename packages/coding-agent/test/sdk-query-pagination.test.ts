...
describe("SDK query pagination", () => {
	it("returns the exact Q26 standard page in surface order and enforces installed-query authority", async () => {
		...
		const response = await query.dispatch({ query: "providers.list/active", id: "q26", connectionId: "c" });
		...
		const rejected = await notInstalled.dispatch({ query: "providers.list/active", id: "q26", connectionId: "c" });
	...
	it("keeps Q26 continuation order and snapshot contents after the surface mutates", async () => {
	...
	it("rejects Q10 and Q26 cursor reuse across query resources", async () => {
		const models = [
		...
		const q26WithQ10Cursor = await query.dispatch({
			query: "providers.list/active",
			cursor: q10First.page?.continuationCursor,
			connectionId: "c",
		});
		expect(q26WithQ10Cursor.error).toMatchObject({
			code: "invalid_input",
			message: "cursor does not match query",
		});

		const q26First = await query.dispatch({ query: "providers.list/active", connectionId: "c" });
		expect(q26First.page?.complete).toBe(false);
		const q10WithQ26Cursor = await query.dispatch({
			query: "Q10",
			cursor: q26First.page?.continuationCursor,
			connectionId: "c",
		});
		expect(q10WithQ26Cursor.error).toMatchObject({
			code: "invalid_input",
			message: "cursor does not match query",
		});
	});
