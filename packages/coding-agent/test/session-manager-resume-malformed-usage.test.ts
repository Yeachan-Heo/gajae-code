...
// `usage` fixtures intentionally carry invalid persisted shapes (missing fields, numeric
// strings, negatives), so it is typed `unknown` — the load path must tolerate any parseable JSON.
...
async function writeAndOpen(
	dir: string,
	assistants: Array<ReturnType<typeof assistantEntry>>,
