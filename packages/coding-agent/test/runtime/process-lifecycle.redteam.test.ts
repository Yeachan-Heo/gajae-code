...
`,
	...
	test("groupLeaderIdentityMatches returns false for undefined start time", () => {
		expect(groupLeaderIdentityMatches(undefined, { kind: "live", startTime: "100", ttyDevice: "0" })).toBe(false);
		expect(groupLeaderIdentityMatches(undefined, { kind: "absent" })).toBe(false);
	});
