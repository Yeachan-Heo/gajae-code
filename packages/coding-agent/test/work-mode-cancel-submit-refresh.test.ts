...
describe("Work Mode cancel-and-submit refresh failure settlement", () => {
	test("cancelAndSubmit settles a claimed token, restores queues, and permits a fresh direct claim", async () => {
		const { session } = await createFixture();
		await session.steer("queued steer");
		await session.followUp("queued follow-up");
		const queuesBefore = session.agent.snapshotQueues();
		const displaysBefore = session.getQueuedMessages();
		await stageTurn(session, "refresh-failure-cancel-submit");
		const runtimeBefore = snapshotRuntime(session);
		const refreshFailure = new Error("subskill refresh failed during cancel-and-submit");
		let refreshCalls = 0;
		vi.spyOn(session, "refreshGjcSubskillTools").mockImplementation(async () => {
			refreshCalls += 1;
			if (refreshCalls === 1) throw refreshFailure;
		});
		session.setCancelAndSubmitAbortOutcomeProviderForTests(async () => ({ kind: "settled" }));

		await expect(session.cancelAndSubmit("send now")).resolves.toEqual({
			kind: "rolled_back",
			outcome: { kind: "error", cause: refreshFailure },
		});
		...
		expect(session.agent.snapshotQueues()).toEqual(queuesBefore);
		expect(session.getQueuedMessages()).toEqual(displaysBefore);
		expect(snapshotRuntime(session)).toEqual(runtimeBefore);

		await stageTurn(session, "refresh-failure-cancel-submit-restaged");
		await expect(session.cancelAndSubmit("next direct request")).resolves.toEqual({ kind: "submitted" });
