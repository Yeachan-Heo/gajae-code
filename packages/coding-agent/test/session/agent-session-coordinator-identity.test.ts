...
describe("AgentSession coordinator runtime-state identity rotation", () => {
	it("rotates /new and switch identities only after commit, replaces its finalizer, and rolls back without marker drift", async () => {
		...
		try {
			...
			// Rotation is fire-and-forget after the commit, so wait for the marker to
			// actually name the successor rather than for the first write of any kind
			// (the finalizer re-registration can win that race).
			await waitFor(
				async () =>
					((JSON.parse(await Bun.file(stateFile).text()) as Record<string, unknown>).session_file ?? null) ===
					successorFile,
				"authorized switch rotation",
			);
		} finally {
			write.mockRestore();
		}
		const afterSwitch = JSON.parse(await Bun.file(stateFile).text()) as Record<string, unknown>;
		expect(afterSwitch.session_file).toBe(successorFile);
		// The successor marker must be FRESH, not the predecessor payload relabeled:
		// a predecessor `completed`/`errored` state would otherwise falsely terminalize
		// the successor and suppress its real postmortem.
		expect(afterSwitch.state).toBe("running");
		expect(afterSwitch.ended_at).toBeUndefined();
		expect(afterSwitch.final_response).toBeUndefined();
