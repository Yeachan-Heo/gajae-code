...
describe("WelcomeComponent viewport sizing", () => {
	...
	it("renders the converted color image logo by default", () => {
		const welcome = new WelcomeComponent("1.2.3", "test-model", "test-provider", [], [], "unicode", {
			getViewportRows: () => 42,
			getReservedBottomRows: () => 4,
		});
		const lines = welcome.render(160);
		const text = lines.map(stripRenderControls).join("\n");
	...
	it("animates the converted image logo during the intro", () => {
		const welcome = new WelcomeComponent("1.2.3", "test-model", "test-provider", [], [], "unicode", {
			getViewportRows: () => 36,
			getReservedBottomRows: () => 4,
		});
		const restingFrame = welcome.render(140).join("\n");
		welcome.playIntro(() => {});
		const animatedFrame = welcome.render(140).join("\n");
		welcome.dispose();

		expect(animatedFrame).not.toBe(restingFrame);
	});
