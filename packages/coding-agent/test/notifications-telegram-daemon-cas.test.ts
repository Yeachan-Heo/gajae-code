...
test("Windows CAS preserves a destination successor substituted after generation validation", async () => {
	...
	const fsImpl: TelegramDaemonFs = {
		...
		readFile: async (target, encoding) => {
			if (target === file && ++destinationReads === 2) {
				fs.unlinkSync(file);
				fs.writeFileSync(file, successor);
			}
			return await fs.promises.readFile(target, encoding);
		},
		fsyncFile: async () => {},
	};
	const registry = new FilesystemTopicRegistryCasAuthority(file, {
		...
		fs: fsImpl,
		platform: "win32",
		exactReplace: (_source, destination, _expectedSource, expectedDestination) => {
			const actual = fs.lstatSync(destination, { bigint: true });
			expect(actual.ino).not.toBe(expectedDestination.ino);
