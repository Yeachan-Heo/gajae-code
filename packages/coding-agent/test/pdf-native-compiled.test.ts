...
describe("compiled native PDF conversion", () => {
	it("converts a PDF through the embedded native addon in a compiled binary", async () => {
		...
		try {
			...
			if (entryIndex < 0 || outputIndex <= entryIndex) throw new Error("could not locate the compiled entrypoint list");
			compileArgs.splice(entryIndex, outputIndex - entryIndex, "./test/fixtures/pdf-native-compiled-convert-entry.ts");
			const compiled = await run([process.execPath, ...compileArgs.slice(1)], packageRoot);
			expect({ exitCode: compiled.exitCode, stderr: compiled.stderr.slice(0, 2000) }).toEqual({ exitCode: 0, stderr: "" });
