...
// The missing-model rejection must be produced by the REAL session preflight and
// travel the REAL production control surface (session-runtime `createControlSurface`
// -> `submit` -> `InvocationReconciliation`), so this fixture wires the notifications
// extension's `sendUserMessage` to a live no-model `AgentSession` exactly the way
// production does in `modes/runtime-init.ts`, with `forwardPreflightCallbacks`
// keeping the preflight callbacks, preflight signal and SDK run capability intact.
const MISSING_MODEL_CODE = "model_not_selected";
const MISSING_MODEL_PUBLIC_MESSAGE =
	"No model is selected for this session. Select a model before submitting a prompt.";
// `createInvocationReconciliation` bounds live prompt admissions at 256 with no
// public seam; an anonymous submission has no clientRef to re-probe, so exceeding
// that bound is the observable proof its reservation was released.
const PROMPT_ADMISSION_CAPACITY = 256;

test("a missing-model preflight is rejected, leaks nothing, and releases its admission through the production control surface", async () => {
	const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "gjc-sdk-missing-model-vertical-"));
	dirs.push(cwd);
	const sessionId = `sdk-missing-model-vertical-${Date.now()}`;
	let aborts = 0;
	const sessionContext = {
		...context(cwd, sessionId),
		abort: async () => {
	...
	const agent = new Agent({
		...
		streamFn: (model, agentContext, streamOptions) => {
			streamCalls++;
			return mock.stream(model, agentContext, streamOptions);
		},
	});
	const sessionManager = SessionManager.inMemory(cwd);
	...
	const handlers = start(
		...
		async (content, options) => {
			...
			return await session.sendUserMessage(content, options);
		},
		true,
	);
	try {
		...
		const request = async (frame: Record<string, unknown>) => {
			...
			const settled = () =>
				frames.find(
					candidate =>
						(candidate.type === "control_response" || candidate.type === "query_response") &&
						candidate.id === frame.id,
				);
			await waitFor(() => settled() !== undefined, `${String(frame.id)} response`);
			return settled()!;
		};

		// 1. turn.prompt: the live producer's typed failure is classified publicly,
		// with no result and therefore no correlation or clientRef echo.
		const rejected = await request({
			type: "control_request",
			id: "missing-model-1",
			operation: "turn.prompt",
			input: { text: "vertical missing model", clientRef: "missing-model-ref" },
		});
		...
		const publicText = JSON.stringify(rejected);
		for (const leak of ["commandId", "turnId", "accepted", "gjc setup provider", "--api-key-env", "/login"])
			expect(publicText).not.toContain(leak);
		expect(produced).toEqual(["vertical missing model"]);

		// 2. No durable record was published for the rejected submission.
		const status = await request({
			type: "query_request",
			id: "missing-model-status",
			query: "turn.prompt_status",
			input: { clientRef: "missing-model-ref" },
		});
		expect(status).toMatchObject({ ok: true, result: { status: "unknown" } });

		// 3. `unknown` alone cannot distinguish a released reservation from a retained
		// one, so re-drive the SAME clientRef on a fresh request: the reservation was
		// released iff this reaches the preflight again instead of client_ref_conflict.
		const sameRefRetry = await request({
			type: "control_request",
			id: "missing-model-2",
			operation: "turn.prompt",
			input: { text: "same ref again", clientRef: "missing-model-ref" },
		});
		expect(sameRefRetry).toMatchObject({
			ok: false,
			error: { code: MISSING_MODEL_CODE, message: MISSING_MODEL_PUBLIC_MESSAGE },
		});

		// 4. turn.abort_and_prompt carries no clientRef in its production contract; it
		// runs the real abort prelude and classifies identically.
		const abortAndPrompt = await request({
			type: "control_request",
			id: "missing-model-abort-and-prompt",
			operation: "turn.abort_and_prompt",
			input: { text: "abort then missing model" },
		});
		expect(abortAndPrompt).toMatchObject({
			ok: false,
			error: { code: MISSING_MODEL_CODE, message: MISSING_MODEL_PUBLIC_MESSAGE },
		});
		...
		// admitted and actually executes, which only holds if its reservation and its
		// retained-ref slot were both released.
		agent.setModel(mock.model);
		const recovered = await request({
			type: "control_request",
			id: "missing-model-recovered",
			operation: "turn.prompt",
			input: { text: "model restored", clientRef: "missing-model-ref" },
		});
		expect(recovered).toMatchObject({
			ok: true,
			result: {
				accepted: true,
				clientRef: "missing-model-ref",
