...
describe("Settings", () => {
	...
	describe("model role overrides", () => {
		...
		it("keeps saves suppressed while a checkpoint baseline is being acquired", async () => {
			await writeSettings({
				task: { agentModelOverrides: { executor: "original/executor" } },
			});
			const settings = await Settings.init({ cwd: projectDir, agentDir });
			...
			settings.setAgentModelOverride("planner", "during-acquisition/planner");
			await Bun.sleep(150);
			...
			expect((await readSettings()).task).toEqual({
				agentModelOverrides: {
					...
					planner: "during-acquisition/planner",
				},
			});
		});
