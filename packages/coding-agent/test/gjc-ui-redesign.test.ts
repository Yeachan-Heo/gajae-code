...
describe("GJC red-claw redesign defaults", () => {
	...
	it("uses red-claw as the default dark theme and red-claw-light as the default light theme", async () => {
		...
		expect(SETTINGS_SCHEMA["theme.light"].default).toBe("red-claw-light");
		expect(themeModule.getCurrentThemeName()).toBe("red-claw");

		themeModule.onTerminalAppearanceChange("light");
		await themeModule.initTheme(false);
		expect(themeModule.getCurrentThemeName()).toBe("red-claw-light");
	});
	...
	it("exposes all bundled themes while preserving the red-claw and red-claw-light defaults", async () => {
