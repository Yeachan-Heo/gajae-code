...
- Fixed a tool block being rendered two or three times in the transcript (a pending `⏳` copy stranded above its own completed `✓` copy, with the rows between duplicated). When a block above the live viewport top grows in place — the bash tool's compact call render becoming a partial box and then a final box, with the editor/status chrome keeping it off-screen — the "commit only the changed visible suffix" path emitted rows by index even though an off-screen insertion had displaced committed content down across the native-scrollback frontier, so rows already in scrollback were appended a second time under their new content. That path now searches for the displacement first: if the previously visible rows reappear almost intact at some uniform offset below their old index, they were pushed across the frontier and the live viewport is repainted instead. Rows merely rewritten in place produce no such run at any offset, so an off-screen status line changing, or the whole visible region being re-rendered, still commits its appended rows to scrollback as before. Neither a single boundary row nor an aggregate match count can make this distinction, because rendered bytes carry no row identity: repeated separators and blank rows let an insertion move everything without changing the rows an index-wise comparison looks at.
