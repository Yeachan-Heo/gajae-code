...
function publishAgentEnd(
	...
	const publishedEvent: Extract<AgentEvent, { type: "agent_end" }> =
		event.stopReason === "maintenance" && event.maintenanceOutcome === "aborted"
			? { type: "agent_end", messages: event.messages, stopReason: "cancelled" }
			: event;
	stream.push(publishedEvent);
	const standalone = config.standaloneRunOwnership
		? standaloneOwnershipStates.get(config.standaloneRunOwnership)
		: undefined;
	if (publishedEvent.stopReason === "maintenance") {
		if (standalone) {
