...
- Cursor provider-executed tool calls now carry a strict `cursor-exec` provenance marker and their provider outcome. The agent loop records the real provider output or error, skips local validation/execution hooks, and emits exactly one paired result without re-running side effects. Unflagged calls, including Cursor's MCP and `todo_write` calls, still execute as before.
- The `streamProxy` wire protocol now relays the `cursor-exec` marker and final arguments/results, validating them at reconstruction so native calls remain attributable and cannot be re-run after proxy transport.
