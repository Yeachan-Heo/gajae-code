...
- Managed snapshot now degrades benign provider variations (unknown `done`/`error` reasons, top-level proxy-wrapped events, unknown event `type`) to a neutral completion instead of throwing `ManagedAttemptSnapshotError` and ending the run. See `HANDOFF-managed-snapshot.md`.
