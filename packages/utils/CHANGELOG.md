...
- The crash-log credential scrubber recognizes GitHub fine-grained PATs (`github_pat_`) and AWS temporary access key ids (`ASIA`). It already had rules for both vendors, but matched only the classic `gh[opsur]_` and long-term `AKIA` shapes, so the current formats survived into a file the module keeps indefinitely.
