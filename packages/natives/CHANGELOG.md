...
- Managed native appends now withhold retry-authorizing receipts when concurrent O_APPEND bytes make the post-write content differ from the expected predecessor plus the operation data.
- Managed native appends now retain descriptor-bound successor receipts when file or parent-directory fsync is not provable, including proven partial writes, allowing the session layer to recover without poisoning subsequent persistence.
