...
- OpenAI Codex GPT-5.6 Sol credential selection no longer treats Plus or unknown ChatGPT usage `planType` labels as authoritative entitlement denials. Those accounts may reach the provider, confirmed Pro accounts remain preferred, and provider-confirmed HTTP or streaming failures retain actionable guidance. Confirmed Free candidates never dispatch Sol, and an all-Free OAuth pool fails locally.
...
- The OpenAI Codex GPT-5.6 Sol entitlement preflight no longer rejects Business, Enterprise, Team, or Plus ChatGPT OAuth accounts. Eligibility was decided by `plan_type.includes("pro")`, so a `business`/`enterprise`/`team` plan (which is entitled to Sol on the backend) failed credential selection before dispatch with a spurious "cannot use model" error, while Plus was treated as authoritatively denied despite variable live access. A shared classifier now recognizes exact Pro/Business/Enterprise/Team tiers as entitled, excludes every confirmed Free candidate from Sol dispatch and fails an all-Free pool locally, ranks Plus below unknown tiers without rejecting it, and defers Plus, missing, or unfamiliar plan access to the provider.
