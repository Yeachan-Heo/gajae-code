...
- Fixed Anthropic unified-limit 429s being declared terminally exhausted while the account still had fallback capacity. A unified rejection is rolling/probabilistic when `anthropic-ratelimit-unified-fallback-percentage > 0` — Anthropic keeps accepting a fraction of requests, and field evidence shows recovery within minutes of a rejection that advertised a 3.7h reset — but the exhaustion fast-path declared the session terminal after a single attempt. Such rejections are now downgraded to an explicit bounded retry, gated on every dial of the captured incident shape at once: a rejected unified status (generic `anthropic-ratelimit-unified-status` or any window-scoped `…-5h/7d-status` variant), a strictly parsed complete fallback fraction within (0, 1], the exact JSON `rate_limit_error` + "request would exceed" error class (non-JSON bodies fail closed), and an OVERSIZED retry hint (`retry-after-ms` precedence, then seconds/HTTP-date; small or absent hints keep their original handling). The retry is performed by the fetch wrapper itself — an abort-aware wait (`scheduler.wait` with the caller signal, so cancellation interrupts the delay promptly) followed by a re-fetch — so the SDK never sees the downgraded 429, no `Response` is reconstructed, and the SDK retry budget is untouched. The downgrade budget is a keyed sliding window (`SlidingWindowBudget`, monotonic `performance.now` clock, expired identities pruned): the key is the request origin plus a non-secret truncated sha256 of the credential header, so one credential/endpoint can never consume another's allowance while sequential and concurrent requests on the same identity stay bounded to 2 downgrades per 60s. `out_of_credits` keeps failing fast — no fallback fraction saves missing credits.
