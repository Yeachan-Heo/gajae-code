...
- Fixed Anthropic unified-limit 429s being declared terminally exhausted while the account still had fallback capacity. A unified 5h rejection is rolling/probabilistic when `anthropic-ratelimit-unified-fallback-percentage > 0` — Anthropic keeps accepting a fraction of requests, and field evidence shows recovery within minutes of a rejection that advertised a 3.7h reset — but the exhaustion fast-path declared the session terminal after a single attempt. Such rejections are now downgraded to an explicit bounded retry, gated on the FULLY verified unified-rejection shape: `anthropic-ratelimit-unified-status=rejected`, a strictly parsed complete fallback fraction within (0, 1] (numeric prefixes, out-of-range, and non-finite values are rejected), and the exact `rate_limit_error` + "request would exceed" error class without `out_of_credits`. The downgrade rewrites only the retry hint (`retry-after-ms=2000`; the SDK obeys `retry-after` verbatim, so passthrough would sleep for hours) while passing the original body stream through untouched, and is budgeted per client instance (at most 2 downgrades per 60s sliding window) so it can never become an unbounded burst nor be triggered by a lone spoofed header on an arbitrary 429. `out_of_credits` keeps failing fast — no fallback fraction saves missing credits.
