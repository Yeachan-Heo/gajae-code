...
- Anthropic-compatible and Codex stream handlers now coerce non-string `text`/`thinking`/`partial_json`/`delta` increments to an empty string before emitting `*_delta` events, so a Z.AI or Codex thinking-delta that arrives as `undefined` or a numeric token count cannot produce a non-string `delta` that managed snapshot staging rejects as `event.delta`. Object-shaped payloads are still dropped at the increment (they become `""`) rather than forwarded as live objects. Anthropic `signature_delta` likewise appends only string signatures, so a numeric or missing signature cannot pollute `thinkingSignature` as `"1"` / `"[object Object]"`.
