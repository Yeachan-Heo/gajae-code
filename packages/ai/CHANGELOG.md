...
- Stored command-backed API keys that resolve empty now fall through to another usable API key or OAuth credential instead of suppressing it. AuthStorage also exposes the currently selectable credential type from cached usability, backoff, expiry, environment, and session evidence so provider ordering matches the credential request dispatch can actually use.
