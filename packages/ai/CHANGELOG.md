...
- Closed the remaining `Request blocked (code=invalid_prompt)` wedge on gpt-5.6 caused by header-form leaked Harmony markers. The reserved-control-token sanitizer only matched the simple `<|ident|>` shape, so a header-form marker carrying a recipient (e.g. `<|assistant to=functions.bash|>`) survived every sanitizer path (replay, request boundary, compaction) and kept re-poisoning history even after the earlier fixes. The pattern now also matches the header grammar (`<|role key=value ...|>`) with an unbounded recipient length, while leaving ordinary pipe/operator syntax untouched (F# `value <| f |> g`, compact `sum<|a+b|>c`, and multi-line bodies never match). Strict superset of the prior identifier-only pattern (#2267).
