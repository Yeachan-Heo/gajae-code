...
- Fixed Anthropic unified-limit 429s being declared terminally exhausted while the account still had fallback capacity. `isAnthropicUsageExhaustionResponse` treated any `retry-after` above the retry-delay cap (and any "request would exceed your account's rate limit" body) as exhaustion-until-reset, but a unified 5h rejection with `anthropic-ratelimit-unified-fallback-percentage > 0` is rolling/probabilistic — Anthropic keeps accepting a fraction of requests, and field evidence shows recovery within minutes of a rejection that advertised a 3.7h reset. Such rejections now get a bounded retry: the fetch wrapper strips the worst-case `retry-after`/`retry-after-ms` hint (the SDK obeys retry-after verbatim, so plain passthrough would sleep for hours) and lets the SDK spend its default exponential backoff budget (0.5s..8s). `out_of_credits` keeps failing fast — no fallback fraction saves missing credits.
