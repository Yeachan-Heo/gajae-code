...
- Anthropic-compatible endpoints now default to no generated cache controls, regardless of Claude-family model id. Canonical `api.anthropic.com` retains top-level automatic caching; non-canonical endpoints must explicitly set `compat.promptCacheMode: automatic` or `explicit`, or pass a non-`none` request-time `cacheRetention` for a one-request automatic opt-in. This prevents proxies that already send block-level markers from receiving a fifth generated top-level `cache_control` and rejecting the request with HTTP 400 (`A maximum of 4 blocks with cache_control may be provided. Found 5.`). `compat.promptCacheMode: automatic` continues to support 1-hour retention with `compat.supportsLongCacheRetention: true`; `cacheRetention: none` and request-over-model retention precedence are unchanged.
