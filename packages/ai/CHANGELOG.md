...
- GLM ZCode OAuth now refreshes its authenticated `/v1/models` catalog automatically, so newly available GLM models are selectable without waiting for the bundled catalog to catch up. Discovery preserves bundled model metadata and sends the ZCode client headers required by the Z.AI endpoint.
...
- The Anthropic CPA Claude-OAuth "cannot restore tool alias" corrective repair (#4338) fired only when zero content blocks had streamed (`firstTokenTime === undefined`), but the rejected `tool_use` block's own `content_block_start` event sets `firstTokenTime` before the CPA failure can be classified, so in real traffic the repair almost never fired: the raw upstream proxy JSON (`cannot restore Claude OAuth MCP tool alias "...": no unique request-local match`) surfaced verbatim to the user instead of an actionable message. Widening repair eligibility to cover this case was evaluated and rejected for unmanaged streams: `streamAnthropic` already publishes a live `toolcall_start` event to stream consumers the instant the block starts, before the CPA failure is known, and `resetOutputForRetry()` cannot retract an already-delivered event — a resend would leak an orphaned/duplicate tool-call lifecycle event. Unmanaged streams therefore get an actionable, statusless terminal error ("repair could not be safely attempted because response content had already begun streaming to the client") whenever a CPA failure is classified after any response content (text, thinking, redacted-thinking, or tool_use) has streamed, with no resend attempted. Under `fallbackManaged` the attempt's events are staged in a `ManagedAttemptTransaction` that never published them, so a controller resend is replay-safe: the ineligible branch records the corrective steering against the turn (when shared session state exists) and rethrows the raw failure with its HTTP 500 transport facts intact, letting the managed fallback controller discard the attempt and rebuild the turn with steering; without shared session state it keeps the raw-throw pass-through, and a steered attempt that fails again mid-stream terminalizes with the actionable ineligible error. The existing zero-content repair-and-retry path, the generic 5xx retry gate, and the other six `firstTokenTime`-gated repair branches are unchanged. `createCpaToolAliasTerminalError` gains a required `reason: "recurrence" | "ineligible"` discriminator so the two distinct terminal messages are never conflated. `resolveCpaCallableToolName` now normalizes `proxy_`-prefixed OAuth wire tool names via `stripClaudeToolPrefix`, so the terminal error names the unique callable tool instead of always falling back to discovery guidance (#4338, PR #4790 fix-forward).
