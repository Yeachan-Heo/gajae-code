...
- OpenAI Responses session-affinity headers now reach explicitly opted-in custom OpenAI relays while remaining default-off for arbitrary endpoints and excluded from GitHub Copilot and all non-OpenAI providers. Explicit caller headers continue to take precedence, and affinity identity remains stable across repeated requests and provider-session-state replay (#3689).
