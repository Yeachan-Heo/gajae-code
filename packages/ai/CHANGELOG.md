...
- Codex credential selection preserves OAuth fallback and refreshes usage after credential authority changes. Fresh usage requests propagate through the auth broker and its per-credential cache; Sol entitlement remains provider-authoritative, while Spark retains its confirmed-Pro filter.
