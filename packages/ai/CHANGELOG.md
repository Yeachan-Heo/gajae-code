...
- Kiro now confines OAuth bearer transmission to the canonical HTTPS runtime origin, propagates caller cancellation and refresh deadlines into token fetches, and ignores late rotating-token responses after cancellation. OAuth/device-poll/event-stream failures remain bounded and redacted.
...
- Kiro's first-class fallback catalog now exposes the 18 validated text models with provider-specific thinking levels and a 32k output cap. DeepSeek 3.2 receives a private non-executable dummy tool only when no real tools are available, preserving its wire protocol without exposing a callable agent tool.
...
- Kiro streaming now preserves an established `MAX_TOKENS` or `TOOL_USE` reason when later metadata is absent or reports `END_TURN`, keeps one conversation id across provider-session turns, emits each matched tool result exactly once, downgrades an orphan result to one untrusted stale user message, keeps system instructions on the initial user message during continuations, and rejects images in tool results before shared message repair can discard them. The transport now also honors request-scoped fetch, payload/response, header, retry-budget, and retry-delay-cap hooks with accurate AWS attempt headers; Kiro's required authorization and target headers remain authoritative over caller additions.
- Kiro social refresh now keeps legacy credentials usable when the response omits `profileArn`, rejects account switching, applies a safe default for zero or absent TTLs, and uses the status-only HTTP error contract. A 401/403 can force-refresh the matching social credential even when its local expiry is still hours away, covering peer-client refresh invalidation. Device registration persistence now fails the login immediately instead of reporting success that cannot refresh later, waits briefly for concurrent SQLite writers, and request credentials no longer carry the unused refresh token.
- Kiro model discovery now deduplicates model ids and detects reasoning from top-level schema properties instead of matching words inside serialized prose. Multi-account usage scopes carry the credential-store row id when no server account id exists.
...
- Kiro no longer sends the reverse-engineered `x-amzn-codewhisperer-optout: false` header on every control-plane, discovery, and runtime request. The header's polarity is undocumented, so GJC no longer asserts a data-sharing preference on the user's behalf: the header is omitted unless `GJC_KIRO_CODEWHISPERER_OPTOUT` explicitly supplies a value.
- Kiro removed the fire-and-forget `client-telemetry.us-east-1.amazonaws.com` request that fired on the first streamed event. It was wrapped in a `try`/`catch` that could not catch its own rejection (a `void fetch(...)` rejects asynchronously), and nothing consumed the response.
- Kiro HTTP and protocol failures no longer persist upstream details in session transcripts. Non-2xx responses are reduced to the status code without consuming the body; AWS event-stream exception payloads and HTTP-200 Builder ID token errors now use fixed summaries instead of forwarding account ids, profile ARNs, request ids, or attacker-controlled text.
- Kiro models no longer advertise image input. The `userInputMessage` wire format is text-only, and the previous code silently replaced every image with an `[Image: ...]` placeholder, so attachments produced a reply that never saw them. The static seed and dynamic discovery now both declare `input: ["text"]`, and image content reaching the transport throws instead of being silently dropped.
- Kiro static seed models now declare `contextWindow: 200_000`, matching the discovery fallback instead of an unverified `1_000_000`, so overflow management no longer trusts a window the server can reject.
- The auth-broker wire schema again requires a non-empty `kiroProfileArn`, matching every producer, which already rejects empty values.
- Kiro profile-ARN memoization now keys on a SHA-256 digest of the access token instead of the raw bearer token, so the secret no longer sits in a long-lived map (or in heap/core dumps) while preserving per-token isolation.
- Kiro Builder ID credentials persist the shared CodeWhisperer profile required by Builder ID tokens, which cannot call `ListAvailableProfiles`; legacy Builder ID credentials with an empty slot are backfilled from the same constant. Social credentials continue to use their server-confirmed account profile.
- Kiro profile routing follows one method-aware invariant across streaming, model discovery, usage, and login: a stored social `kiroProfileArn` is authoritative, a missing social slot is resolved from the access token, and Builder ID uses its shared profile without attempting unsupported discovery.
- Kiro tool-use ids issued by Kiro now replay verbatim instead of being re-hashed. Hashing was applied unconditionally, so a multi-turn tool exchange sent back ids the server never issued; ids that do not already satisfy Kiro's `[A-Za-z0-9_-]{1,64}` shape (for example replayed 100-character OpenAI Responses ids) are still normalized.
- Kiro's device-registration table (`kiro_kv`) is now declared by the credential store's schema init instead of created ad hoc on first write, and the read path probes for the table first. A missing table reads as a cold start while an unreadable database is reported distinctly, so a damaged database no longer drives unbounded SSO-OIDC re-registration.
- Kiro Builder ID token refresh now returns its routing fields (`kiroMethod`, `kiroProfileArn`) explicitly, matching the social refresh path, instead of relying on the caller to merge them back in.
- Kiro's steady-state stream watchdog now uses a five-minute idle floor instead of the shared 120s default, matching the first-event floor it already had. Its reasoning tiers go silent between tokens while planning a tool call, so the watchdog aborted healthy mid-response streams with `Provider stream stalled while waiting for the next event` — and that abort only auto-retries while the turn is still replay-safe. `PI_STREAM_IDLE_TIMEOUT_MS` still overrides it.
