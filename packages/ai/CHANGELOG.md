...
- Restored the OpenAI-specific idle-timeout alias precedence (`GJC_OPENAI_STREAM_IDLE_TIMEOUT_MS` → `PI_OPENAI_STREAM_IDLE_TIMEOUT_MS` → `PI_STREAM_IDLE_TIMEOUT_MS`) that #4797 reordered; setting both the OpenAI-specific and generic legacy aliases now keeps the OpenAI-specific value instead of the generic one.
