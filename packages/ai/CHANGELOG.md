...
- Codex stale-continuation recovery now classifies anchor rejections by message as well as by error code, so a provider event shaped `{"type":"error","error":{"type":"invalid_request_error","code":"invalid_request_error","message":"Invalid `previous_response_id`."}}` clears `previous_response_id` and the websocket append state and retries once with full conversation context instead of terminating the active session (#4752). `invalid_request_error` stays in the non-retryable code set — a generic invalid-request failure that does not implicate a continuation anchor remains fatal, so only anchor rejections gain the retry. Previously only the `previous_response_not_found` and `codex_previous_response_stale` codes (#731) were recognized, and this shape reached users as a fatal `Codex error event: Invalid \`previous_response_id\`. (code=invalid_request_error)`.
