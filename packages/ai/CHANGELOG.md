...
- Cursor HTTP/2 streams now drain admitted exec responses before normal teardown and centralize terminal failures, preventing delayed handlers and retained shell callbacks from writing after the request has ended. The coordinator write path skips a closed Connect stream, contains Bun's promise-returning `write after end` / `ERR_STREAM_DESTROYED`, and waits for `drain` after backpressure before sending the next frame.
