...
- ChatGPT Codex Sol credential selection no longer denies `gpt-5.6-sol` from a Plus or unknown account plan label. A Plus-labelled OAuth account was rejected locally before any request, but the provider accepts that account and returns a normal `200`; trial, grandfathered, and experiment-enabled accounts can also carry ordinary or unknown labels. Freshly confirmed Free candidates remain locally excluded, confirmed Pro candidates remain preferred, and Plus or unknown candidates reach the provider when eligible for fallback. Spark keeps its existing confirmed-Pro filtering when a Pro account is present. Provider entitlement failures retain the same actionable guidance (#5270).
...
- Codex credential selection preserves OAuth fallback and refreshes usage after credential authority changes. Fresh usage requests propagate through the auth broker and its per-credential cache; Sol excludes freshly confirmed Free candidates locally while deferring Plus and unknown plans to the provider, and Spark retains its confirmed-Pro filter.
