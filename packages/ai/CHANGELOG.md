...
- Anthropic `input_json_delta` and Codex `function_call`/`custom_tool_call` argument increments now fail the turn closed when the increment is object- or function-shaped, instead of coercing the live value to `""` and continuing with missing or default tool arguments. Primitive thinking/text/signature/argument anomalies still degrade to an empty string, and each stream emits at most one payload-free diagnostic per increment type naming only the envelope (`deltaType`/`eventType` and `receivedType`).
- Anthropic-compatible and Codex stream handlers now coerce non-string `text`/`thinking`/`partial_json`/`delta` increments to an empty string before emitting `*_delta` events, so a Z.AI or Codex thinking-delta that arrives as `undefined` or a numeric token count cannot produce a non-string `delta` that managed snapshot staging rejects as `event.delta`. Object-shaped thinking/text increments are still dropped at the increment (they become `""`) rather than forwarded as live objects; tool-argument increments of that shape fail closed instead. Anthropic `signature_delta` likewise appends only string signatures, so a numeric or missing signature cannot pollute `thinkingSignature` as `"1"` / `"[object Object]"`.
