...
- Regenerating the catalog no longer replaces a previously generated `cloudflare-ai-gateway` row with the hardcoded Cloudflare stand-in, which used to overwrite models.dev-backed limits (1M → 200k) and the row name whenever discovery had no Cloudflare credentials. The stand-in is now only synthesized when neither the live sources nor the previous catalog carry that provider, which also makes credential-less regeneration a fixed point of the committed catalog (asserted by `generate-models-composition.test.ts`).
- Corrected the bundled `opengateway/anthropic/claude-sonnet-4-5` thinking range to the value the generator derives (`maxLevel: "xhigh"`, was a stale `"high"`) and restored provider key sorting for `opengateway`; both were pre-existing divergences between the committed catalog and its own generator output.
