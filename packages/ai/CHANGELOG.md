...
- Kiro HTTP failures no longer interpolate the upstream response body into the thrown error. AWS envelopes carry account ids, profile ARNs, and request ids, and the message is persisted to session transcripts; only the status code is surfaced. The `__type` discriminator is dropped too — it is attacker-controlled text on a failure path — so the body is never read.
