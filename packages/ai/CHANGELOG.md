...
- Cursor native tool calls (`shellToolCall`, `globToolCall`, `grepToolCall`, `lsToolCall`, `readLintsToolCall`, …) rendered into assistant content now carry the strict `cursor-exec` provenance marker plus the provider's success/error outcome, because Cursor already executes them during streaming over its exec channel. Without the marker the agent loop treated the rendered block as a model-issued call and dispatched it a second time (`shellToolCall{command}` ran the command twice), or aborted the turn with `Tool glob not found` for native kinds whose display label is not a registered tool. MCP and `todo_write` calls are unaffected and still dispatch normally.
