...
function assertLooseAskSchema(parameters: unknown): void {
	...
	if (!question) throw new Error("OpenAI Codex Responses payload omitted AskTool question schema");
...
describe("OpenAI Codex responses tool choice capability", () => {
	...
	it("keeps the non-strict ask question metadata optional on the final wire payload", async () => {
		...
		if (!isObject(ask)) throw new Error("OpenAI Codex Responses payload omitted AskTool");
