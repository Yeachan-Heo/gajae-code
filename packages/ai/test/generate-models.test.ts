...
describe("injectFirstPartyCatalogSeeds", () => {
	it("carries archived, digest-bound sources for every seed", () => {
		...
		for (const seed of FIRST_PARTY_CATALOG_SEEDS) {
			...
			for (const source of seed.sources) {
				...
				expect(source.artifact).toMatch(/^artifacts\//);
	...
	it("binds provenance into every injected row", () => {
		...
		for (const seed of FIRST_PARTY_CATALOG_SEEDS) {
			...
			expect(injected?.catalogProvenance).toEqual({ sources: seed.sources.map(source => ({ ...source })) });
		}
	});
