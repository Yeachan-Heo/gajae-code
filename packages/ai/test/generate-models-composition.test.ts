...
describe("composeCatalog seeded surface", () => {
	...
	it("binds seed provenance into the emitted row", () => {
		...
		for (const seed of FIRST_PARTY_CATALOG_SEEDS) {
			const row = composedRow(catalog, `${seed.model.provider}/${seed.model.id}`);
			expect(row?.catalogProvenance).toEqual({ sources: seed.sources.map(source => ({ ...source })) });
		}
	});
	...
	it("keeps the entire committed catalog stable through a full recompose", () => {
		// Whole-output guard, not just the seeded keys: recomposing the committed
		// catalog with no live sources must reproduce it exactly, so any collateral
		// change anywhere in the catalog — a policy tweak, a seed edit, a stale row —
		// fails here instead of slipping into a future regeneration diff.
		const catalog = composeCatalog({
			liveModels: [],
			modelsDevModels: [],
		...
		expect(Object.keys(catalog)).toEqual(Object.keys(bundledCatalog));
		for (const provider of Object.keys(bundledCatalog)) {
			expect(Object.keys(catalog[provider] ?? {})).toEqual(Object.keys(bundledCatalog[provider] ?? {}));
		}
		expect(clone(catalog)).toEqual(clone(bundledCatalog));
