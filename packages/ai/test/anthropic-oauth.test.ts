...
describe("anthropic paste-a-code login", () => {
	...
	it("leaves the loopback login untouched when the flag is absent", () => {
		expect(new AnthropicOAuthFlow({}).redirectUri).toBeUndefined();
		expect(new AnthropicOAuthFlow({}).skipCallbackServer).toBe(false);
		expect(new AnthropicOAuthFlow({}, { manualCode: false }).skipCallbackServer).toBe(false);
		expect(new AnthropicOAuthFlow({}, { manualCode: true }).redirectUri).toBe(ANTHROPIC_MANUAL_REDIRECT_URI);
